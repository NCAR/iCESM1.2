module atm_comp_mct
  ...
  use phys_grid        , only: get_ncols_p, get_gcol_all_p, & 
	                       ...
	                       get_rlon_all_p, get_area_all_p, get_rlat_p, get_rlon_p
         ...
         !Need to define lat/lon for water tracers:
          ...
          do j = 1, wtrc_nsrfvap
            select case(wtrc_species(wtrc_iasrfvap(j)))
              case (isph2o)
                if(j .eq. 1) then !H2O tracer?
                  ...
                  if( -x2a_a%rAttr(index_x2a_Faxx_evap,ig) .lt. 0._r8) then !dew/frost?
                    ...
                    if(j .eq. 5) then    
                   !All Land:
                      cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) =  cam_in(c)%landfrac(i)*-x2a_a%rAttr(index_x2a_Faxx_evap,ig)
                    else if(j .eq. 6) then  
                   !North Pacific:
                    ...
                    !NOTE:  If you have more water tags, you would put them here!
              ...
              case (isph218o)
                cam_in(c)%cflx(i,wtrc_indices(wtrc_iasrfvap(j))) = -x2a_a%rAttr(index_x2a_Faxx_evap_18O,ig)
            end select
          end do
  ...
  subroutine atm_export_mct( cam_out, a2x_a )
    ...
    do c=begchunk, endchunk
       ...
       do i=1,ncols
          ...
          ! NOTE: isph2o is total water, so is the same as Q
          if(trace_water) then 
            a2x_a%rAttr(index_a2x_Sa_shum_16O   ,ig) = 0._r8
            a2x_a%rAttr(index_a2x_Sa_shum_HDO   ,ig) = 0._r8
            a2x_a%rAttr(index_a2x_Sa_shum_18O   ,ig) = 0._r8
              ...
              select case(wtrc_species(wtrc_iasrfvap(j)))
                case (isph216o)
                  if(pass16) then !pass on H216O?
                    a2x_a%rAttr(index_a2x_Sa_shum_16O   ,ig) = cam_out(c)%qbot(i,wtrc_indices(wtrc_iasrfvap(j))) 
                    pass16 = .false.
                  end if
                case (isphdo)
                  if(passD) then !pass on HDO?
                    a2x_a%rAttr(index_a2x_Sa_shum_HDO   ,ig) = cam_out(c)%qbot(i,wtrc_indices(wtrc_iasrfvap(j))) 
                    passD = .false.
                  end if
                case (isph218o)
                  if(pass18) then !pass on H218O?
                    a2x_a%rAttr(index_a2x_Sa_shum_18O   ,ig) = cam_out(c)%qbot(i,wtrc_indices(wtrc_iasrfvap(j))) 
                    pass18 = .false.
                  end if
              end select
            end do
          ...
          !NOTE:  converting m/s to kg/m2/s here too(may need to convert snow to equiv. water???):
            a2x_a%rAttr(index_a2x_Faxa_rainl_16O,ig)=cam_out(c)%precrl_16O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowl_16O,ig)=cam_out(c)%precsl_16O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_rainc_16O,ig)=cam_out(c)%precrc_16O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowc_16O,ig)=cam_out(c)%precsc_16O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_rainl_HDO,ig)=cam_out(c)%precrl_HDO(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowl_HDO,ig)=cam_out(c)%precsl_HDO(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_rainc_HDO,ig)=cam_out(c)%precrc_HDO(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowc_HDO,ig)=cam_out(c)%precsc_HDO(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_rainl_18O,ig)=cam_out(c)%precrl_18O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowl_18O,ig)=cam_out(c)%precsl_18O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_rainc_18O,ig)=cam_out(c)%precrc_18O(i)*1000._r8
            a2x_a%rAttr(index_a2x_Faxa_snowc_18O,ig)=cam_out(c)%precsc_18O(i)*1000._r8
          end if
          !----------------------
