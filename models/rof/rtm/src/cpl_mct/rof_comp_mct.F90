...
! in MCT (Model Coupling Toolkit) format and converting it to use by RTM
  ...
  use RtmVar           , only : rtmlon, rtmlat, ice_runoff, iulog, &
                                ...
                                rtm_active, flood_active, wiso_runoff
...
!====================================================================================
  ...
  subroutine rof_import_mct( x2r_r, totrunin)
    ...
    do n = begr,endr
       ...
       if ( wiso_runoff ) then
          totrunin(n,3) = x2r_r%rAttr(index_x2r_Flrl_rofliq_16O,n2)
          totrunin(n,4) = x2r_r%rAttr(index_x2r_Flrl_rofice_16O,n2)
          totrunin(n,5) = x2r_r%rAttr(index_x2r_Flrl_rofliq_18O,n2)
          totrunin(n,6) = x2r_r%rAttr(index_x2r_Flrl_rofice_18O,n2)
          totrunin(n,7) = x2r_r%rAttr(index_x2r_Flrl_rofliq_HDO,n2)
          totrunin(n,8) = x2r_r%rAttr(index_x2r_Flrl_rofice_HDO,n2)
       end if
...
!====================================================================================
  ...
  subroutine rof_export_mct( r2x_r )
    ...
    do n = runoff%begr, runoff%endr
       ...
       ! adding water isotopes
       ! if ( wiso_runoff ) then
       !    r2x_r%rattr(index_r2x_Flrr_flood,ni) = -runoff%flood(n,1)
       !    r2x_r%rattr(index_r2x_Flrr_flood_16O,ni) = -runoff%flood(n,2)
       !    r2x_r%rattr(index_r2x_Flrr_flood_18O,ni) = -runoff%flood(n,3)
       !    r2x_r%rattr(index_r2x_Flrr_flood_HDO,ni) = -runoff%flood(n,4)
       ! end if
    ...
    do n = runoff%begr, runoff%endr
      ni = ni + 1
       ...
       ! adding water isotopes
       ! volr does not include ice part?
       if ( wiso_runoff ) then
          r2x_r%rattr(index_r2x_Slrr_volr_16O,ni)    = runoff%volr_nt3(n) &
                                                     / (runoff%area(n))
          r2x_r%rattr(index_r2x_Slrr_volr_18O,ni)    = runoff%volr_nt5(n) &
                                                     / (runoff%area(n))
          r2x_r%rattr(index_r2x_Slrr_volr_HDO,ni)    = runoff%volr_nt7(n) &
                                                     / (runoff%area(n))
       end if
    end do
