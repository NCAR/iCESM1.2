...
!-----------------------------------------------------------------------
  ...
  subroutine RtmFloodInit(frivinp, begr, endr, fthresh, evel, &
    ...
    real(r8), parameter :: effvel4_0 = 0.35_r8   ! downstream velocity (m/s)

    real(r8), parameter :: effvel4_5 = 1.0_r8   ! downstream velocity (m/s)
    real(r8), parameter :: min_ev4_5 = 0.05_r8  ! minimum downstream velocity (m/s)
    ...
    if (is_effvel_on) then
       ...
       do nt = 1,nt_rtm
          do n = begr, endr
             ...
             evel(n,nt) = max(min_ev4_5,effvel4_5*sqrt(max(0._r8,rslope(n)))) 
          end do
       end do
    else
