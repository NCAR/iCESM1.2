...
SUBROUTINE coarsen(f,fcoarse,factor,n,dA_coarse)
  use shr_kind_mod, only: r8 => shr_kind_r8
  IMPLICIT NONE
  REAL (R8), DIMENSION(n)       , INTENT(IN)  :: f
  REAL (R8), DIMENSION(n/factor), INTENT(OUT) :: fcoarse             
  INTEGER                                          , INTENT(IN)  :: factor,n
  REAL(R8), DIMENSION(INT(SQRT(DBLE(n/6)))/factor,INT(SQRT(DBLE(n/6)))/factor),INTENT(OUT) :: dA_coarse
  ...
  !        REAL(R8), DIMENSION(INT(SQRT(DBLE(n/6)))/factor,INT(SQRT(DBLE(n/6)))/factor) :: dAtmp
  
  ncube = INT(SQRT(DBLE(n/6)))
  coarse_ncube = ncube/factor
  
  IF (ABS(DBLE(ncube)/DBLE(factor)-coarse_ncube)>0.000001) THEN
    WRITE(*,*) "ncube/factor must be an integer"
    WRITE(*,*) "ncube and factor: ",ncube,factor
    STOP
  END IF
  ...
  DO jp=1,6
    DO jy_coarse=1,coarse_ncube
      DO jx_coarse=1,coarse_ncube
        ...
        sum_area = 0.0
        DO jy_s=1,factor
          jy = (jy_coarse-1)*factor+jy_s
          DO jx_s=1,factor
            jx = (jx_coarse-1)*factor+jx_s
            ii = (jp-1)*ncube*ncube+(jy-1)*ncube+jx
...
SUBROUTINE overlap_weights(weights_lgr_index_all,weights_eul_index_all,weights_all,&
  ...
  DO i=1,ntarget
    ...
    CALL remove_duplicates_latlon(ncorner,target_corner_lon(:,i),target_corner_lat(:,i),&
         ncorner_this_cell,lon,lat,tiny)
    ...
    DO ip = 1,k
      ...
      !            IF (jcollect>50) WRITE(*,*) "WARNING WARNING WARNING"
    END DO
    !          IF (i==5) stop
  END DO
  jall = jall-1
...
SUBROUTINE remove_duplicates_latlon(n_in,lon_in,lat_in,n_out,lon_out,lat_out,tiny)
  use shr_kind_mod, only: r8 => shr_kind_r8
  integer, intent(in) :: n_in
  real(r8),dimension(n_in), intent(in) :: lon_in,lat_in
  real, intent(in) :: tiny
  integer, intent(out) :: n_out
