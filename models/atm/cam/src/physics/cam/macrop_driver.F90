...
!-------------------------------------------------
   ...
   if (trace_water) then
     ...
     call wtrc_init_rates(process_rates)
     ...
     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtvap, iwtvap, qvlat + qcten + qiten)

     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtliq, iwtvap, pqctn)
     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtliq, iwtliq, nqctn)
     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtice, iwtvap, pqitn)
     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtice, iwtice, nqitn)

    !debugging:
!     call wtrc_add_rates(process_rates, ncol, iwtvap, iwtvap, qvlat, do_reverse=.false. )
!     call wtrc_add_rates(process_rates, ncol, iwtliq, iwtliq, qcten, do_reverse=.false. )
!     call wtrc_add_rates(process_rates, ncol, iwtice, iwtice, qiten, do_reverse=.false. )

     ! Apply these rates.
     call wtrc_apply_rates(state_loc, ptend_loc, pbuf, dtime, .false., pre_rates=process_rates, &
                           prelat=tlat)
