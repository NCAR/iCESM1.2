...
!     end do
  ...
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
   ...
   ! 3. Variables associated with cumulus convection
    ...
    real(r8)    rain_radius                                   !  raindrop radius [m] !TESTING
    real(r8)    fequil                                        !  fraction of rain that has experience equilibration !TESTING     
    real(r8)    dz(0:mkx)                                     !  layer thickness [m] !TESTING
    integer     ispec                                         !  water isotope species !TESTING
    !************
 ...
 45    continue
       ...
       if( fer(kpen)*(-ppen) .lt. 1.e-4_r8 ) then
          ...
          !************* 
           if(trace_water) then
             do m=1,wtrc_nwset
               wtu_top(m) = wtu(kpen-1,m) + ( wt0(kpen,m) +  sswt0(kpen,m) * (-ppen) / 2._r8  - wtu(kpen-1,m) ) * fer(kpen) * (-ppen)
             end do
           end if
          !*************
       else
       ...
       if(trace_water) then
         wtout(:,:) = 0._r8
         call conden(ps0(kpen-1)+ppen,thlu_top,qtu_top,thj,qvj,qlj,qij,qse,id_check,wtrc_nwset,qv0=qtu_top,tr0=wtu_top(:),wtout=wtout)
       else
       ...
       do k = 1, kpen
         ...
         !*************
          ...
          elseif( k .eq. kpen ) then 
              if(trace_water) then
                wtout(:,:) = 0.0_r8
                call conden(ps0(k-1)+ppen,thlu_top,qtu_top,thj,qvj,qlj,qij,qse,id_check,wtrc_nwset,qv0=qtu_top,tr0=wtu_top(:),wtout=wtout)
              else
      ...
      !*************
               ...
               !apply epsilon:
                ...
                if((fr .eq. 0._r8) .and. (bet .lt. 0._r8)) then !preven NaNs (plus there shouldn't be any evaporation anyways)
                  ...
                  if(dp0(k) .gt. 0._r8) then !prevent NaNs
                    !wtevp(k,m) = (wtflxrn(k,m) - Rstw*(wtflxrn(k,1)-(wtevp(k,1)*dp0(k)/g)))*g/dp0(k)
       ...
       !                if we use new 'qlten,qiten', there is no problem.
        ...
        if(trace_water) then
          ...
          call positive_moisture_single( xlv, xls, mkx, dt, qmin(1), qmin(ixcldliq), qmin(ixcldice), dp0, qv0_star, ql0_star, qi0_star, s0_star, &
