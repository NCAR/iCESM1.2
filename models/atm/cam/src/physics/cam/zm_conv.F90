...
if(doconvtran(wtrc_iatype(1,iwtliq))) then !are water tracers being transported?
  do m=2,wtrc_ntype(iwtliq) !loop over water tracers
    do k=1,pver
      do i=il1g,il2g
        Rwt(ideep(i),k,m,1) = wtrc_ratio(iwspec(wtrc_iatype(m,iwtliq)),dqdt(ideep(i),k,wtrc_iatype(m,iwtliq)),dqdt(ideep(i),k,wtrc_iatype(1,iwtliq)))
        Rwt(ideep(i),k,m,2) = wtrc_ratio(iwspec(wtrc_iatype(m,iwtice)),dqdt(ideep(i),k,wtrc_iatype(m,iwtice)),dqdt(ideep(i),k,wtrc_iatype(1,iwtice)))
      end do
    end do 
  end do
end if
