...
subroutine micro_mg_cam_tend(state, ptend, dtime, pbuf, cmeliq)
   ...
   real(r8)              :: pre_rates(pcols,pver,pwtype,pwtype,pwtype)    ! Process rates (kg/kg/sec)
   real(r8)              :: sed_rates(pcols,pver,pwtype)                  ! Sedimentation rates (kg/kg/sec)
   real(r8)              :: post_rates(pcols,pver,pwtype,pwtype,pwtype)   ! Process rates (kg/kg/sec)
   real(r8)              :: preo(pcols,pver)                              ! rain re-evaporation (kg/kg/sec)
   real(r8)              :: prdso(pcols,pver)                             ! snow sublimation (kg/kg/sec)
   real(r8)              :: frzro(pcols,pver)                             ! rain freezing (kg/kg/sec)
   real(r8)              :: meltso(pcols,pver)                            ! snow melting  (kg/kg/sec)
   real(r8)              :: wtfc(pcols,pver)                              ! Initial cloud liquid fall velocity
   real(r8)              :: wtfi(pcols,pver)                              ! Initial cloud ice fall velocity
   real(r8)              :: wtprelat(pcols,pver)                          ! Latent heat change due to pre_rates
   real(r8)              :: wtpostlat(pcols,pver)                         ! Latent heat change due to post_rates 
   real(r8)              :: pcmei(pcols,pver)                             ! Positive cmeiout - deposition
   real(r8)              :: ncmei(pcols,pver)                             ! Negative cmeiout - sublimation
   real(r8)              :: pmelts(pcols,pver)                            ! Positive melts - melting
   real(r8)              :: nmelts(pcols,pver)                            ! Negative melts (freezing?)
   ...
   !-------------------------------------
    ...
    if (trace_water) then
         ...
         !create state and ptend copy for apply_rates call:
!         call physics_state_copy(state, state_loc) 
!         call physics_ptend_copy(ptend, ptend_loc)

         ! Rain & snow are not currently prognostic, so fill in the temporary
         ! state variables with the diagnosed precip.
         ! NOTE:  Not sure if the bulk precip variables are used, and thus this code
         ! may not be necessary - JN.
         if (wtrc_add_stprecip) then
!           ptend_loc%lq(wtrc_bulk_indices(iwtstrain))        = .true.
!           ptend_loc%lq(wtrc_bulk_indices(iwtstsnow))        = .true.
!           state%q(:ncol,:,wtrc_bulk_indices(iwtstrain)) = 0._r8
!           state%q(:ncol,:,wtrc_bulk_indices(iwtstsnow)) = 0._r8
!           ptend_loc%q(:ncol,:,wtrc_bulk_indices(iwtstrain)) = qrten(:,:)
!           ptend_loc%q(:ncol,:,wtrc_bulk_indices(iwtstsnow)) = qsten(:,:)
         end if

         ! Setup microphysics rates to be applied before sedimentation.
         call wtrc_init_rates(pre_rates)
...
!         call wtrc_add_rates(pre_rates, ncol, iwtvap,    iwtice,    iwtvap, cmeiout)

          call wtrc_add_rates(pre_rates, ncol, iwtvap, iwtice, iwtvap, pcmei) !deposition
          call wtrc_add_rates(pre_rates, ncol, iwtvap, iwtice, iwtice, ncmei) !sublimation  

          call wtrc_add_rates(pre_rates, ncol, iwtvap,    iwtstrain, iwtstrain, preo)  !rain re-evaporation
          call wtrc_add_rates(pre_rates, ncol, iwtvap,    iwtstsnow, iwtstsnow, prdso) !snow sublimation

!          call wtrc_add_rates(pre_rates, ncol, iwtvap, iwtstsnow, iwtstsnow, nprds) !snow sublimation
!          call wtrc_add_rates(pre_rates, ncol, iwtvap, iwtstsnow, iwtvap, pprds) !snow deposition?

         ! Processes that consume liquid
         !Freezing,accretion on ice,and evaporation to deposition:
         call wtrc_add_rates(pre_rates, ncol, iwtliq,    iwtice,    iwtliq, mnuccco + mnuccto + msacwio) 
         !Accretion on rain,autoconversion: 
         call wtrc_add_rates(pre_rates, ncol, iwtliq,    iwtstrain, iwtliq, prao + prco)
         !Accretion on snow, Bergeron process on snow:
         call wtrc_add_rates(pre_rates, ncol, iwtliq,    iwtstsnow, iwtliq, psacwso)
         ...
         call wtrc_add_rates(pre_rates, ncol, iwtliq, iwtliq, iwtliq, bergo)
         call wtrc_add_rates(pre_rates, ncol, iwtice, iwtice, iwtice, bergso) 

         ! Processes that consume ice
         !Ice accretion on snow,autoncversion of snow:
         call wtrc_add_rates(pre_rates, ncol, iwtice,    iwtstsnow, iwtice, praio + prcio)

         ! Processes that consume rain
         !Accretion on snow, freezing (hetero and homo?):
!         call wtrc_add_rates(pre_rates, ncol, iwtstrain, iwtstsnow, iwtstrain, pracso + mnuccro + frzro)
        !don't include freezing of rain (handled separately):
         call wtrc_add_rates(pre_rates, ncol, iwtstrain, iwtstsnow, iwtstrain, pracso + mnuccro)

         ! Processes that consume snow
         !Melting of snow: 
         !call wtrc_add_rates(pre_rates, ncol, iwtstsnow, iwtstrain, iwtstsnow, meltso)

         !don't include melting of snow (handled separately):
!         call wtrc_add_rates(pre_rates, ncol, iwtstsnow, iwtstrain, iwtstsnow, pmelts)
!         call wtrc_add_rates(pre_rates, ncol, iwtstsnow, iwtstrain, iwtstrain, nmelts)

         ! Setup  sedimentation.
         sed_rates(:, :, :)        = 0._r8
         sed_rates(:, :, iwtliq)   = qcsedten
         sed_rates(:, :, iwtice)   = qisedten

         ! Setup microphysics rates to be applied after sedimentation.
         call wtrc_init_rates(post_rates)

         ! Processes that consume water vapor.
         !Condenstation (sets supersat = 0):
         call wtrc_add_rates(post_rates, ncol, iwtvap,    iwtliq,   iwtvap, qcreso)
         !Deposition (sets supersat = 0): 
         call wtrc_add_rates(post_rates, ncol, iwtvap,    iwtice,   iwtvap, qireso) !positive
        ...
        ! call wtrc_add_rates(post_rates, ncol, iwtliq,    iwtvap,   qcsevap, do_reverse=.false.)
         !Freezing:
         call wtrc_add_rates(post_rates, ncol, iwtliq,    iwtice,   iwtliq, homoo) !positive
         ...
         ! level. The source of the vapor has already been include in the
         ! sedimentation tendency. (NOTE:  managed in wtrc_sediment - JN)
        ! call wtrc_add_rates(post_rates, ncol, iwtice,    iwtvap,   qisevap, do_reverse=.false.)
         !Melting:
         call wtrc_add_rates(post_rates, ncol, iwtice,    iwtliq,   iwtice, melto) !positive

         !Debugging tendencies:
         !call wtrc_add_rates(pre_rates, ncol, iwtvap,    iwtvap, qvlat, do_reverse=.false.)
         !call wtrc_add_rates(pre_rates, ncol, iwtliq,    iwtliq, qcten, do_reverse=.false.)
         !call wtrc_add_rates(pre_rates, ncol, iwtice,    iwtice, qiten, do_reverse=.false.)
         !call wtrc_add_rates(pre_rates, ncol, iwtstrain, iwtstrain, qrten, do_reverse=.false.)
         !call wtrc_add_rates(pre_rates, ncol, iwtstsnow, iwtstsnow, qsten, do_reverse=.false.)

         !Debugging rate:
       !  call wtrc_apply_rates(state1, ptend_loc, pbuf, dtime, pre_rates=pre_rates, sed_rates=sed_rates, do_stprecip=.true.) 

         ! Apply the microphysical process to the isotopes. rates.
          call wtrc_apply_rates(state, ptend, pbuf, dtime, .true., pre_rates=pre_rates, sed_rates=sed_rates, post_rates=post_rates, &
                                do_stprecip=.true., liqcldf=alst_mic, icecldf=aist_mic, fc=wtfc, fi=wtfi, prelat=wtprelat, postlat=wtpostlat, &
  ...
  subroutine micro_mg1_0_tend_intr
    ...
    call micro_mg_tend( &
	 ...
	 relvar, accre_enhan,                               &
         ...
         tnd_nsnow, re_ice, errstring, preo, prdso,       &
