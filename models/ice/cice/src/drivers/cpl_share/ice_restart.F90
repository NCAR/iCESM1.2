...
!
      ...
      if (tr_iso) then
         do k=1,n_iso
            write(nchar,'(i1.1)') k
            call define_rest_field(File,'isosnossl'//nchar, dims)
            call define_rest_field(File,'isosnoint'//nchar, dims)
            call define_rest_field(File,'isoicessl'//nchar, dims)
            call define_rest_field(File,'isoiceint'//nchar, dims)
         enddo
      endif
      ...
      if (tr_iso) then
         do k=1,n_iso
            ...
            status = pio_inq_varid(File,'isosnossl'//nchar,varid)
            call pio_write_darray(File, varid, iodesc3d_ncat, trcrn(:,:,nt_iso+  (k-1)*4,:,:), status, fillval=c0)

            status = pio_inq_varid(File,'isosnoint'//nchar,varid)
            call pio_write_darray(File, varid, iodesc3d_ncat, trcrn(:,:,nt_iso+1+(k-1)*4,:,:), status, fillval=c0)

            status = pio_inq_varid(File,'isoicessl'//nchar,varid)
            call pio_write_darray(File, varid, iodesc3d_ncat, trcrn(:,:,nt_iso+2+(k-1)*4,:,:), status, fillval=c0)

            status = pio_inq_varid(File,'isoiceint'//nchar,varid)
            call pio_write_darray(File, varid, iodesc3d_ncat, trcrn(:,:,nt_iso+3+(k-1)*4,:,:), status, fillval=c0)
         enddo
      endif
...
!
      ...
      use ice_constants, only: rhos, rhoi
!
! !INPUT/OUTPUT PARAMETERS:
!
...
!
      ...
      !if (tr_iso) trcrn(:,:,nt_iso:nt_iso+n_iso*4-1,:,:) = c0
      if (tr_iso) then
         do it=1,n_iso
            trcrn(:,:,nt_iso+(it-1)*4  :nt_iso+(it-1)*4+1,:,:) = rhos
            trcrn(:,:,nt_iso+(it-1)*4+2:nt_iso+(it-1)*4+3,:,:) = rhoi
         enddo
      endif
      ...
      if (my_task == master_task) then
         ...
         read(nu_restart, end=99) filename_iso
   ...
   99    continue
      ...
      if (my_task == master_task) then
         ...
         write(nu_diag,'(a,a)') 'filename_iso : ',filename_iso
      endif
      ...
      call broadcast_scalar(filename_iso,  master_task)
...
!
      ...
      if (tr_iso) then
         call pio_seterrorhandling(File, PIO_BCAST_ERROR)
         status = pio_inq_varid(File,'isosnossl1',varid)
         if (status == PIO_noerr) then
            do k=1,n_iso
               write(nchar,'(i1.1)') k
               ...
               status = pio_inq_varid(File,'isosnossl'//nchar,varid)
               call pio_read_darray(File, varid, iodesc3d_ncat, &
                                    trcrn(:,:,nt_iso+(k-1)*4,:,:), status)

               status = pio_inq_varid(File,'isosnoint'//nchar,varid)
               call pio_read_darray(File, varid, iodesc3d_ncat, &
                                    trcrn(:,:,nt_iso+1+(k-1)*4,:,:), status)
   
               status = pio_inq_varid(File,'isoicessl'//nchar,varid)
               call pio_read_darray(File, varid, iodesc3d_ncat, &
                                    trcrn(:,:,nt_iso+2+(k-1)*4,:,:), status)

               status = pio_inq_varid(File,'isoiceint'//nchar,varid)
               call pio_read_darray(File, varid, iodesc3d_ncat, &
                                    trcrn(:,:,nt_iso+3+(k-1)*4,:,:), status)
            enddo
         endif
         call pio_seterrorhandling(File, PIO_INTERNAL_ERROR)
      endif
