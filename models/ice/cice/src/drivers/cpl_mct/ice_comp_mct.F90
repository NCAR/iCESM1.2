  ...
  use ice_flux,        only : strairxt, strairyt, strocnxt, strocnyt,    &
			      alvdr, alidr, alvdf, alidf, Tref, Qref, Uref, &
		              ...
		              sss, tf, wind, fsw, init_flux_atm, init_flux_ocn,&
                              faero_atm, fiso_atm, fiso_ocn, fiso_rain,&
                              fiso_evap, fiso_Qa, fiso_Qref,           &
                              HDO_ocn, H2_18O_ocn, H2_16O_ocn
  use ice_state,       only : vice, vsno, aice, trcr, filename_aero, filename_iage, filename_iso, &
                              filename_volpn, filename_FY, filename_lvl, &
                              tr_aero, tr_iso, tr_iage, tr_FY, tr_pond, tr_lvl
    ...
    if (rstwr) then
       ...
       if (restart_format /= 'nc') then
          ...
          if (tr_iso) then
               ...
               string1 = trim(fname(1:n-1))
               string2 = trim(fname(n:lenstr(fname)))
               write(filename_iso,'(a,a,a,a)') &
                  string1(1:lenstr(string1)),'.iso', &
                  string2(1:lenstr(string2))
          endif
          if (tr_iage) then
               n = index(fname,'cice.r') + 6
     ...
     do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            if ( tmask(i,j,iblk) .and. ailohi(i,j,iblk) > c0 ) then
               ...
               i2x_i%rAttr(index_i2x_Fioi_meltw_HDO,n)  = fiso_ocn (i,j,1,iblk)  ! Isotopes to ocean
               i2x_i%rAttr(index_i2x_Fioi_meltw_16O,n)  = fiso_ocn (i,j,2,iblk)  ! Isotopes to ocean
               i2x_i%rAttr(index_i2x_Fioi_meltw_18O,n)  = fiso_ocn (i,j,3,iblk)  ! Isotopes to ocean
               i2x_i%rAttr(index_i2x_Faii_evap_HDO ,n)  = fiso_evap(i,j,1,iblk)  ! Isotope evap to atm
               i2x_i%rAttr(index_i2x_Faii_evap_16O ,n)  = fiso_evap(i,j,2,iblk)  ! Isotope evap to atm
               i2x_i%rAttr(index_i2x_Faii_evap_18O ,n)  = fiso_evap(i,j,3,iblk)  ! Isotope evap to atm
               i2x_i%rAttr(index_i2x_Si_qref_HDO   ,n)  = fiso_Qref(i,j,1,iblk)  ! Isotope qref to atm
               i2x_i%rAttr(index_i2x_Si_qref_16O   ,n)  = fiso_Qref(i,j,2,iblk)  ! Isotope qref to atm
               i2x_i%rAttr(index_i2x_Si_qref_18O   ,n)  = fiso_Qref(i,j,3,iblk)  ! Isotope qref to atm
	 ...
	 first_call = .false.
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            n = n+1
            faero_atm(i,j,1,iblk) = x2i_i%rAttr(index_x2i_Faxa_bcphodry,n)

            faero_atm(i,j,2,iblk) = x2i_i%rAttr(index_x2i_Faxa_bcphidry,n) &
                              + x2i_i%rAttr(index_x2i_Faxa_bcphiwet,n)
         ! Combine all of the dust into one category
            faero_atm(i,j,3,iblk) = x2i_i%rAttr(index_x2i_Faxa_dstwet1,n) &
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            n = n+1
            fiso_Qa(i,j,1,iblk)  = x2i_i%rAttr(index_x2i_Sa_shum_HDO,n)
            fiso_Qa(i,j,2,iblk)  = x2i_i%rAttr(index_x2i_Sa_shum_16O,n)
            fiso_Qa(i,j,3,iblk)  = x2i_i%rAttr(index_x2i_Sa_shum_18O,n)

            fiso_rain(i,j,1,iblk) = x2i_i%rAttr(index_x2i_Faxa_rain_HDO,n)
            fiso_rain(i,j,2,iblk) = x2i_i%rAttr(index_x2i_Faxa_rain_16O,n)
            fiso_rain(i,j,3,iblk) = x2i_i%rAttr(index_x2i_Faxa_rain_18O,n)

            fiso_atm(i,j,1,iblk) = x2i_i%rAttr(index_x2i_Faxa_snow_HDO,n)
            fiso_atm(i,j,2,iblk) = x2i_i%rAttr(index_x2i_Faxa_snow_16O,n)
            fiso_atm(i,j,3,iblk) = x2i_i%rAttr(index_x2i_Faxa_snow_18O,n)

            HDO_ocn(i,j,iblk)    = x2i_i%rAttr(index_x2i_So_roce_HDO,n)
            H2_16O_ocn(i,j,iblk) = x2i_i%rAttr(index_x2i_So_roce_16O,n)
            H2_18O_ocn(i,j,iblk) = x2i_i%rAttr(index_x2i_So_roce_18O,n)
         enddo    !i
         enddo    !j
