...
! !USES:
  ...
  use ice_flux,        only : strairxt, strairyt, strocnxt, strocnyt,    &
		              ...
		              sss, tf, wind, fsw, init_flux_atm, init_flux_ocn,&
                              faero_atm, faero_iso
  use ice_state,       only : vice, vsno, aice, trcr, filename_aero, filename_iso, filename_iage, &
     ...
     !-------------------------------------------------------
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            n = n+1
            faero_atm(i,j,1,iblk) = fptr(index_x2i_Faxa_bcphodry,n)

            faero_atm(i,j,2,iblk) = fptr(index_x2i_Faxa_bcphidry,n) &
                              + fptr(index_x2i_Faxa_bcphiwet,n)
         ! Combine all of the dust into one category
            faero_atm(i,j,3,iblk) = fptr(index_x2i_Faxa_dstwet1,n) &
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            n = n+1
            fiso_atm(i,j,1,iblk) = fptr(index_x2i_Faxa_HDO,n)
            fiso_atm(i,j,2,iblk) = fptr(index_x2i_Faxa_H2_16O,n)
            fiso_atm(i,j,3,iblk) = fptr(index_x2i_Faxa_H2_18O,n)
