...
! !USES:
!
      use ice_state, only: nt_Tsfc, nt_aero, nt_iso
!
! !INPUT/OUTPUT PARAMETERS:
!
      integer (kind=int_kind), intent(in) :: &
...
!ocl novrec      !Fujitsu
          do ij = 1, icells
           ...
           do it=1,n_iso
            xtmp &
              = (vsnon(i,j,n)*(trcrn(i,j,nt_iso  +4*(it-1),n)   &
...
! vsnon and vicen have been zapped, so these should be moved
! upward, i.e., zap the tracers first.
!       if (tr_iso) then
!!DIR$ CONCURRENT !Cray
!!cdir nodep      !NEC
!!ocl novrec      !Fujitsu
