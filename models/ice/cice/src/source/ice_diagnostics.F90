...
!
      ...
      real (kind=dbl_kind), dimension(n_isomx) :: &
...
!
      ...
      real (kind=dbl_kind) :: &
         ...
         ftmp,faeron,faeros,faero_ocnn,faero_ocns,     &
...
! for isotope diagnostics
      integer (kind=int_kind) :: &
        kiso, niso
      real (kind=dbl_kind) :: &
        isomx1n, isomx1s, isomx2n, isomx2s, &
        isomx3n, isomx3s, isomx4, &
        isototn, isotots
...
! put in aerosol diagnostics
      if (tr_aero) then
         ! aerosols
        do naero=1,n_aero
         faeron = global_sum_prod(faero_atm(:,:,naero,:), aice_init, distrb_info, &
                               field_loc_center, tarean)
         faeros = global_sum_prod(faero_atm(:,:,naero,:), aice_init, distrb_info, &
                               field_loc_center, tareas)
         faeron = faeron*dt
         faeros = faeros*dt

         faero_ocnn = global_sum_prod(faero_ocn(:,:,naero,:), aice, distrb_info, &
                               field_loc_center, tarean)
         faero_ocns = global_sum_prod(faero_ocn(:,:,naero,:), aice, distrb_info, &
                               field_loc_center, tareas)
         faero_ocnn = faero_ocnn*dt
         faero_ocns = faero_ocns*dt
         ...
         if (my_task == master_task) then
          write(nu_diag,*) 'aero: ',naero,' faero_atm         : ',&
                faeron, faeros
          write(nu_diag,*) 'aero: ',naero,' faero_ocn         : ',&
                faero_ocnn, faero_ocns
          write(nu_diag,*) 'aero: ',naero,' faero_atm-faero_ocn   : ',&
                faeron-faero_ocnn, faeros-faero_ocns
...
! put in isosol diagnostics
      if (tr_iso) then
         ! isotopes
        do niso=1,n_iso
         fison = global_sum_prod(fiso_atm(:,:,niso,:), aice_init, distrb_info, &
                               field_loc_center, tarean)
         fisos = global_sum_prod(fiso_atm(:,:,niso,:), aice_init, distrb_info, &
                               field_loc_center, tareas)
         fison = fison*dt
         fisos = fisos*dt

         fiso_ocnn = global_sum_prod(fiso_ocn(:,:,niso,:), aice, distrb_info, &
                               field_loc_center, tarean)
         fiso_ocns = global_sum_prod(fiso_ocn(:,:,niso,:), aice, distrb_info, &
                               field_loc_center, tareas)
         fiso_ocnn = fiso_ocnn*dt
         fiso_ocns = fiso_ocns*dt

         do iblk = 1, nblocks
           do j = 1, ny_block
           do i = 1, nx_block
            work1(i,j,iblk) = trcr(i,j,nt_iso  +4*(niso-1),iblk)*vsno(i,j,iblk) &
                            + trcr(i,j,nt_iso+1+4*(niso-1),iblk)*vsno(i,j,iblk) &
                            + trcr(i,j,nt_iso+2+4*(niso-1),iblk)*vice(i,j,iblk) &
                            + trcr(i,j,nt_iso+3+4*(niso-1),iblk)*vice(i,j,iblk)
           enddo
           enddo
         enddo
         isototn= global_sum(work1, distrb_info, field_loc_center, tarean)
         isotots= global_sum(work1, distrb_info, field_loc_center, tareas)
         isomx1n = global_maxval(work1, distrb_info, lmask_n)
         isomx1s = global_maxval(work1, distrb_info, lmask_s)
         if (my_task == master_task) then
          write(nu_diag,*) 'iso: ',niso,' fiso_atm         : ',&
                fison, fisos
          write(nu_diag,*) 'iso: ',niso,' fiso_ocn         : ',&
                fiso_ocnn, fiso_ocns
          write(nu_diag,*) 'iso: ',niso,' fiso_atm-fiso_ocn   : ',&
                fison-fiso_ocnn, fisos-fiso_ocns
          write(nu_diag,*) 'iso: ',niso,' isotot       : ',&
                isototn, isotots
          write(nu_diag,*) 'iso: ',niso,' isotot change: ',&
                isototn-totison(niso), isotots-totisos(niso)
          write(nu_diag,*) 'iso: ',niso,' isomax agg: ',&
                isomx1n,isomx1s
         endif
...
!           do j = 1, ny_block
!           do i = 1, nx_block
!            work1(i,j,iblk) = trcrn(i,j,nt_iso,kiso,iblk)
!           enddo
!           enddo
!          enddo
!          isomx1n = global_maxval(work1, distrb_info, lmask_n)
!          isomx1s = global_maxval(work1, distrb_info, lmask_s)
!          if (my_task == master_task) &
!           write(nu_diag,*) 'MH isomx1s: ',isomx1n,isomx1s,kiso
!         enddo

!       do iblk = 1, nblocks
!         do j = 1, ny_block
!         do i = 1, nx_block
!            work1(i,j,iblk) = trcrn(i,j,nt_iso+1,1,iblk)
!         enddo
!         enddo
!       enddo
!       isomx2n = global_maxval(work1, distrb_info, lmask_n)
!       write(nu_diag,*) 'MH isomx2n: ',isomx2n
!       isomx2s = global_maxval(work1, distrb_info, lmask_s)
!       write(nu_diag,*) 'MH isomx2s: ',isomx2s
!
!       do iblk = 1, nblocks
!         do j = 1, ny_block
!         do i = 1, nx_block
!            work1(i,j,iblk) = trcrn(i,j,nt_iso+2,1,iblk)
!         enddo
!         enddo
!       enddo
!       isomx3n = global_maxval(work1, distrb_info, lmask_n)
!       write(nu_diag,*) 'MH isomx2n: ',isomx3n
!       isomx3s = global_maxval(work1, distrb_info, lmask_s)
...
!
      ...
      integer (kind=int_kind) :: naero, niso
      ...
      if (tr_iso) then
       do niso=1,n_iso
        do iblk = 1, nblocks
         ...
         enddo
        enddo
        totison(niso)= global_sum(work1, distrb_info, field_loc_center, tarean)
        totisos(niso)= global_sum(work1, distrb_info, field_loc_center, tareas)
       enddo
      endif
