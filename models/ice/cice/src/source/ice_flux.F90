...
!
      ...
      real (kind=dbl_kind), &
         ...
         fiso_Qa,  & ! isotope surface specific humidity (kg/kg)
         fiso_atm, & ! isotope deposition rate (kg/m^2 s)
         fiso_rain   ! isotope rainfall rate from atm (kg/m^2 s)
      ...
      real (kind=dbl_kind), &
        dimension (nx_block,ny_block,n_isomx,max_blocks) :: &
         ...
         fiso_Qref    ! 2m atm reference spec humidity (kg/kg)
...
!echmod
      ...
      fiso_Qref (:,:,:,:)   = c0  ! 2m atm reference spec humidity (kg/kg)
...
!
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block,n_isomx), intent(in):: &
          fiso_Qrefn    , & ! air sp hum reference level      (kg/kg)
          fiso_evapn    , & ! evaporation                     (kg/m2/s)
          fiso_ocnn         ! isotopfic water flux to ocn     (kg/m2/s)
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block,n_isomx), intent(inout):: &
          fiso_Qref     , & ! air sp hum reference level      (kg/kg)
          fiso_evap     , & ! evaporation                     (kg/m2/s)
          fiso_ocn          ! isotopic water flux to ocn      (kg/m2/s)
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         ...
         fiso_Qref(i,j,:) = fiso_Qref(i,j,:) + fiso_Qrefn(i,j,:)*aicen(i,j)
