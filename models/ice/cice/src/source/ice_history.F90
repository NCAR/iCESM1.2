...
!          f_example   = 'mdxxx', &
           ...
           f_fiso_atm  = 'mxxxx', f_fiso_ocn   = 'mxxxx', &
           f_fiso_evap = 'mxxxx', f_fiso_Qref  = 'mxxxx', &
...
!     call broadcast_scalar (f_example, master_task)
      ...
      call broadcast_scalar (f_fiso_evap, master_task)
      call broadcast_scalar (f_fiso_Qref, master_task)
      ...
      if (f_fiso_atm(1:1) /= 'x') then
         do n=1,n_iso
            ...
            call define_hist_field(n_fiso_atm(n,:),vname_in,"kg/m^2 s", &
                tstr, tcstr,"isotope deposition rate","none", c1, c0,    &
                ns1, f_fiso_atm)
         enddo
      endif
      ...
      if (f_fiso_ocn(1:1) /= 'x') then
         do n=1,n_iso
            ...
            call define_hist_field(n_fiso_ocn(n,:),vname_in,"kg/m^2 s", &
                tstr, tcstr,"isotope flux to ocean","none", c1, c0,      &
                ns1, f_fiso_ocn)
         enddo
      endif

      if (f_fiso_evap(1:1) /= 'x') then
         do n=1,n_iso
            write(nchar,'(i3.3)') n
            write(vname_in,'(a,a)') 'fiso_evap', trim(nchar)
            call define_hist_field(n_fiso_evap(n,:),vname_in,"cm/day", &
                tstr, tcstr,"isotope evaporation flux","none",           &
                mps_to_cmpdy/rhofresh, c0, ns1, f_fiso_evap)
         enddo
      endif

      if (f_fiso_Qref(1:1) /= 'x') then
         do n=1,n_iso
            write(nchar,'(i3.3)') n
            write(vname_in,'(a,a)') 'fiso_Qref', trim(nchar)
            call define_hist_field(n_fiso_Qref(n,:),vname_in,"g/kg", &
                tstr, tcstr,"isotope 2m reference specific humidity",&
                "none", kg_to_g, c0, ns1, f_fiso_Qref)
         enddo
      endif
...
!           call accum_hist_field(n_sig2, iblk, sig2(:,:,iblk))
        ...
        if (f_fiso_evap(1:1) /= 'x') then
           do n=1,n_iso
              call accum_hist_field(n_fiso_evap(n,:), iblk, &
                                    fiso_evap(:,:,n,iblk))
           enddo
        endif
        ...
        if (f_fiso_Qref(1:1) /= 'x') then
           do n=1,n_iso
              call accum_hist_field(n_fiso_Qref(n,:), iblk, &
                                    fiso_Qref(:,:,n,iblk))
           enddo
        endif
        ...
        if (f_iso(1:1) /= 'x') then
           do n=1,n_iso
              call accum_hist_field(n_isosn1(n,:), iblk, &
                                 trcr(:,:,nt_iso  +4*(n-1),iblk)/rhos)
              call accum_hist_field(n_isosn2(n,:), iblk, &
                                 trcr(:,:,nt_iso+1+4*(n-1),iblk)/rhos)
              call accum_hist_field(n_isoic1(n,:), iblk, &
                                 trcr(:,:,nt_iso+2+4*(n-1),iblk)/rhoi)
              call accum_hist_field(n_isoic2(n,:), iblk, &
                                 trcr(:,:,nt_iso+3+4*(n-1),iblk)/rhoi)
           enddo
        endif
