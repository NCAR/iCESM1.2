...
!     call broadcast_array(histfreq_n(:),       master_task)
      ...
      if (my_task == master_task) then
         ...
         if (tr_iso) then
            nt_iso = ntrcr + 1
            ntrcr = ntrcr + n_iso*4 ! 2 for snow and 2 for ice
         else
            nt_iso = max_ntrcr
         endif
...
!
      ...
      if (tr_iso) then
        do n=1,n_iso
         trcr_depend(nt_iso+(n-1)*4  :nt_iso+(n-1)*4+1) = 2 ! snow volume-weighted
         trcr_depend(nt_iso+(n-1)*4+2:nt_iso+(n-1)*4+3) = 1 ! volume-weighted
        enddo
      endif
