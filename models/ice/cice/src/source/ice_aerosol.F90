...
!     basal ice melt.  Assume all aerosols lost in basal melt
       do ij = 1, icells
        i = indxi(ij)
        j = indxj(ij)
      ...
      do ij = 1, icells
        ...
        if (dhs_snoice(ij) > puny) then
         do k=1,n_aero
          ...
          faero_ocn(i,j,k)=faero_ocn(i,j,k)+kscavsi(k)*(sloss2+sloss1)/dt
         enddo
...
!     redistribute aerosol within vertical layers
      do ij = 1, icells
         ...
         if (vicen(i,j) > puny) then ! may want a limit on hi instead?
          ...
          faero_ocn(i,j,:)=faero_ocn(i,j,:)+(aeroice(i,j,:,1)+aeroice(i,j,:,2))/dt
          aeroice(i,j,:,:) = c0
         endif
         ...
         do k=1,n_aero
          ...
          if ( ( (aerotot(ij,k)-aerotot0(ij,k)) &
             ...
             -  (faero_ocn(i,j,k)-faero_ocn_old(i,j,k)) )*dt ) > 0.00001) then
            write(nu_diag,*) 'aerosol tracer:      ',k
            write(nu_diag,*) 'aerotot-aerotot0     ',aerotot(ij,k)-aerotot0(ij,k) 
            write(nu_diag,*) 'faero-faero_ocn          ',faero_atm(i,j,k)*aicen(i,j)*dt &
                                                   -(faero_ocn(i,j,k)-faero_ocn_old(i,j,k))*dt
          endif
         enddo
      enddo
...
!       enddo
      ...
      do ij = 1, icells
       ...
       .or. trcrn(i,j,nt_aero+2) < -puny .or. trcrn(i,j,nt_aero+3) < -puny) then
           ...
           write(nu_diag,*) 'INT neg in aerosol my_task = ',&
                               my_task &
                               ,' printing point = ',n &
                               ,' i and j = ',i,j
           write(nu_diag,*) 'Int Neg aero snowssl= ',aerosno0(i,j,1,1)
           write(nu_diag,*) 'Int Neg aero new snowssl= ',aerosno(i,j,1,1)
           write(nu_diag,*) 'Int Neg aero snowint= ',aerosno0(i,j,1,2)
           write(nu_diag,*) 'Int Neg aero new snowint= ',aerosno(i,j,1,2)
           write(nu_diag,*) 'Int Neg aero ice_ssl= ',aeroice0(i,j,1,1)
           write(nu_diag,*) 'Int Neg aero new ice_ssl= ',aeroice(i,j,1,1)
           write(nu_diag,*) 'Int Neg aero ice_int= ',aeroice0(i,j,1,2)
           write(nu_diag,*) 'Int Neg aero new ice_int= ',aeroice(i,j,1,2)
           write(nu_diag,*) 'Int Neg aero aicen= ',aicen(i,j)
           write(nu_diag,*) 'Int Neg aero vicen= ',vicen(i,j)
           write(nu_diag,*) 'Int Neg aero vsnon= ',vsnon(i,j)
           write(nu_diag,*) 'Int Neg aero viceold= ',vice_old(i,j)
           write(nu_diag,*) 'Int Neg aero vsnoold= ',vsno_old(i,j)
           write(nu_diag,*) 'Int Neg aero melts= ',melts(i,j)
           write(nu_diag,*) 'Int Neg aero meltt= ',meltt(i,j)
           write(nu_diag,*) 'Int Neg aero meltb= ',meltb(i,j)
           write(nu_diag,*) 'Int Neg aero congel= ',congel(i,j)
           write(nu_diag,*) 'Int Neg aero snoice= ',snoice(i,j)
           write(nu_diag,*) 'Int Neg aero evap sno?= ',dhs_evap(ij)
           write(nu_diag,*) 'Int Neg aero evap ice?= ',dhi_evap(ij)
           write(nu_diag,*) 'Int Neg aero fsnow= ',fsnow(i,j)
           write(nu_diag,*) 'Int Neg aero faero_atm= ',faero_atm(i,j,1)
           write(nu_diag,*) 'Int Neg aero faero_ocn= ',faero_ocn(i,j,1)
...
!
      ...
      if (my_task == master_task) then
         ...
         if (present(filename_spec)) then
            ...
            if (n == 0) call abort_ice('aerosol restart: filename discrepancy')
