...
!
      ...
      real (kind=dbl_kind), dimension (icells,n_aeromx) :: &
         maero_ocn       ! mass of aerosol added to ocean (kg m-2)
...
!         aopen    (ij) = c0
      ...
      if (present(faero_ocn)) then
         do ij = 1, icells
            i = indxi(ij)
            j = indxj(ij)
            faero_ocn(i,j,:) = faero_ocn(i,j,:) + maero_ocn(ij,:)*dti
         enddo
      endif
      if (present(fiso_ocn)) then
         do ij = 1, icells
            i = indxi(ij)
            j = indxj(ij)
            fiso_ocn(i,j,:) = fiso_ocn(i,j,:) + miso_ocn(ij,:)*dti
         enddo
      endif
...
! !USES:
!
      use ice_state, only: nt_aero, nt_iso, &
...
!
      ...
      real (kind=dbl_kind), dimension(icells,n_aeromx), intent(inout) :: &
         maero_ocn      ! mass of aerosols added to ocean (kg m-2)
...
!ocl novrec      !Fujitsu
      ...
      !-----------------------------------------------------------------
            ...
            if (n_aero >= 1) then
               do iaero=1,n_aero
                maero_ocn(m,iaero) = maero_ocn(m,iaero) &
      ...
      !-----------------------------------------------------------------
      !  Place part of the isotopes lost by ridging into the ocean.
      !-----------------------------------------------------------------

            if (n_iso >= 1) then
               do iiso=1,n_iso
                miso_ocn(m,iiso) = miso_ocn(m,iiso) &
                        + vsrdgn(ij)*(c1-fsnowrdg) &
                        *(trcrn(i,j,nt_iso  +4*(iiso-1),n)   &
                        + trcrn(i,j,nt_iso+1+4*(iiso-1),n))
               enddo
            endif
