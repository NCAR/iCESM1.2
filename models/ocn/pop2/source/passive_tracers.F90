...
! !REVISION HISTORY:
!  SVN:$Id: passive_tracers.F90 89614 2018-07-31 19:45:27Z jzhu47@wisc.edu $
...
!-----------------------------------------------------------------------
   ...
   logical (kind=log_kind) ::  &
      ecosys_on, cfc_on, wiso_on, nd_on, iage_on, moby_on, abio_dic_dic14_on
...
!-----------------------------------------------------------------------
   ...
   call broadcast_scalar(nd_on,             master_task)
...
!-----------------------------------------------------------------------
   ...
   if (abio_dic_dic14_on .and. .not. registry_match('lcoupled')) then
      call exit_POP(sigAbort,'Abiotic DIC-DIC14 module requires the flux coupler')
   end if
...
!-----------------------------------------------------------------------
   ...
   if (nd_on) then
      call set_tracer_indices('ND', nd_tracer_cnt, cumulative_nt,  &
                              nd_ind_begin, nd_ind_end)
   end if
...
!-----------------------------------------------------------------------
if (nd_on) then
      call nd_init(init_ts_file_fmt, read_restart_filename, &
                     tracer_d(nd_ind_begin:nd_ind_end), &
                     TRACER(:,:,:,nd_ind_begin:nd_ind_end,:,:), &
                     errorCode)

      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_passive_tracers: error in nd_init')
         return
      endif
...
!-----------------------------------------------------------------------
   ...
   if (nd_on) then
      call nd_set_interior(k,                                    &
         TRACER(:,:,:,nd_ind_begin:nd_ind_end,oldtime,bid),      &
         TRACER(:,:,:,nd_ind_begin:nd_ind_end,curtime,bid),      &
         TRACER_SOURCE (:,:,nd_ind_begin:nd_ind_end) ,this_block)
   end if
...
!-----------------------------------------------------------------------
!  ND does not compute and store 3D source-sink terms
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  ND does not have surface fluxes
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  ND does not write additional restart fields
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  Nd does not use virtual fluxes
!-----------------------------------------------------------------------
