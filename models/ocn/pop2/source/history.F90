...
!
! !REVISION HISTORY:
!  SVN:$Id: history.F90 89614 2018-07-31 19:45:27Z jzhu47@wisc.edu $
!
! !USES:
   ...
   use forcing_shf
   use exit_mod
   use passive_tracers, only: wiso_ind_begin
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::   &! history field ids
      ...
      hist_id_18o,         &! id for d18o
      hist_id_hdo,         &! id for dD
      ...
      do nfield = 1,num_avail_hist_fields  ! check all available fields
         ...
         if (avail_hist_fields(nfield)%requested) then 
            ...
            do iblock=1,nblocks_clinic
               ...
               else if (nfield == hist_id_18o) then ! JZ, 09172017
                  WORK3D(:,:,:,iblock) = (TRACER(:,:,:,wiso_ind_begin,curtime,iblock)-c1)*c1000
               else if (nfield == hist_id_hdo) then ! JZ, 09172017
                  WORK3D(:,:,:,iblock) = (TRACER(:,:,:,wiso_ind_begin+1,curtime,iblock)-c1)*c1000
...
!-----------------------------------------------------------------------
   ...
   if (lhistory_on) then
      ...
      call define_hist_field(hist_id_18o, 'd18O', 3,  & !JZ
                             long_name = 'delta 18o',  &
                             units     = 'per mil',    &
                             grid_loc  = '3111')

      call define_hist_field(hist_id_hdo, 'dD', 3,  &
                             long_name = 'delta HDO',  &
                             units     = 'per mil',    &
                             grid_loc  = '3111')
