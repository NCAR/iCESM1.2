 module ms_balance_wiso
 
!BOP
! !MODULE: ms_balance_wiso

! !DESCRIPTION:
!  This module contains routines necessary for the balancing of isotopic
!  water fluxes in marginal-seas regions of the ccsm coupled ocean model
!
! !REVISION HISTORY:
...
!***********************************************************************
!BOP
! !IROUTINE:  init_ms_balance_wiso
! !INTERFACE:

   subroutine init_ms_balance_wiso
...
!-----------------------------------------------------------------------
   ...
   if (debug1) call print_regions_wiso (REGION_MASK_G)
...
!-----------------------------------------------------------------------
       ...
       i_loop: do i=1,nx_global
         ...
         if (mblat-lat_reach <= TLAT_G(i,j)    .and. &
             ...
             !--------------------------------------------------
             if (duplicate) then
                if(debug2) call pt_print_wiso('reject duplicate point',i,j) 
             ...
             !--------------------------------------------------
                ...
                if(debug2) &
                   call pt_print_wiso('reject marginal sea point',i,j) 
    
             !--------------------------------------------------
             ! reject land points
             !--------------------------------------------------
             else if (land) then             
                if(debug2) call pt_print_wiso('reject land point',i,j) 
             ...
             !--------------------------------------------------
                ...
                if( area >= region_info(n)%ms_bal%area)then
                    if (debug1) &
                    write(stdout,1002)'(init_ms_balance_wiso) ', &
...
!-----------------------------------------------------------------------
     if (npts <= 0) then
         call document ('init_ms_balance_wiso', 'marginal sea ' /&
                     &/  trim(region_info(n)%name) )
         call document ('init_ms_balance_wiso', &
    ...
    if (my_task == master_task) then
     ...
     do pt=1,npts
       if(region_number /= REGION_MASK_G(ipts(pt),jpts(pt)))then
          write(stdout,1000) '(init_ms_balance_wiso) ','marginal sea '/&
                          &/ trim(region_info(n)%name)
        write(stdout,1000) '(init_ms_balance_wiso)', &
                           ' WARNING: distribution points span two regions' 
          call POP_IOUnitsFlush(POP_stdout) ; call POP_IOUnitsFlush(stdout)
          if (pt > 1) then
            write(stdout,*)'(init_ms_balance_wiso)', ' region1 : ' ,&
                             REGION_MASK_G(ipts(pt-1),jpts(pt-1)) 
            write(stdout,*) '(init_ms_balance_wiso)', ' region2 : ', &
...
!-----------------------------------------------------------------------
   if (debug1) then
     if (my_task == master_task) then
      write(stdout,1002)'(init_ms_balance_wiso) ','actual area = ',area 
     ...
     call document('init_ms_balance_wiso', 'area       = ', area       )
     call document('init_ms_balance_wiso', 'npts       = ', npts       )
     call document('init_ms_balance_wiso', 'mask_index = ', mask_index )
...
!-----------------------------------------------------------------------
!EOC
   end subroutine init_ms_balance_wiso

 
!***********************************************************************
!BOP
! !IROUTINE:  ms_balancing_wiso
! !INTERFACE:

   subroutine ms_balancing_wiso (STF_wiso)
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real (r8), dimension(nx_block,ny_block,max_blocks_clinic),intent(inout):: &
     STF_wiso               ! contains STF(:,:,wiso_ind,:) from forcing_coupled
...
!-----------------------------------------------------------------------
     if (region_info(n)%marginal_sea) then
        ...
        where (REGION_MASK(:,:,iblock) == region_info(n)%number)
          TRANSPORT(:,:,iblock) =  STF_wiso(:,:,iblock) * DXT(:,:,iblock)*DYT(:,:,iblock)
          STF_wiso(:,:,iblock)  = c0
        end where
        ...
        STF_wiso(:,:,iblock) = STF_wiso(:,:,iblock)  &
...
!-----------------------------------------------------------------------
!EOC
   end subroutine ms_balancing_wiso
!
!
!
!***********************************************************************
!BOP
! !IROUTINE:  pt_print_wiso
! !INTERFACE:

   subroutine pt_print_wiso (string, i, j)
...
!-----------------------------------------------------------------------
!EOC
   end subroutine pt_print_wiso

 
!***********************************************************************
!BOP
! !IROUTINE:  print_regions_wiso
! !INTERFACE:

   subroutine print_regions_wiso (REGION_MASK_G)
...
!EOC
   ...
   end subroutine print_regions_wiso
 
 end module ms_balance_wiso
