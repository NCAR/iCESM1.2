...
!-----------------------------------------------------------------------
   ...
   if (index_x2o_Faxa_prec_16O > 0) then
      call named_field_register('PREC_16O_F', PREC_16O_F_nf_ind)
   endif
   if (index_x2o_Faxa_prec_18O > 0) then
      call named_field_register('PREC_18O_F', PREC_18O_F_nf_ind)
   endif
   if (index_x2o_Faxa_prec_HDO > 0) then
      call named_field_register('PREC_HDO_F', PREC_HDO_F_nf_ind)
   endif
   if (index_x2o_Foxx_evap_16O > 0) then
      call named_field_register('EVAP_16O_F', EVAP_16O_F_nf_ind)
   endif
   if (index_x2o_Foxx_evap_18O > 0) then
      call named_field_register('EVAP_18O_F', EVAP_18O_F_nf_ind)
   endif
   if (index_x2o_Foxx_evap_HDO > 0) then
      call named_field_register('EVAP_HDO_F', EVAP_HDO_F_nf_ind)
   endif

   if (index_x2o_Fioi_meltw_16O > 0) then
      call named_field_register('MELT_16O_F', MELT_16O_F_nf_ind)
   endif
   if (index_x2o_Fioi_meltw_18O > 0) then
      call named_field_register('MELT_18O_F', MELT_18O_F_nf_ind)
   endif
   if (index_x2o_Fioi_meltw_HDO > 0) then
      call named_field_register('MELT_HDO_F', MELT_HDO_F_nf_ind)
   endif

   if (index_x2o_Forr_roff_16O > 0) then
      call named_field_register('ROFF_16O_F', ROFF_16O_F_nf_ind)
   endif
   if (index_x2o_Forr_ioff_16O > 0) then
      call named_field_register('IOFF_16O_F', IOFF_16O_F_nf_ind)
   endif
   if (index_x2o_Forr_roff_18O > 0) then
      call named_field_register('ROFF_18O_F', ROFF_18O_F_nf_ind)
   endif
   if (index_x2o_Forr_ioff_18O > 0) then
      call named_field_register('IOFF_18O_F', IOFF_18O_F_nf_ind)
   endif
   if (index_x2o_Forr_roff_HDO > 0) then
      call named_field_register('ROFF_HDO_F', ROFF_HDO_F_nf_ind)
   endif
   if (index_x2o_Forr_ioff_HDO > 0) then
      call named_field_register('IOFF_HDO_F', IOFF_HDO_F_nf_ind)
   endif
   call flushm (stdout)
...
!-----------------------------------------------------------------------
   if (index_x2o_Faxa_prec_16O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Faxa_prec_16O,n)  !h2_16o precip 
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating PREC_16O_F halo')
         return
      endif

      call named_field_set(PREC_16O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Faxa_prec_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Faxa_prec_18O,n)  !h2_18o precip 
         enddo
         enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating PREC_18O_F halo')
         return
      endif

      call named_field_set(PREC_18O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Faxa_prec_HDO > 0) then
       ...
       do iblock = 1, nblocks_clinic
          ...
          do i=this_block%ib,this_block%ie
             n = n + 1
             WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Faxa_prec_HDO,n)  !h2_hdo precip 
          enddo
          enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
       ...
       if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating PREC_HDO_F halo')
         return
       endif

       call named_field_set(PREC_HDO_F_nf_ind, WORK1)

   end if
!-----------------------------------------------------------------------
!
! unpack isotopic evap fluxes: 
!  
!-----------------------------------------------------------------------
   if (index_x2o_Foxx_evap_16O > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Foxx_evap_16O,n)  !h2_16o evap 
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating EVAP_16O_F halo')
         return
      endif

      call named_field_set(EVAP_16O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Foxx_evap_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Foxx_evap_18O,n)  !h2_18o evap 
         enddo
         enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating EVAP_18O_F halo')
         return
      endif

      call named_field_set(EVAP_18O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Foxx_evap_HDO > 0) then
       ...
       do iblock = 1, nblocks_clinic
          ...
          do i=this_block%ib,this_block%ie
             n = n + 1
             WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Foxx_evap_HDO,n)  !h2_hdo evap 
          enddo
          enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
       ...
       if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating EVAP_HDO_F halo')
         return
       endif

       call named_field_set(EVAP_HDO_F_nf_ind, WORK1)

   end if
!-----------------------------------------------------------------------
!
! unpack isotopic melt fluxes: 
!  
!-----------------------------------------------------------------------
   if (index_x2o_Fioi_meltw_16O > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Fioi_meltw_16O,n)  !h2_16o evap 
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating MELT_16O_F halo')
         return
      endif

      call named_field_set(MELT_16O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Fioi_meltw_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Fioi_meltw_18O,n)  !h2_18o evap 
         enddo
         enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating MELT_18O_F halo')
         return
      endif

      call named_field_set(MELT_18O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Fioi_meltw_HDO > 0) then
       ...
       do iblock = 1, nblocks_clinic
          ...
          do i=this_block%ib,this_block%ie
             n = n + 1
             WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Fioi_meltw_HDO,n)  !h2_hdo evap 
          enddo
          enddo
       enddo
       call POP_HaloUpdate(WORK1,POP_haloClinic,          &
       ...
       if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating MELT_HDO_F halo')
         return
       endif

       call named_field_set(MELT_HDO_F_nf_ind, WORK1)
   end if

!-----------------------------------------------------------------------
!
! unpack isotopic roff fluxes: 
!  
!-----------------------------------------------------------------------
   if (index_x2o_Forr_roff_16O > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_roff_16O,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating ROFF_16O_F halo')
         return
      endif

      call named_field_set(ROFF_16O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Forr_ioff_16O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_ioff_16O,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating IOFF_16O_F halo')
         return
      endif

      call named_field_set(IOFF_16O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Forr_roff_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_roff_18O,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating ROFF_18O_F halo')
         return
      endif

      call named_field_set(ROFF_18O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Forr_ioff_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_ioff_18O,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating IOFF_18O_F halo')
         return
      endif

      call named_field_set(IOFF_18O_F_nf_ind, WORK1)
   ...
   if (index_x2o_Forr_roff_HDO > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_roff_HDO,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating ROFF_HDO_F halo')
         return
      endif

      call named_field_set(ROFF_HDO_F_nf_ind, WORK1)
   ...
   if (index_x2o_Forr_ioff_HDO > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o_o%rAttr(index_x2o_Forr_ioff_HDO,n)
         enddo
         enddo
      enddo
      call POP_HaloUpdate(WORK1,POP_haloClinic,          &
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating IOFF_HDO_F halo')
         return
      endif

      call named_field_set(IOFF_HDO_F_nf_ind, WORK1)
...
!-----------------------------------------------------------------------
!
!     pack Surface ocean water isotope tracer ratios roce, if requested (dimensionless)
!     units conversion occurs where roce is computed
!
!-----------------------------------------------------------------------
   if (index_o2x_So_roce_16O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            o2x_o%rAttr(index_o2x_So_roce_16O,n) = &
               SBUFF_SUM(i,j,iblock,index_o2x_So_roce_16O)/tlast_coupled
         enddo
         enddo
      enddo
   endif
   if (index_o2x_So_roce_18O > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            o2x_o%rAttr(index_o2x_So_roce_18O,n) = &
               SBUFF_SUM(i,j,iblock,index_o2x_So_roce_18O)/tlast_coupled
         enddo
         enddo
      enddo
   endif
   if (index_o2x_So_roce_HDO > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            o2x_o%rAttr(index_o2x_So_roce_HDO,n) = &
               SBUFF_SUM(i,j,iblock,index_o2x_So_roce_HDO)/tlast_coupled
         enddo
         enddo
      enddo
   endif
...
!-----------------------------------------------------------------------
   ...
   real (r8) ::   &
      ...
      delt_wlast          ! time interval for previous step water tracers?
...
!-----------------------------------------------------------------------
!
! As for fco2, do the same???
!  allow for roce fields to not be registered on first call
!     because init_forcing is called before init_passive_tracers
!  use weight from previous timestep because flux used here is that
!     computed during the previous timestep
!
!-----------------------------------------------------------------------
   ...
   if (index_o2x_So_roce_16O > 0) then
      if (ROCE_16O_nf_ind == 0) then
         call named_field_get_index('Roce_16O', ROCE_16O_nf_ind, &
                                    exit_on_err=.not. first)
      endif
   ...
   if (index_o2x_So_roce_18O > 0) then
      if (ROCE_18O_nf_ind == 0) then
         call named_field_get_index('Roce_18O', ROCE_18O_nf_ind, &
                                    exit_on_err=.not. first)
      endif
   ...
   if (index_o2x_So_roce_HDO > 0) then
      if (ROCE_HDO_nf_ind == 0) then
         call named_field_get_index('Roce_HDO', ROCE_HDO_nf_ind, &
                                    exit_on_err=.not. first)
      endif
...
!-----------------------------------------------------------------------
   ...
   if (index_o2x_So_roce_16O > 0 .and. ROCE_16O_nf_ind > 0) then
      call named_field_get(ROCE_16O_nf_ind, iblock, WORK(:,:,iblock))
      SBUFF_SUM(:,:,iblock,index_o2x_So_roce_16O) = &
         SBUFF_SUM(:,:,iblock,index_o2x_So_roce_16O) + delt_wlast*WORK(:,:,iblock)
   endif
   if (index_o2x_So_roce_18O > 0 .and. ROCE_18O_nf_ind > 0) then
      call named_field_get(ROCE_18O_nf_ind, iblock, WORK(:,:,iblock))
      SBUFF_SUM(:,:,iblock,index_o2x_So_roce_18O) = &
         SBUFF_SUM(:,:,iblock,index_o2x_So_roce_18O) + delt_wlast*WORK(:,:,iblock)
   endif
   if (index_o2x_So_roce_HDO > 0 .and. ROCE_HDO_nf_ind > 0) then
      call named_field_get(ROCE_HDO_nf_ind, iblock, WORK(:,:,iblock))
      SBUFF_SUM(:,:,iblock,index_o2x_So_roce_HDO) = &
         SBUFF_SUM(:,:,iblock,index_o2x_So_roce_HDO) + delt_wlast*WORK(:,:,iblock)
   endif
