...
! !INTERFACE:
  subroutine ncd_io_real_var2(varname, data, dim1name, &
                               lowerb2, upperb2, flag, ncid, nt, readvar, &
                               switchdim, cnvrtnan2fill, single_column_hack)
!
! !DESCRIPTION:
! Netcdf i/o of 2d initial integer field out to netCDF file
!
! TW - 19 August 2014 - NOTE: iCLM4 version of this routine has a hack for working
! around the known CLM4 bug of PTS_MODE not being able to do restart runs or
! read global finidat files. Initializing with finidat requires restart via
! BiogeophysRestMod.F90, which dies when reading 2D arrays. This version of
! ncd_io_real_var2 tricks CLM into thinking these arrays (hard-coded in here)
! are from a non-single-column run. The dimensions do not need to be switched
! because of how the data will be treated, so switchdim is undone.
...
! !ARGUMENTS:
    ...
    logical, optional, intent(in)  :: single_column_hack ! true=> use non-single-column READing - TW
!
! !REVISION HISTORY:
!
!
...
!EOP
    ...
    logical :: local_single_column_hack ! single-column initialization from finidat - TW
!-----------------------------------------------------------------------

! Needed to trick CLM into reading a restart file for single-column initialization - TW
local_single_column_hack=.false.
if(present(single_column_hack)) local_single_column_hack=single_column_hack
...
!DEBUG - TW
!          if (single_column) then
          if (single_column .and. .not.local_single_column_hack) then
...
!DEBUG - TW
                if(local_single_column_hack) then
                   data = transpose(temp)
                else
                   do j = lb2,ub2
                   do i = lb1,ub1
