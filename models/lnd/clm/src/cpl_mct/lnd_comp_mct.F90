...
!DEBUG
    ...
    !Determine atmospheric mode - JN:
!    call seq_infodata_GetData(infodata, atm_prognostic=atm_prognostic) !done earlier - TW

    ! Create land export state 

    call get_proc_bounds(begg, endg) 
    call lnd_export_mct( clm_l2a, l2x_l, begg, endg, atm_prognostic)
...
!    call seq_infodata_GetData(infodata, atm_prognostic=atm_prognostic) !called earlier - JN
    if (masterproc) then
...
!---------------------------------------------------------------------------
    ...
    call lnd_import_mct( x2l_l, clm_a2l, begg, endg, atm_prognostic )
...
!
! !INTERFACE:
  subroutine lnd_export_mct( clm_l2a, l2x_l, begg, endg, atm_prognostic)   
!
! !DESCRIPTION:
!
...
! !USES:
    ...
    use clm_cpl_indices
    use clmtype
    use HydrologyTracer    , only : pwtrc, hydro_tracer
    implicit none
! !ARGUMENTS:
    ...
    logical           , intent(in)    :: atm_prognostic !Coupled to CAM? - JN
!
! !LOCAL VARIABLES:
    integer  :: g,i                           ! indices
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
       ...
       if( hydro_tracer .and. atm_prognostic) then !Using Water Tracers and coupled to iCAM5?

          ! Water isotope tracers (comment out if CLM standalone)
          !    Snow water equivalent height
          l2x_l%rAttr(index_l2x_Sl_snowh_16O,i)    =  clm_l2a%wtr_h2osno(g,1)
          l2x_l%rAttr(index_l2x_Sl_snowh_HDO,i)    =  clm_l2a%wtr_h2osno(g,2)
          l2x_l%rAttr(index_l2x_Sl_snowh_18O,i)    =  clm_l2a%wtr_h2osno(g,3)
          !    Reference specific humidity at 2 m
          l2x_l%rAttr(index_l2x_Sl_qref_16O,i)     =  clm_l2a%wtr_q_ref2m(g,1)
          l2x_l%rAttr(index_l2x_Sl_qref_HDO,i)     =  clm_l2a%wtr_q_ref2m(g,2)
          l2x_l%rAttr(index_l2x_Sl_qref_18O,i)     =  clm_l2a%wtr_q_ref2m(g,3)
          !    Evaporation water flux
          l2x_l%rAttr(index_l2x_Fall_evap_16O,i)   = -clm_l2a%wtr_qflx_evap_tot(g,1)
          l2x_l%rAttr(index_l2x_Fall_evap_HDO,i)   = -clm_l2a%wtr_qflx_evap_tot(g,2)
          l2x_l%rAttr(index_l2x_Fall_evap_18O,i)   = -clm_l2a%wtr_qflx_evap_tot(g,3)
       end if
       ...
       if(hydro_tracer .and. atm_prognostic) then !Using Water Tracers and coupled to iCAM5?
          ! Water isotope tracers (comment out if CLM standalone)
          l2x_l%rattr(index_l2x_Flrl_rofliq_16O,i) = clm_l2a%wtr_rofliq(g,1)
          l2x_l%rattr(index_l2x_Flrl_rofliq_HDO,i) = clm_l2a%wtr_rofliq(g,2)
          l2x_l%rattr(index_l2x_Flrl_rofliq_18O,i) = clm_l2a%wtr_rofliq(g,3)
          l2x_l%rattr(index_l2x_Flrl_rofice_16O,i) = clm_l2a%wtr_rofice(g,1)
          l2x_l%rattr(index_l2x_Flrl_rofice_HDO,i) = clm_l2a%wtr_rofice(g,2)
          l2x_l%rattr(index_l2x_Flrl_rofice_18O,i) = clm_l2a%wtr_rofice(g,3)
       end if
...
!
! !INTERFACE:
  subroutine lnd_import_mct( x2l_l, a2l, begg, endg, atm_prognostic)
!
! !DESCRIPTION:
!
...
! !USES:
    ...
    use clm_cpl_indices
    use clmtype
    use HydrologyTracer , only : pwtrc, Rstnd, hydro_tracer, tracer_forc
...
! !LOCAL VARIABLES:
    ...
    real(r8) :: wtr_forc_rainc(pwtrc)! tracer rainxy Atm flux mm/s
    real(r8) :: wtr_forc_rainl(pwtrc)! tracer rainxy Atm flux mm/s
    real(r8) :: wtr_forc_snowc(pwtrc)! tracer snowfxy Atm flux  mm/s
    real(r8) :: wtr_forc_snowl(pwtrc)! tracer snowfxl Atm flux  mm/s
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
        ...
        if(hydro_tracer .and. tracer_forc) then    !Using water tracers standalone, with tracer forcing?

          ! Water isotope tracers, forcing by observations
          a2l%forc_wtr_q(g,2) = x2l_l%rAttr(index_x2l_Sa_shum_HDO,i)      ! forc_qxy Atm state kg/kg
          a2l%forc_wtr_q(g,3) = x2l_l%rAttr(index_x2l_Sa_shum_18O,i)      ! forc_qxy Atm state kg/kg
          ! Convert from deltas (forcing) to humidity (kg/kg)
          a2l%forc_wtr_q(g,2) = a2l%forc_q(g)*Rstnd(2)*((a2l%forc_wtr_q(g,2)/1000._r8)+1._r8)
          a2l%forc_wtr_q(g,3) = a2l%forc_q(g)*Rstnd(3)*((a2l%forc_wtr_q(g,3)/1000._r8)+1._r8)

          wtr_forc_rainc(2) = x2l_l%rAttr(index_x2l_Faxa_rainc_HDO,i)   ! mm/s
          wtr_forc_rainc(3) = x2l_l%rAttr(index_x2l_Faxa_rainc_18O,i)   ! mm/s
          wtr_forc_rainl(2) = x2l_l%rAttr(index_x2l_Faxa_rainl_HDO,i)   ! mm/s
          wtr_forc_rainl(3) = x2l_l%rAttr(index_x2l_Faxa_rainl_18O,i)   ! mm/s
          ! Convert from deltas (forcing) to flux (mm/s)
          wtr_forc_rainc(2) = forc_rainc*Rstnd(2)*((wtr_forc_rainc(2)/1000._r8)+1._r8)
          wtr_forc_rainc(3) = forc_rainc*Rstnd(3)*((wtr_forc_rainc(3)/1000._r8)+1._r8)
          wtr_forc_rainl(2) = forc_rainl*Rstnd(2)*((wtr_forc_rainl(2)/1000._r8)+1._r8)
          wtr_forc_rainl(3) = forc_rainl*Rstnd(3)*((wtr_forc_rainl(3)/1000._r8)+1._r8)

          wtr_forc_snowc(2)   = x2l_l%rAttr(index_x2l_Faxa_snowc_HDO,i)   ! mm/s
          wtr_forc_snowc(3)   = x2l_l%rAttr(index_x2l_Faxa_snowc_18O,i)   ! mm/s
          wtr_forc_snowl(2)   = x2l_l%rAttr(index_x2l_Faxa_snowl_HDO,i)   ! mm/s
          wtr_forc_snowl(3)   = x2l_l%rAttr(index_x2l_Faxa_snowl_18O,i)   ! mm/s
          ! Convert from deltas (forcing) to flux (mm/s)
          wtr_forc_snowc(2) = forc_snowc*Rstnd(2)*((wtr_forc_snowc(2)/1000._r8)+1._r8)
          wtr_forc_snowc(3) = forc_snowc*Rstnd(3)*((wtr_forc_snowc(3)/1000._r8)+1._r8)
          wtr_forc_snowl(2) = forc_snowl*Rstnd(2)*((wtr_forc_snowl(2)/1000._r8)+1._r8)
          wtr_forc_snowl(3) = forc_snowl*Rstnd(3)*((wtr_forc_snowl(3)/1000._r8)+1._r8)

          !Perfect forcing:
          a2l%forc_wtr_q(g,1) = x2l_l%rAttr(index_x2l_Sa_shum,i)
          wtr_forc_rainc(1)   = x2l_l%rAttr(index_x2l_Faxa_rainc,i)
          wtr_forc_rainl(1)   = x2l_l%rAttr(index_x2l_Faxa_rainl,i)
          wtr_forc_snowc(1)   = x2l_l%rAttr(index_x2l_Faxa_snowc,i)
          wtr_forc_snowl(1)   = x2l_l%rAttr(index_x2l_Faxa_snowl,i)

          do m = 1, pwtrc
             a2l%forc_wtr_rain(g,m)  = wtr_forc_rainc(m) + wtr_forc_rainl(m)
             a2l%forc_wtr_snow(g,m)  = wtr_forc_snowc(m) + wtr_forc_snowl(m)

             a2l%forc_wtr_Rclim(g,m) = Rstnd(m)
             a2l%forc_wtr_flood(g,m) = Rstnd(m)*a2l%forc_flood(g)
          end do
        ...
        if(hydro_tracer .and. atm_prognostic) then !Using Water Tracers and coupled to iCAM5?
          ...
          ! Water isotope tracers, forcing by iCAM
          a2l%forc_wtr_Rclim(g,1) = Rstnd(1)
          a2l%forc_wtr_Rclim(g,2) = Rstnd(2)
          a2l%forc_wtr_Rclim(g,3) = Rstnd(3)

!          a2l%forc_wtr_flood(g,1) = -x2l_l%rattr(index_x2l_Flrr_flood_16O,i)  
!          a2l%forc_wtr_flood(g,2) = -x2l_l%rattr(index_x2l_Flrr_flood_HDO,i)  
!          a2l%forc_wtr_flood(g,3) = -x2l_l%rattr(index_x2l_Flrr_flood_18O,i)  
          a2l%forc_wtr_flood(g,1) = -x2l_l%rattr(index_x2l_Flrr_flood,i)
          a2l%forc_wtr_flood(g,2) = -x2l_l%rattr(index_x2l_Flrr_flood,i)
          a2l%forc_wtr_flood(g,3) = -x2l_l%rattr(index_x2l_Flrr_flood,i)

          a2l%forc_wtr_q(g,1) = x2l_l%rAttr(index_x2l_Sa_shum_16O,i)      ! forc_qxy Atm state kg/kg
          a2l%forc_wtr_q(g,2) = x2l_l%rAttr(index_x2l_Sa_shum_HDO,i)      ! forc_qxy Atm state kg/kg
          a2l%forc_wtr_q(g,3) = x2l_l%rAttr(index_x2l_Sa_shum_18O,i)      ! forc_qxy Atm state kg/kg
          wtr_forc_rainc(1)   = x2l_l%rAttr(index_x2l_Faxa_rainc_16O,i)   ! mm/s
          wtr_forc_rainc(2)   = x2l_l%rAttr(index_x2l_Faxa_rainc_HDO,i)   ! mm/s
          wtr_forc_rainc(3)   = x2l_l%rAttr(index_x2l_Faxa_rainc_18O,i)   ! mm/s
          wtr_forc_rainl(1)   = x2l_l%rAttr(index_x2l_Faxa_rainl_16O,i)   ! mm/s
          wtr_forc_rainl(2)   = x2l_l%rAttr(index_x2l_Faxa_rainl_HDO,i)   ! mm/s
          wtr_forc_rainl(3)   = x2l_l%rAttr(index_x2l_Faxa_rainl_18O,i)   ! mm/s
          wtr_forc_snowc(1)   = x2l_l%rAttr(index_x2l_Faxa_snowc_16O,i)   ! mm/s
          wtr_forc_snowc(2)   = x2l_l%rAttr(index_x2l_Faxa_snowc_HDO,i)   ! mm/s
          wtr_forc_snowc(3)   = x2l_l%rAttr(index_x2l_Faxa_snowc_18O,i)   ! mm/s
          wtr_forc_snowl(1)   = x2l_l%rAttr(index_x2l_Faxa_snowl_16O,i)   ! mm/s
          wtr_forc_snowl(2)   = x2l_l%rAttr(index_x2l_Faxa_snowl_HDO,i)   ! mm/s
          wtr_forc_snowl(3)   = x2l_l%rAttr(index_x2l_Faxa_snowl_18O,i)   ! mm/s
        end if
    ...
    if( hydro_tracer .and. (.not. atm_prognostic) .and. (.not. tracer_forc)) then !Using Water Tracers and NOT coupled to iCAM5?
       do g = begg,endg                                                           ! and NOT tracer-forced?
         a2l%forc_wtr_flood(g,1) = (a2l%forc_flood(g))
         a2l%forc_wtr_q(g,1) = (a2l%forc_q(g))
         a2l%forc_wtr_rain(g,1) = (a2l%forc_rain(g))
         a2l%forc_wtr_snow(g,1) = (a2l%forc_snow(g))
         a2l%forc_wtr_Rclim(g,1) = 1._r8
         do m = 2, pwtrc
           a2l%forc_wtr_flood(g,m) = (a2l%forc_flood(g))*Rstnd(m)
           a2l%forc_wtr_q(g,m) = (a2l%forc_q(g))*Rstnd(m)
           a2l%forc_wtr_rain(g,m) = (a2l%forc_rain(g))*Rstnd(m)
           a2l%forc_wtr_snow(g,m) = (a2l%forc_snow(g))*Rstnd(m)
           a2l%forc_wtr_Rclim(g,m) = Rstnd(m)
         end do
       end do
    end if
