...
! variables needed for SNICAR
    ...
    real(r8), pointer :: wtr_h2osno(:,:)    ! tracer total snow water (col) [kg/m2]
    real(r8), pointer :: wtr_h2osoi_liq(:,:,:)    ! tracer liquid water (col,lyr) [kg/m2]
    real(r8), pointer :: wtr_h2osoi_ice(:,:,:)    ! tracer ice content (col,lyr) [kg/m2]
...
! !USES:
    ...
    use HydrologyIsotope, only : wiso_alpsol
!
! !ARGUMENTS:
    implicit none
    integer , intent(in) :: lbc, ubc                             ! column bounds
...
!
    ...
    real(r8), pointer :: wtr_qflx_snow_melt(:,:)    ! net snow melt
...
!
    ...
    real(r8), pointer :: wtr_qflx_snofrz_lyr(:,:,:) !tracer snow freezing rate (positive definite) (col,lyr) [kg m-2 s-1]
    real(r8), pointer :: wtr_qflx_snofrz_col(:,:)  !tracer column-integrated snow freezing rate (positive definite) [kg m-2 s-1]
    real(r8), pointer :: wtr_qflx_glcice(:,:)   !tracer flux of new glacier ice (mm H2O/s) [+ = ice grows]
    real(r8), pointer :: wtr_qflx_glcice_melt(:,:)  !tracer ice melt (positive definite) (mm H2O/s)
!
! local pointers to original implicit in arrays
!
    ...
    real(r8), pointer :: wtr_h2osoi_liq(:,:,:)  !tracer liquid water (kg/m2) (new)
    real(r8), pointer :: wtr_h2osoi_ice(:,:,:)  !tracer ice lens (kg/m2) (new)
!
...
!
    ...
    real(r8) :: wtr_wmass0(lbc:ubc,-nlevsno+1:nlevgrnd,pwtrc)!tracer initial mass of ice and liquid (kg/m2)
    real(r8) :: wtr_wice0 (lbc:ubc,-nlevsno+1:nlevgrnd,pwtrc)!tracer initial mass of ice (kg/m2)
    real(r8) :: wtr_wliq0 (lbc:ubc,-nlevsno+1:nlevgrnd,pwtrc)!tracer initial mass of liquid (kg/m2)
    ...
    real(r8) :: RMelt(pwtrc), RFrez(pwtrc)         !tracer ratio of freeze and melt water
    real(r8) :: RSnow(pwtrc)                       !tracer ratio of snow 
    real(r8) :: alpeq                              !isotopic fractionation factor for melting
    logical  :: iceflag                            !how to deal with tracer soil ice
...
!-----------------------------------------------------------------------
    ...
    wtr_h2osoi_liq   => cws%wtr_h2osoi_liq
    wtr_h2osoi_ice   => cws%wtr_h2osoi_ice
    wtr_qflx_snofrz_lyr => cwf%wtr_qflx_snofrz_lyr
    wtr_qflx_snofrz_col => cwf%wtr_qflx_snofrz_col
    wtr_qflx_glcice  => cwf%wtr_qflx_glcice
    wtr_qflx_glcice_melt  => cwf%wtr_qflx_glcice_melt
...
!cdir nodep
    do fc = 1,num_nolakec
       ...
       do m = 1, pwtrc
         wtr_qflx_snomelt(c,m) = 0._r8
         wtr_qflx_snow_melt(c,m) = 0._r8
       end do
       xmf(c) = 0._r8
       qflx_snofrz_lyr(c,-nlevsno+1:0) = 0._r8
       qflx_snofrz_col(c) = 0._r8
       do m = 1, pwtrc
         wtr_qflx_snofrz_lyr(c,-nlevsno+1:0,m) = 0._r8
         wtr_qflx_snofrz_col(c,m) = 0._r8
       end do
       if (ltype(l)==istice_mec) then
          ...
          do m = 1, pwtrc
            wtr_qflx_glcice_melt(c,m) = 0._r8
          end do
       end if
    end do
...
!cdir nodep
       do fc = 1,num_nolakec
          ...
          if (j >= snl(c)+1) then
             ...
             do m = 1, pwtrc
               wtr_wice0(c,j,m) = wtr_h2osoi_ice(c,j,m)
               wtr_wliq0(c,j,m) = wtr_h2osoi_liq(c,j,m)
               wtr_wmass0(c,j,m) = wtr_h2osoi_ice(c,j,m) + wtr_h2osoi_liq(c,j,m)
             end do
             ...
             if (imelt(c,j) > 0 .and. abs(hm(c,j)) > 0._r8) then
                ...
                if (j == 1) then
                   if (snl(c)+1 == 1 .AND. h2osno(c) > 0._r8 .AND. xm(c,j) > 0._r8) then
                      ...
                      do m = 1, pwtrc
                         if( h2osno(c) == 0._r8 ) then                      ! if no more bulk snow, 
                            wtr_qflx_snomelt(c,m) = wtr_h2osno(c,m)/dtime   ! get rid of all tracer
                            wtr_h2osno(c,m) = 0._r8
                            wtr_qflx_snow_melt(c,m) = wtr_qflx_snomelt(c,m)
                         else
!!                            alpeq = wiso_alpsol(m)
                            alpeq = 1._r8               ! no fractionation during snow melt - TW
                            wtr_qflx_snomelt(c,m) = (RSnow(m)*qflx_snomelt(c))/alpeq
                            wtr_h2osno(c,m) = max(0._r8,(wtr_h2osno(c,m) - (dtime*wtr_qflx_snomelt(c,m))))
                            wtr_qflx_snow_melt(c,m) = wtr_qflx_snomelt(c,m)
                         end if
                      end do
                   endif
                ...
                if (xm(c,j) > 0._r8) then
                   do m = 1, pwtrc
                      RMelt(m) = get_wratio(wtr_wice0(c,j,m),wtr_wice0(c,j,ixbase),h2otiny)
                   end do
                   h2osoi_ice(c,j) = max(0._r8, wice0(c,j)-xm(c,j))
                   do m = 1, pwtrc
!!                      alpeq = wiso_alpsol(m)
                      alpeq = 1._r8               ! no fractionation during snow melt - TW
                      wtr_h2osoi_ice(c,j,m) = max(0._r8, wtr_wice0(c,j,m)-RMelt(m)*xm(c,j)/alpeq)
                   end do
                   heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice(c,j))/dtime
                else if (xm(c,j) < 0._r8) then
                   if (j <= 0) then
                      do m = 1, pwtrc
                         !Is this right? ice normally doesn't freeze into ice.
                         !I have replaced it with wliq0 to prevent instability
                         !if RFrez ever became >1, plus it seems more physically
                         !realistic.  However, we can always change it to just
                         !wmass0 if using wliq0 becomes a problem. - JN 
                         !RFrez(m) = get_wratio(wtr_wice0(c,j,m),wtr_wice0(c,j,ixbase),h2otiny)
                         RFrez(m) = get_wratio(wtr_wliq0(c,j,m),wtr_wliq0(c,j,ixbase),h2otiny)
                      end do
                      h2osoi_ice(c,j) = min(wmass0(c,j), wice0(c,j)-xm(c,j))  ! snow
                      do m = 1, pwtrc
!!                         alpeq = wiso_alpsol(m)
                         alpeq = 1._r8               ! no fractionation during snow melt - TW
                         wtr_h2osoi_ice(c,j,m) = min(wtr_wmass0(c,j,m), wtr_wice0(c,j,m)-RFrez(m)*xm(c,j)*alpeq)
                      end do
                   else
                      if (wmass0(c,j) < supercool(c,j)) then
                         h2osoi_ice(c,j) = 0._r8
                         do m = 1, pwtrc
                            wtr_h2osoi_ice(c,j,m) = 0._r8
                         end do
                      else
                         do m = 1, pwtrc
                            RFrez(m) = get_wratio(wtr_wmass0(c,j,m),wtr_wmass0(c,j,ixbase),h2otiny)
                         end do
                         h2osoi_ice(c,j) = min(wmass0(c,j) - supercool(c,j),wice0(c,j)-xm(c,j))
                         do m = 1, pwtrc
!!                            alpeq = wiso_alpsol(m)
                            alpeq = 1._r8               ! no fractionation during snow melt - TW
                            wtr_h2osoi_ice(c,j,m) = min(wtr_wmass0(c,j,m) - RFrez(m)*supercool(c,j)*alpeq, &
                                                        wtr_wice0(c,j,m)-RFrez(m)*xm(c,j)*alpeq)
                         end do
                      endif
                   endif
                   heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice(c,j))/dtime
                ...
                do m = 1, pwtrc
                   wtr_h2osoi_liq(c,j,m) = max(0._r8,wtr_wmass0(c,j,m)-wtr_h2osoi_ice(c,j,m))
                  !Add check in case all liquid has been frozen (the ratios sometime result
                  !in no bulk water mass but non-zero isotopic mass, which is unphysical
                  !and causes the model to crash.  This code should prevent it. - JN
                  !NOTE:  Also added same check for all ice melting to liquid, just in case. -JN
                  if((m > ixbase) .and. (wtr_h2osoi_liq(c,j,ixbase) == 0._r8)) then
                    wtr_h2osoi_ice(c,j,m) = wtr_wmass0(c,j,m) !all mass is ice
                    wtr_h2osoi_liq(c,j,m) = 0._r8             !no liquid mass.
                  end if
                  if((m > ixbase) .and. (wtr_h2osoi_ice(c,j,ixbase) == 0._r8)) then
                    wtr_h2osoi_liq(c,j,m) = wtr_wmass0(c,j,m) !all mass is liquid
                    wtr_h2osoi_ice(c,j,m) = 0._r8             !no ice mass.
                  end if
                end do
                ...
                if (imelt(c,j) == 1 .AND. j < 1) then
                   ...
                   do m = 1, pwtrc
                     wtr_qflx_snomelt(c,m) = wtr_qflx_snomelt(c,m) + &
                                        max(0._r8,(wtr_wice0(c,j,m)-wtr_h2osoi_ice(c,j,m)))/dtime
                   end do
                endif
                ...
                if (imelt(c,j) == 2 .AND. j < 1) then
                   ...
                   do m = 1, pwtrc
                      wtr_qflx_snofrz_lyr(c,j,m) = max(0._r8,(wtr_h2osoi_ice(c,j,m)-wtr_wice0(c,j,m)))/dtime
                   end do
                endif
       ...
       !  as computed in Hydrology2Mod.F90.
          ...
          if (ltype(l)==istice_mec) then
             ...
             if (j>=1 .and. h2osoi_liq(c,j) > 0._r8) then   ! ice layer with meltwater
                ...
                do m = 1, pwtrc
                   wtr_qflx_glcice_melt(c,m) = wtr_qflx_glcice_melt(c,m) + wtr_h2osoi_liq(c,j,m)/dtime
                   wtr_qflx_glcice(c,m) = wtr_qflx_glcice(c,m) - wtr_h2osoi_liq(c,j,m)/dtime
                end do
                ...
                do m = 1, pwtrc
                   wtr_h2osoi_ice(c,j,m) = wtr_h2osoi_ice(c,j,m) + wtr_h2osoi_liq(c,j,m)
...
!cdir nodep
    ...
    do j = -nlevsno+1,0
       do fc = 1,num_nolakec
          ...
          do m = 1, pwtrc
            wtr_qflx_snofrz_col(c,m) = wtr_qflx_snofrz_col(c,m) + wtr_qflx_snofrz_lyr(c,j,m)
          end do
       end do
    end do
