...
!   new2 version contains:
!   1. "in-line" sort of tracer tridiagonal set up and solve
!   2. call to HydrologyTracerSoilWater added to Drainage
!
! !PUBLIC TYPES:
  implicit none
  save
...
! !USES:
    ...
    use clm_varcon      , only : denice, denh2o, wimp, pondmx_urban, &
                                 ...
                                 icol_road_imperv, icol_road_perv, watmin
                             
    use clm_varpar      , only : nlevsoi, maxpatch_pft
    use clm_time_manager, only : get_step_size
    use HydrologyTracer , only : pwtrc, get_wratio, h2otiny, ixbase, TracerCheckEqual, &
                                 Rstnd, cdbg, pdbg, mdbg, qflxtiny, jdbg
!
! !ARGUMENTS:
    implicit none
...
!rtm_flood
    ...
    real(r8), pointer :: wtr_h2osoi_ice(:,:,:)   !tracer ice water (kg/m2)
    ...
    real(r8), pointer :: wtr_qflx_evap_tot(:,:) ! tracer total evaporation rate (mm H2O/s) [+]
...
!
    ...
    real(r8) :: wtr_xs(lbc:ubc,pwtrc)      ! tracer excess soil water above urban ponding limit
    ...
    real(r8) :: RSurf(pwtrc)               !local ratio of surface water isotopes for runoff
...
!cdir nodep
    do fc = 1, num_hydrologyc
       ...
       do m = 1, pwtrc
         RSurf(m) = get_wratio(wtr_qflx_top_soil(c,m),wtr_qflx_top_soil(c,ixbase),qflxtiny)
         ! This is an OK check because would choke otherwise if no original tracer water
         if( qflx_surf(c).ne.0._r8 .and. abs(qflx_top_soil(c))<=qflxtiny ) RSurf(m)=Rstnd(m)
         wtr_qflx_surf(c,m) = RSurf(m)*qflx_surf(c)
       end do
    ...
    do fc = 1, num_urbanc
       ...
       if (ctype(c) == icol_roof .or. ctype(c) == icol_road_imperv) then
          ...
          if (snl(c) < 0) then
             ...
             do m = 1, pwtrc
               wtr_qflx_surf(c,m) = max(0._r8,wtr_qflx_top_soil(c,m))
             end do
          else
             xs(c) = max(0._r8, &
                         ...
                         pondmx_urban/dtime)
             do m = 1, pwtrc
                RSurf(m) = get_wratio(wtr_h2osoi_liq(c,1,m),wtr_h2osoi_liq(c,1,ixbase),h2otiny)
                ! This is an OK check because would choke otherwise if no original tracer water
                if( wtr_h2osoi_liq(c,1,ixbase)<=h2otiny ) RSurf(m)=Rstnd(m)
                wtr_xs(c,m) = RSurf(m)*xs(c)
!                wtr_xs(c,m) = max(0._r8, &
!                                 wtr_h2osoi_liq(c,1,m)/dtime + wtr_qflx_top_soil(c,m) - wtr_qflx_evap_grnd(c,m) - &
!                                 pondmx_urban/dtime)
             end do
             ...
             do m = 1, pwtrc
                if (wtr_xs(c,m) > 0._r8) then
                   wtr_h2osoi_liq(c,1,m) = RSurf(m)*pondmx_urban
                else
                   wtr_h2osoi_liq(c,1,m) = max(0._r8,wtr_h2osoi_liq(c,1,m)+ &
                                              (wtr_qflx_top_soil(c,m)-wtr_qflx_evap_grnd(c,m))*dtime)
                end if
                if(h2osoi_liq(c,1)==0._r8) then !if no bulk water, get rid of tracer water...
                   !need to account for this fix in the water balance; add to wtr_qflx_evap_tot(COLUMN version)
                   wtr_qflx_evap_tot(c,m) = wtr_qflx_evap_tot(c,m) + wtr_h2osoi_liq(c,1,m)/dtime
                   wtr_qflx_evap_grnd(c,m) = wtr_qflx_evap_grnd(c,m) + wtr_h2osoi_liq(c,1,m)/dtime
                   wtr_h2osoi_liq(c,1,m) = 0._r8
                end if
             end do

             qflx_surf(c) = xs(c)
             do m = 1, pwtrc
               wtr_qflx_surf(c,m) = wtr_xs(c,m)
             end do
          end if
...
!rtm_flood:  send flood water flux to runoff for all urban columns
       ...
       do m = 1, pwtrc
          wtr_qflx_surf(c,m) = wtr_qflx_surf(c,m)  + wtr_qflx_floodc(c,m)
       end do
!rtm_flood
    end do
...
!cdir nodep
    do fc = 1, num_hydrologyc
       ...
       do m = 1, pwtrc
          wtr_qflx_top_soil(c,m) = wtr_qflx_top_soil(c,m) + wtr_qflx_floodc(c,m)
       end do
    end do
...
! !USES:
    ...
    use clm_varcon  , only : icol_roof, icol_road_imperv, icol_sunwall, icol_shadewall, &
                             icol_road_perv
    use clmtype
    use HydrologyTracer, only : pwtrc, cdbg, mdbg
!
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: lbc, ubc                     ! column bounds
...
!-----------------------------------------------------------------------
    ...
    do fc = 1, num_hydrologyc
       ...
       if (snl(c) >= 0) then
          ...
          do m = 1, pwtrc
            wtr_qflx_infl(c,m) = wtr_qflx_top_soil(c,m) - wtr_qflx_surf(c,m) - wtr_qflx_evap_grnd(c,m)
          end do
       else
          qflx_infl(c) = qflx_top_soil(c) - qflx_surf(c)
          do m = 1, pwtrc
            wtr_qflx_infl(c,m) = wtr_qflx_top_soil(c,m) - wtr_qflx_surf(c,m)
          end do
       end if
    end do
...
! !INTERFACE:
  subroutine SoilWater(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
                       num_urbanc, filter_urbanc, &
                       vol_liq, dwat, wtr_dwat, hk, dhkdw)
!
! !DESCRIPTION:
! Soil hydrology
...
! !USES:
    ...
    use clm_time_manager  , only : get_step_size
    use HydrologyTracer , only : pwtrc, get_wratio, h2otiny, ixbase, Rstnd, cdbg, jdbg, mdbg, &
                                 HydrologyTracerSoilWater, qflxtiny
!
! !ARGUMENTS:
    implicit none
    integer , intent(in)  :: lbc, ubc                     ! column bounds
    ...
    real(r8), intent(out) :: wtr_dwat(lbc:ubc,1:nlevsoi,pwtrc) ! tracer change of soil water [m3/m3]
...
!
    ...
    real(r8), pointer :: wtr_qflx_infl(:,:)         ! tracer infiltration (mm H2O /s)
    real(r8), pointer :: wtr_qflx_tran_veg_pft(:,:) ! tracer vegetation transpiration (mm H2O/s) (+ = to atm)
    real(r8), pointer :: wtr_qflx_tran_veg_col(:,:) ! tracer vegetation transpiration (mm H2O/s) (+ = to atm)
...
!
    ...
    real(r8) :: wtr_rmx(lbc:ubc,1:nlevsoi+1,pwtrc)  ! "r" forcing term of tridiagonal matrix
    ...
    real(r8) :: wtr_qin(lbc:ubc,1:nlevsoi+1,pwtrc)    ! flux of water into soil layer [mm h2o/s]
    real(r8) :: wtr_qout(lbc:ubc,1:nlevsoi+1,pwtrc)   ! flux of water out of soil layer [mm h2o/s]
    ...
    real(r8) :: wtr_dwat2(lbc:ubc,1:nlevsoi+1,pwtrc)
    ...
    real(r8) :: Rtmp(pwtrc)                 ! temporary variable to store water tracer ratios
    real(r8) :: h2osoi_old(lbc:ubc,1:nlevsoi) ! initial soil profile
    logical, parameter :: ltsw=.true.       ! do HydrologyTracerSoilWater?
    logical :: ldbg=.false.
!    if(lbc<cdbg .and. ubc>cdbg) ldbg=.true.
!-----------------------------------------------------------------------
    ...
    do j = 1, nlevsoi
       do fc = 1, num_hydrologyc
          ...
          ! Save a copy of the h2osoi_liq for updating tracers once water
          ! transport and drainage plus corrections are done.
          h2osoi_old(c,j) = h2osoi_liq(c,j)
       end do
    end do
    ...
    do fc = 1, num_hydrologyc
       ...
       qin(c,j)    = qflx_infl(c)
       do m = 1, pwtrc
          wtr_qin(c,j,m) = wtr_qflx_infl(c,m)
       end do
       ...
       do m = 1, pwtrc
          Rtmp(m) = get_wratio(wtr_h2osoi_liq(c,j,m),wtr_h2osoi_liq(c,j,ixbase),h2otiny)
          wtr_qout(c,j,m) = qout(c,j)*Rtmp(m)
       end do
       ...
       do m = 1, pwtrc
          wtr_rmx(c,j,m) =  wtr_qin(c,j,m) - wtr_qout(c,j,m) - wtr_qflx_tran_veg_col(c,m)*rootr_col(c,j)
       end do
       amx(c,j) =  0._r8
    ...
    do j = 2, nlevsoi - 1
       do fc = 1, num_hydrologyc
          ...
          do m = 1, pwtrc
             wtr_qin(c,j,m) = wtr_qout(c,j-1,m)
          end do
    ...
    do fc = 1, num_hydrologyc
       ...
       if(j > jwt(c)) then !water table is in soil column
         ...
         qin(c,j+1)    = qout(c,j)
         do m = 1, pwtrc
            wtr_qin(c,j+1,m) = wtr_qout(c,j,m)
         end do
         ...
         do m = 1, pwtrc
            wtr_qout(c,j+1,m)   =  0._r8  ! zero-flow bottom boundary condition
         end do
         dqodw1(c,j+1) =  0._r8  ! zero-flow bottom boundary condition
         rmx(c,j+1) =  qin(c,j+1) - qout(c,j+1)
         do m = 1, pwtrc
            wtr_rmx(c,j+1,m) =  wtr_qin(c,j+1,m) - wtr_qout(c,j+1,m)
         end do
...
if(.not. ltsw) then
    do m = 1, pwtrc
       jtop(:) = 1
       call Tridiagonal(lbc, ubc, 1, nlevsoi+1, jtop, num_hydrologyc, filter_hydrologyc, &
                        amx, bmx, cmx, wtr_rmx(:,:,m), wtr_dwat2(:,:,m) )
    end do
end if
    ...
    do fc = 1,num_hydrologyc
       ...
       do j = 1, nlevsoi
          dwat(c,j)=dwat2(c,j)
          do m = 1, pwtrc
             wtr_dwat(c,j,m)=wtr_dwat2(c,j,m)
          end do
       end do
    end do
...
if(.not. ltsw) then
          do m = 1, pwtrc
              wtr_h2osoi_liq(c,j,m) = wtr_h2osoi_liq(c,j,m) + wtr_dwat2(c,j,m)*dzmm(c,j)
          end do
end if
       end do
...
! !INTERFACE:
  subroutine Drainage(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
                      ...
                      icefrac, h2osoi_old, wtr_h2osoi_old)
!!                      icefrac, h2osoi_old)
!
! !DESCRIPTION:
...
! !USES:
    ...
    use HydrologyTracer, only : pwtrc, HydrologyTracerSoilWater, Rstnd, &
                                get_wratio, h2otiny, qflxtiny, ixbase, cdbg, jdbg, mdbg
!
! !ARGUMENTS:
    implicit none
    integer , intent(in) :: lbc, ubc                     ! column bounds
    ...
    real(r8), intent(in) :: h2osoi_old(lbc:ubc,1:nlevsoi)  ! initial soil profile
    real(r8), intent(inout) :: wtr_h2osoi_old(lbc:ubc,1:nlevsoi,pwtrc)  !tracer initial soil profile
!
! !CALLED FROM:
!
! !REVISION HISTORY:
...
!
    ...
    real(r8), pointer :: RGNIP(:,:)  !climatology tracer ratio for water table
!
! local pointers to original implicit inout arguments
!
    real(r8), pointer :: h2osoi_ice(:,:)   !ice lens (kg/m2)
...
!
    ...
    real(r8), pointer :: wtr_qflx_qrgwl(:,:)     !tracer qflx_surf at glaciers, wetlands, lakes (mm H2O /s)
    real(r8), pointer :: wtr_qflx_rsub_sat(:,:)  !tracer soil saturation excess [mm h2o/s]
!! Tony's fix for tracer water exceeding bulk water = watmin
    real(r8), pointer :: wtr_qflx_evap_grnd(:,:) ! tracer ground surface evaporation rate (mm H2O/s) [+]
    real(r8), pointer :: wtr_qflx_evap_tot(:,:) ! tracer total evaporation rate (mm H2O/s) [+]
!
!EOP
...
!
    ...
    real(r8) :: temp                     !temporary guy
    real(r8) :: Rxs(pwtrc)               !excess water tracer ratio
    real(r8) :: RSoil_save(lbc:ubc,1:nlevsoi+1,pwtrc) !soil water tracer ratio at beginning of Drainage
    real(r8) :: wratio(pwtrc)
    logical :: ldbg=.false.
!    if(lbc<=cdbg .and. ubc>=cdbg) ldbg=.true.
!-----------------------------------------------------------------------
...
!   cgridcell      => col%gridcell
    ...
    wtr_qflx_rsub_sat => cwf%wtr_qflx_rsub_sat
    wtr_h2osoi_liq    => cws%wtr_h2osoi_liq
    wtr_h2osoi_ice    => cws%wtr_h2osoi_ice
    RGNIP    => cws%RGNIP
    wtr_qflx_evap_grnd => pwf_a%wtr_qflx_evap_grnd
...
!! Get ratio of old soil water
do j = 1, nlevsoi+1
   do fc = 1, num_hydrologyc
      c = filter_hydrologyc(fc)
      do m = 1, pwtrc
         if( j < nlevsoi+1 ) then 
            RSoil_save(c,j,m) = get_wratio( wtr_h2osoi_old(c,j,m),wtr_h2osoi_old(c,j,ixbase),h2otiny )
         elseif( j == nlevsoi+1) then 
            RSoil_save(c,j,m) = sum( RSoil_save(c,1:nlevsoi,m) )/nlevsoi
         end if
      end do
   end do
end do
    ...
    do fc = 1, num_hydrologyc
       ...
       do m = 1, pwtrc
         wtr_qflx_drain(c,m) = 0._r8  ! subsurface runoff
         wtr_qflx_rsub_sat(c,m) = 0._r8
       end do
    end do
...
! drainage
    ...
    ! Now all the updates are done, do the tracer soil water transport, 
    ! (h2osoi_old is from BEFORE the soil water transport, i.e., SoilWater)
    ! (trying to do just an h2osoi_liq corretion?)
    call HydrologyTracerSoilWater(lbc,ubc, num_hydrologyc,filter_hydrologyc,h2osoi_old)

    ! Fix for negative tracer water and check to make sure if bulk water is at
    ! watmin, that the tracers aren't exceeding this. Put the excess water in
    ! the next layer down
    do j = 1, nlevsoi
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          do m = 1, pwtrc
             ! Fix for negative tracer water:
             if (wtr_h2osoi_liq(c,j,m) < 0._r8) then 

                ! Option 1: Preserve the ratio before it went negative, taking
                ! whatever tracer water is needed from the next layer down
                !   => what to do in the lowest layer? set=0
#if 0
                if(j<nlevsoi) then
                  wtr_h2osoi_liq(c,j+1,m) = wtr_h2osoi_liq(c,j+1,m) - ((RSoil_save(c,j,m)*h2osoi_liq(c,j))-wtr_h2osoi_liq(c,j,))
                  wtr_h2osoi_liq(c,j  ,m) = RSoil_save(c,j,m)*h2osoi_liq(c,j)
                elseif(j==nlevsoi) then
                  wtr_h2osoi_liq(c,j,m) = 0._r8
                end if
#endif
                ! Option 2: Use the next layer down to add enough tracer water
                ! to bring the negative layer up to w=0, and that's it
#if 0
                if(j<nlevsoi) then
                  wtr_h2osoi_liq(c,j+1,m) = wtr_h2osoi_liq(c,j+1,m) + wtr_h2osoi_liq(c,j,m) !wtr_h2osoi_liq < 0
                  wtr_h2osoi_liq(c,j  ,m) = 0._r8
                elseif(j==nlevsoi) then
                  wtr_h2osoi_liq(c,j,m) = 0._r8
                end if
#endif
                ! Option 3: try applying old ratio to all soil layers. something went
                ! terribly wrong in one layer, and they're all interdependent, so
                ! fix them all. but use ratio from lower.
#if 0
                do i = 1, nlevsoi-1
                   wtr_h2osoi_liq(c,i,m) = RSoil_save(c,i+1,m)*h2osoi_liq(c,i)
                end do
                wtr_h2osoi_liq(c,nlevsoi,m) = h2osoi_liq(c,nlevsoi)*(nlevsoi*RSoil_save(c,nlevsoi+1,m) - sum(RSoil_save(c,1:nlevsoi-1,m)))
#endif
                ! Option 4: mimic what is done if bulk water is less than watmin:
                ! Layers 1 through nlevsoi-1: fill layer j using layer j+1
                ! Layer nlevsoi: fill using above layers, then aquifer if needed
                !   + account for this by reducing drainage flux if necessary
!#if 0
                if (j<nlevsoi) then
                   wtr_h2osoi_liq(c,j+1,m) = wtr_h2osoi_liq(c,j+1,m) + wtr_h2osoi_liq(c,j,m)
                   wtr_h2osoi_liq(c,j  ,m) = 0._r8
                elseif (j==nlevsoi) then
                   xs(c) = -wtr_h2osoi_liq(c,nlevsoi,m)
                   tracersearchforwater: do i = nlevsoi-1, 1, -1
                      available_h2osoi_liq = max(wtr_h2osoi_liq(c,i,m)-xs(c) , 0._r8)
                      if (available_h2osoi_liq .ge. xs(c)) then 
                         wtr_h2osoi_liq(c,nlevsoi,m) = wtr_h2osoi_liq(c,nlevsoi,m) + xs(c)
                         wtr_h2osoi_liq(c,i,m) = wtr_h2osoi_liq(c,i,m) - xs(c)
                         xs(c) = 0._r8
                         exit tracersearchforwater
                      else 
                         wtr_h2osoi_liq(c,nlevsoi,m) = wtr_h2osoi_liq(c,nlevsoi,m) + available_h2osoi_liq
                         wtr_h2osoi_liq(c,i,m) = wtr_h2osoi_liq(c,i,m) - available_h2osoi_liq
                         xs(c) = xs(c) - available_h2osoi_liq
                      end if
                   end do tracersearchforwater
                   wtr_h2osoi_liq(c,nlevsoi,m) = wtr_h2osoi_liq(c,nlevsoi,m) + xs(c)
                   wtr_wt(c,m) = wtr_wt(c,m) - xs(c)
                   wtr_qflx_drain(c,m) = wtr_qflx_drain(c,m) - (xs(c)/dtime)
               end if
!#endif
             end if
          end do
    ...
    ! Fix for if bulk water dries out (watmin) but tracers are still wet
    ! Put the extra tracer water into ground evap
    do j = 1, nlevsoi
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          do m = 1, pwtrc
             if (h2osoi_liq(c,j)==watmin .and. wtr_h2osoi_liq(c,j,m)>watmin) then
                wtr_qflx_evap_grnd(c,m) = wtr_qflx_evap_grnd(c,m) + (wtr_h2osoi_liq(c,j,m)-watmin)/dtime
                wtr_qflx_evap_tot(c,m) = wtr_qflx_evap_tot(c,m) + (wtr_h2osoi_liq(c,j,m)-watmin)/dtime
                wtr_h2osoi_liq(c,j,m) = watmin
             end if
          end do
       end do
    end do
    ...
    do fc = 1, num_hydrologyc
       ...
       do m = 1, pwtrc
         wtr_qflx_qrgwl(c,m) = wtr_qflx_snwcp_liq(c,m)
       end do
       ...
       if (snl(c)+1 >= 1) then
          ...
          do m = 1, pwtrc
            wtr_h2osoi_liq(c,1,m) = wtr_h2osoi_liq(c,1,m) + (wtr_qflx_dew_grnd(c,m) * dtime)
            wtr_h2osoi_ice(c,1,m) = wtr_h2osoi_ice(c,1,m) + (wtr_qflx_dew_snow(c,m) * dtime)
          end do
          ...
          do m = 1, pwtrc
             !Perform sublimation check again, for water tracers specifically: -JN
             if(wtr_qflx_sub_snow(c,m)*dtime > wtr_h2osoi_ice(c,1,m)) then 
                wtr_qflx_sub_snow(c,m) = wtr_h2osoi_ice(c,1,m)/dtime
                wtr_h2osoi_ice(c,1,m) = 0._r8
             else 
                wtr_h2osoi_ice(c,1,m) = wtr_h2osoi_ice(c,1,m) - (wtr_qflx_sub_snow(c,m) * dtime)
             end if
          end do
       end if
    end do
    ...
    do fc = 1, num_urbanc
       ...
       if (ctype(c) == icol_roof .or. ctype(c) == icol_road_imperv) then
         if (snl(c)+1 >= 1) then
            ...
            if (qflx_sub_snow(c)*dtime > h2osoi_ice(c,1)) then
               ...
               h2osoi_ice(c,1) = h2osoi_ice(c,1) - (qflx_sub_snow(c) * dtime)
               !Perform sublimation check again, for water tracers specifically: -JN
               !NOTE:  This is done to prevent the creation of negative isotopic
               !soil ice, which will cause the model to crash. - JN
            end if
            do m = 1, pwtrc
               if(wtr_qflx_sub_snow(c,m)*dtime > wtr_h2osoi_ice(c,1,m)) then
                  wtr_qflx_sub_snow(c,m) = wtr_h2osoi_ice(c,1,m)/dtime
            ...
            if (h2osoi_ice(c,1) == 0._r8) then
              do m = 1, pwtrc
                 wtr_qflx_evap_tot(c,m) = wtr_qflx_evap_tot(c,m) + wtr_h2osoi_ice(c,1,m)/dtime
                 wtr_qflx_sub_snow(c,m) = wtr_qflx_sub_snow(c,m) + wtr_h2osoi_ice(c,1,m)/dtime
                 wtr_h2osoi_ice(c,1,m) = 0._r8
              end do
            end if
         end if
       ...
       ! NOW calculate the tracer drainage, once all the corrections to tracer soil water are done
       do m = 1, pwtrc
