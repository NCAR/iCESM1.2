...
! !USES:
    ...
    use HydrologyTracer   , only : pwtrc, ixbase, h2otiny, qflxtiny, get_wratio, ispec, &
                                   TracerCheckEqual, cdbg, pdbg, mdbg, Rstnd, voltiny
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(r8), pointer :: wtr_qflx_evap_can(:,:)   ! tracer evaporation from leaves and stems (mm H2O/s) (+ = to atm)
...
!
    ...
    real(r8) :: wtr_egsmax(lbc:ubc,pwtrc) ! tracer max. evaporation which soil can provide at one time step
    real(r8) :: wtr_egirat(lbc:ubc,pwtrc) ! tracer ratio of topsoil_evap_tot : egsmax
    ...
    real(r8) :: wtr_topsoil_evap_tot(lbc:ubc,pwtrc)! tracer column-level total evaporation from top soil layer
    ...
    real(r8) :: RFlux(pwtrc)         ! constituent ratio of tracer flux
    real(r8) :: wtr_surf_tot, base_surf_tot ! temporary storage for calculation of surface evaporation ratio

    logical  :: lfail                ! fail flag for TracerCheckEqual
    logical  :: lflxsgn=.false.         ! flag to force the tracer evap and tran to be in the same direction
    logical  :: ldbg=.false.
!    if(cdbg>=lbc .and. cdbg<=ubc) ldbg=.true.
!-----------------------------------------------------------------------
    ...
    do fc = 1,num_nolakec
       ...
       !DEBUG -TW
       do m = 1, pwtrc
          wtr_egsmax(c,m) = (wtr_h2osoi_ice(c,j,m)+wtr_h2osoi_liq(c,j,m)) / dtime
       end do
       ...
       !DEBUG -TW
       do m = 1, pwtrc
          if(wtr_egsmax(c,m) < 0._r8) wtr_egsmax(c,m) = 0._r8
       end do
    end do
...
!cdir nodep
    ...
    do fp = 1,num_nolakep
       ...
       do m = 1, pwtrc
          RFlux(m)=Rstnd(m) !initialize
          !Instead, decide what kind of update to do for tracer evap based on what the bulk evap does AFTER the correction
          if( qflx_evap_soi(p) < 0._r8) then !dew/frost
             RFlux(m) = pws%RCanopyVapor(p,m)
!             wtr_qflx_evap_soi(p,m) = RFlux(m)*qflx_evap_soi(p)
             wtr_qflx_evap_soi(p,m) = wtr_qflx_evap_soi(p,m) + RFlux(m)*tinc(c)*cgrndl(p)
          else ! bulk is evap or sublimation
             !RFlux(m) = get_wratio(wtr_qflx_evap_soi(p,m),wtr_qflx_evap_soi(p,ixbase),qflxtiny)
             ...
             RFlux(m) = get_wratio(wtr_surf_tot,base_surf_tot,voltiny)
             if( RFlux(m)==0._r8 .and. abs(tinc(c)*cgrndl(p))>0._r8 ) then
             !   wtr_surf_tot  = wtr_h2osoi_ice(c,snl(c)+1,m)+wtr_h2osoi_liq(c,snl(c)+1,m)
             !   base_surf_tot = wtr_h2osoi_ice(c,snl(c)+1,ixbase)+wtr_h2osoi_liq(c,snl(c)+1,ixbase)
             !   RFlux(m) = get_wratio(wtr_surf_tot,base_surf_tot,voltiny)
             !   RFlux(m) = get_wratio(wtr_qflx_evap_soi(p,m),wtr_qflx_evap_soi(p,ixbase),qflxtiny)
                RFlux(m)=Rstnd(m)   ! Proven to be necessary to prevent large tracer errors when run at medium-high resolution (<=2-degree)
             end if                 ! TW 6 July 2015
             wtr_qflx_evap_soi(p,m) = wtr_qflx_evap_soi(p,m) + RFlux(m)*tinc(c)*cgrndl(p)
          end if
       end do
...
!cdir nodep
    do fc = 1,num_nolakec
       ...
       sumwt(c) = 0._r8
       do m = 1, pwtrc
          wtr_topsoil_evap_tot(c,m) = 0._r8
       end do
    end do
...
!cdir nodep
       do fc = 1,num_nolakec
          ...
          if ( pi <= npfts(c) ) then
             ...
             if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
                ...
                !DEBUG -TW
                do m = 1, pwtrc
                   wtr_topsoil_evap_tot(c,m) = wtr_topsoil_evap_tot(c,m) + wtr_qflx_evap_soi(p,m)*wtcol(p)
                end do
             end if
          end if
       end do
    ...
    do fc = 1,num_nolakec
       ...
       !DEBUG -TW
       do m = 1, pwtrc
          if(wtr_topsoil_evap_tot(c,m) > wtr_egsmax(c,m)) then
             wtr_egirat(c,m) = (wtr_egsmax(c,m)/wtr_topsoil_evap_tot(c,m))
          else
             wtr_egirat(c,m) = 1.0_r8
          end if
       end do
    end do
...
!DEBUG -TW
!          do m = 1, pwtrc
!            wtr_qflx_evap_soi(p,m) = wtr_qflx_evap_soi(p,m) * egirat(c)
!          end do
          eflx_sh_grnd(p) = eflx_sh_grnd(p) + (save_qflx_evap_soi - qflx_evap_soi(p))*htvp(c)
       end if
       !DEBUG -TW
       do m = 1, pwtrc
          if (wtr_egirat(c,m) < 1.0_r8) wtr_qflx_evap_soi(p,m) = wtr_qflx_evap_soi(p,m)*wtr_egirat(c,m)
       end do
       ...
       do m = 1, pwtrc
         wtr_qflx_evap_tot(p,m) = wtr_qflx_evap_veg(p,m) + wtr_qflx_evap_soi(p,m)
       end do
       ...
       ! Same, for tracers. Need different structure because tracer evap_soi may
       ! be doing different things than bulk evap_soi.
       do m = 1, pwtrc
          if(wtr_qflx_evap_soi(p,m)>=0._r8) then
             if((wtr_h2osoi_liq(c,j,m)+wtr_h2osoi_ice(c,j,m)) > 0._r8) then !prevent NaNs -JN
               wtr_qflx_evap_grnd(p,m) = max(wtr_qflx_evap_soi(p,m)*(wtr_h2osoi_liq(c,j,m)/ &
                                         (wtr_h2osoi_liq(c,j,m)+wtr_h2osoi_ice(c,j,m))), 0._r8)
             else
               wtr_qflx_evap_grnd(p,m) = 0._r8 !set to zero if NaN
             end if
             wtr_qflx_sub_snow(p,m)  = wtr_qflx_evap_soi(p,m) - wtr_qflx_evap_grnd(p,m)
          else
             if (t_grnd(c) < tfrz) then
               wtr_qflx_dew_snow(p,m) = abs(wtr_qflx_evap_soi(p,m))
             else
               wtr_qflx_dew_grnd(p,m) = abs(wtr_qflx_evap_soi(p,m))
             end if
          end if
       end do
       ...
       if (snl(c) < 0 .and. do_capsnow(c)) then
          ...
          do m = 1, pwtrc
             wtr_qflx_snwcp_liq(p,m) = wtr_qflx_snwcp_liq(p,m) + wtr_qflx_dew_grnd(p,m)
             wtr_qflx_snwcp_ice(p,m) = wtr_qflx_snwcp_ice(p,m) + wtr_qflx_dew_snow(p,m)
          end do
       end if
       ...
       do m = 1, pwtrc
         wtr_qflx_evap_can(p,m)  = wtr_qflx_evap_veg(p,m) - wtr_qflx_tran_veg(p,m)
       end do
