...
!   new2 includes:
!   1. h2osoi_old (between Infiltration and SoilWater) for 
!       inclusion of HydrologyTracerSoilWater (within Drainage)
!   2. nlevgrnd replaces nlevsoi everywhere (possible wtr error)
!
! !USES:
...
! !USES:
    ...
    use HydrologyTracer , only : pwtrc, TracerCheckColumn, TracerCheckColumnDelta, &
                                 cdbg, jdbg, mdbg, pdbg
...
!
    ...
    real(r8), pointer :: wtr_qflx_glcice(:,:)     ! tracer flux of new glacier ice (mm H2O /s)
...
!
    ...
    real(r8) :: wtr_dwat(lbc:ubc,1:nlevgrnd,pwtrc)   ! tracer change in soil water
    real(r8) :: h2osoi_old(lbc:ubc,1:nlevsoi) ! initial soil profile
    real(r8) :: wtr_h2osoi_old(lbc:ubc,1:nlevsoi,pwtrc) !tracer initial soil profile
...
!rtm_flood
    ...
    wtr_h2osoi_liqice_10cm => cws%wtr_h2osoi_liqice_10cm
    ...
    wtr_qflx_runoff_r     => cwf%wtr_qflx_runoff_r
...
!cdir nodep
       do fc = 1,num_hydrologyc
          ...
          do m = 1,pwtrc
             wtr_h2osoi_old(c,j,m) = wtr_h2osoi_liq(c,j,m)
          end do
       end do
    end do
...
!!                   vol_liq, dwat, hk, dhkdw)
    ...
    call Drainage(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
                  num_urbanc, filter_urbanc, &
                  vol_liq, hk, icefrac, h2osoi_old, wtr_h2osoi_old)
!!                  vol_liq, hk, icefrac)
...
!       c = filter_snowc(fc)
       ...
       snowliq(c) = 0._r8
       do m = 1, pwtrc
          wtr_snowice(c,m) = 0._r8
          wtr_snowliq(c,m) = 0._r8
       end do
    end do
!dir$ concurrent
!cdir nodep
...
!cdir nodep
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             do m = 1, pwtrc
                 wtr_snowice(c,m) = wtr_snowice(c,m) + wtr_h2osoi_ice(c,j,m)
                 wtr_snowliq(c,m) = wtr_snowliq(c,m) + wtr_h2osoi_liq(c,j,m)
             end do
          end if
       end do
    end do
    ...
    do fc = 1, num_nolakec
       ...
       if (ityplun(l) /= isturb) then
          ...
          do m = 1, pwtrc
            wtr_h2osoi_liqice_10cm(c,m) = 0._r8
          end do
       end if
    end do
    do j = 1, nlevsoi
          ...
          if (ityplun(l) /= isturb) then
            if (zi(c,j) <= 0.1_r8) then
              ...
              h2osoi_liqice_10cm(c) = h2osoi_liqice_10cm(c) + (h2osoi_liq(c,j)+h2osoi_ice(c,j))* &
                                       fracl
              do m = 1, pwtrc
                wtr_h2osoi_liqice_10cm(c,m) = wtr_h2osoi_liqice_10cm(c,m) + (wtr_h2osoi_liq(c,j,m)+wtr_h2osoi_ice(c,j,m))* &
                                         fracl
              end do
            else
    ...
    do fc = 1, num_nolakec
       ...
       if (ctype(c) == icol_roof .or. ctype(c) == icol_sunwall &
         ...
         do m = 1, pwtrc
            wtr_endwb(c,m) = wtr_h2ocan(c,m) + wtr_h2osno(c,m)
         end do
       else
         endwb(c) = h2ocan(c) + h2osno(c) + wa(c)
         do m = 1, pwtrc
            wtr_endwb(c,m) = wtr_h2ocan(c,m) + wtr_h2osno(c,m) + wtr_wa(c,m)
         end do
       end if
    end do
...
!cdir nodep
       do fc = 1, num_nolakec
          ...
          do m = 1, pwtrc
            wtr_endwb(c,m) = wtr_endwb(c,m) + wtr_h2osoi_ice(c,j,m) + wtr_h2osoi_liq(c,j,m)
...
!rtm_flood
          do m = 1, pwtrc
            ...
            wtr_qflx_qrgwl(c,m) = forc_wtr_rain(g,m) + forc_wtr_snow(g,m) + wtr_qflx_floodg(g,m) - wtr_qflx_evap_tot(c,m) - & 
                                  wtr_qflx_snwcp_ice(c,m) - (wtr_endwb(c,m)-wtr_begwb(c,m))/dtime
          end do
          ...
          if (glc_dyntopo .and. ityplun(l)==istice_mec) then
             ...
             do m = 1, pwtrc
                wtr_qflx_qrgwl(c,m) = wtr_qflx_qrgwl(c,m) - wtr_qflx_glcice(c,m)   ! meltwater from melted ice
             end do
          endif
          fcov(c)       = spval
          fsat(c)       = spval
       ...
       if (ityplun(l)==istice_mec) then
          ...
          do m = 1, pwtrc
             wtr_qflx_glcice_frz(c,m) = wtr_qflx_snwcp_ice(c,m)
             wtr_qflx_glcice(c,m) = wtr_qflx_glcice(c,m) + wtr_qflx_glcice_frz(c,m)
          end do
       ...
       if (ityplun(l)==istsoil .and. col%wtgcell(c) > 0.0_r8 ) then
          ...
          do m = 1, pwtrc
             wtr_qflx_runoff(c,m) = wtr_qflx_runoff(c,m) - wtr_qflx_irrig(c,m)
          end do
       end if
       if (ityplun(l)==isturb) then
         qflx_runoff_u(c) = qflx_runoff(c)
         do m = 1, pwtrc
            wtr_qflx_runoff_u(c,m) = wtr_qflx_runoff(c,m)
         end do
       else if (ityplun(l)==istsoil .or. ityplun(l)==istcrop) then
         qflx_runoff_r(c) = qflx_runoff(c)
         do m = 1, pwtrc
            wtr_qflx_runoff_r(c,m) = wtr_qflx_runoff(c,m)
         end do
       end if
