...
! !USES:
    ...
    use HydrologyTracer   , only : pwtrc, h2otiny, ixbase, get_wratio, &
                                   cdbg, jdbg, mdbg
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(r8), pointer :: wtr_qflx_snow_melt(:,:)   !tracer net snow melt
...
!
    ...
    integer  :: m                                 ! tracer loop index
    real(r8) :: RFlux(lbc:ubc,pwtrc)              ! water tracer ratio of gravity flux
    ...
    real(r8) :: Rsnow(pwtrc)                      ! temporary storage of snow tracer ratio
    integer :: idbg
...
!cdir nodep
    do fc = 1,num_snowc
       ...
       if (do_capsnow(c)) then
          ...
          do m = 1, pwtrc
            wgdif = wtr_h2osoi_ice(c,snl(c)+1,m) - wtr_qflx_sub_snow(c,m)*dtime
            wtr_h2osoi_ice(c,snl(c)+1,m) = wgdif
            if (wgdif < 0._r8) then
               wtr_h2osoi_ice(c,snl(c)+1,m) = 0._r8
               wtr_h2osoi_liq(c,snl(c)+1,m) = wtr_h2osoi_liq(c,snl(c)+1,m) + wgdif
            end if
            if (h2osoi_ice(c,snl(c)+1) == 0._r8) then
               wtr_h2osoi_liq(c,snl(c)+1,m) = wtr_h2osoi_liq(c,snl(c)+1,m) + wtr_h2osoi_ice(c,snl(c)+1,m)
               wtr_h2osoi_ice(c,snl(c)+1,m) = 0._r8
            endif
            wtr_h2osoi_liq(c,snl(c)+1,m) = wtr_h2osoi_liq(c,snl(c)+1,m) - wtr_qflx_evap_grnd(c,m) * dtime
          end do
       else
          ...
          do m = 1, pwtrc
            wgdif = wtr_h2osoi_ice(c,snl(c)+1,m) + (wtr_qflx_dew_snow(c,m) - wtr_qflx_sub_snow(c,m)) * dtime
            wtr_h2osoi_ice(c,snl(c)+1,m) = wgdif
            if (wgdif < 0._r8) then
               wtr_h2osoi_ice(c,snl(c)+1,m) = 0._r8
               wtr_h2osoi_liq(c,snl(c)+1,m) = wtr_h2osoi_liq(c,snl(c)+1,m) + wgdif
            ...
            wtr_h2osoi_liq(c,snl(c)+1,m) = wtr_h2osoi_liq(c,snl(c)+1,m) +  &
                 (wtr_qflx_rain_grnd(c,m) + wtr_qflx_dew_grnd(c,m) - wtr_qflx_evap_grnd(c,m)) * dtime
          end do
       end if
       h2osoi_liq(c,snl(c)+1) = max(0._r8, h2osoi_liq(c,snl(c)+1))
       do m = 1, pwtrc
         wtr_h2osoi_liq(c,snl(c)+1,m) = max(0._r8, wtr_h2osoi_liq(c,snl(c)+1,m))
       end do
    end do
...
!cdir nodep
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             do m = 1, pwtrc
               wtr_h2osoi_liq(c,j,m) = wtr_h2osoi_liq(c,j,m) + RFlux(c,m)*qin(c)
             end do
             ...
             do m = 1, pwtrc
               RFlux(c,m) = get_wratio(wtr_h2osoi_liq(c,j,m),wtr_h2osoi_liq(c,j,ixbase),h2otiny)
             end do
             h2osoi_liq(c,j) = h2osoi_liq(c,j) - qout(c)
             do m = 1, pwtrc
               wtr_h2osoi_liq(c,j,m) = wtr_h2osoi_liq(c,j,m) - RFlux(c,m)*qout(c)
             end do
             qin(c) = qout(c)
    ...
    do fc = 1, num_snowc
       ...
       do m = 1, pwtrc
         wtr_qflx_top_soil(c,m) = RFlux(c,m)*qflx_top_soil(c)
         wtr_qflx_snow_melt(c,m) = wtr_qflx_snow_melt(c,m) + wtr_qflx_top_soil(c,m)
       end do
    end do
    ...
    do fc = 1, num_nosnowc
       ...
       do m = 1, pwtrc
         wtr_qflx_top_soil(c,m) = wtr_qflx_rain_grnd(c,m) + wtr_qflx_snomelt(c,m)
         wtr_qflx_snow_melt(c,m) = wtr_qflx_snomelt(c,m)
       end do
    end do
...
! !USES:
    ...
    use clm_time_manager, only : get_step_size
    use HydrologyTracer, only : pwtrc, cdbg, jdbg, mdbg
!
! !ARGUMENTS:
    implicit none
    integer, intent(in)    :: lbc, ubc                    ! column bounds
...
!
    ...
    real(r8), pointer :: wtr_qflx_sl_top_soil(:,:) ! tracer liquid water + ice from layer above soil to top soil layer or sent to qflx_qrgwl (mm H2O/s)
!
!
! !OTHER LOCAL VARIABLES:
!EOP
!
    ...
    real(r8):: wtr_zwice(lbc:ubc,pwtrc) ! tracer total ice mass in snow
    real(r8):: wtr_zwliq(lbc:ubc,pwtrc) ! tracer total liquid water in snow
...
!-----------------------------------------------------------------------
    ...
    wtr_h2osoi_ice => cws%wtr_h2osoi_ice
    wtr_h2osoi_liq => cws%wtr_h2osoi_liq
    wtr_qflx_sl_top_soil => cwf%wtr_qflx_sl_top_soil
    ...
    do fc = 1, num_snowc
       ...
       do m = 1, pwtrc
          wtr_qflx_sl_top_soil(c,m) = 0._r8
       end do
    end do
          ...
          if (h2osoi_ice(c,j) <= .1_r8) then
             if (ltype(l) == istsoil .or. ltype(l)==isturb .or. ltype(l) == istcrop) then
                ...
                do m = 1, pwtrc
                  ...
                  wtr_h2osoi_ice(c,j+1,m) = wtr_h2osoi_ice(c,j+1,m) + wtr_h2osoi_ice(c,j,m)
                end do

                if (j == 0) then
                  qflx_sl_top_soil(c) = (h2osoi_liq(c,j) + h2osoi_ice(c,j))/dtime
                  do m = 1, pwtrc
                    wtr_qflx_sl_top_soil(c,m) = (wtr_h2osoi_liq(c,j,m) + wtr_h2osoi_ice(c,j,m))/dtime
                  end do
                end if
             ...
             if (j > snl(c)+1 .and. snl(c) < -1) then
                do i = j, snl(c)+2, -1
                   ...
                   if (ltype(l) /= istsoil .and. ltype(l) /= istcrop .and. ltype(l) /= isturb .and. i == 0) then
                     ...
                     do m = 1, pwtrc
                       wtr_qflx_sl_top_soil(c,m) = (wtr_h2osoi_liq(c,i,m) + wtr_h2osoi_ice(c,i,m))/dtime
                     end do
                   end if
                   ...
                   do m = 1, pwtrc
                     wtr_h2osoi_liq(c,i,m) = wtr_h2osoi_liq(c,i-1,m)
                     wtr_h2osoi_ice(c,i,m) = wtr_h2osoi_ice(c,i-1,m)
                   end do
...
!cdir nodep
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             do m = 1, pwtrc
               wtr_h2osno(c,m) = wtr_h2osno(c,m) + wtr_h2osoi_ice(c,j,m) + wtr_h2osoi_liq(c,j,m)
               wtr_zwice(c,m)  = wtr_zwice(c,m) + wtr_h2osoi_ice(c,j,m)
               wtr_zwliq(c,m)  = wtr_zwliq(c,m) + wtr_h2osoi_liq(c,j,m)
             end do
          end if
       end do
    end do
    ...
    do fc = 1, num_snowc
       ...
       if (snowdp(c) < 0.01_r8 .and. snowdp(c) > 0._r8) then
          ...
          if (ltype(l) == istsoil .or. ltype(l) == isturb .or. ltype(l) == istcrop) then
             ...
             do m = 1, pwtrc
               wtr_h2osoi_liq(c,1,m) = wtr_h2osoi_liq(c,1,m) + wtr_zwliq(c,m)
             end do
          end if
       end if
    end do
       ...
       if (snl(c) < -1) then
          ...
          do i = msn_old(c)+1,0
             if (dz(c,i) < dzmin(mssi(c))) then
                ...
                do m = 1, pwtrc
                  wtr_h2osoi_liq(c,j,m) = wtr_h2osoi_liq(c,j,m) + wtr_h2osoi_liq(c,l,m)
                  wtr_h2osoi_ice(c,j,m) = wtr_h2osoi_ice(c,j,m) + wtr_h2osoi_ice(c,l,m)
                end do
                ...
                if (j-1 > snl(c)+1) then
                   do k = j-1, snl(c)+2, -1
                      ...
                      do m = 1, pwtrc
                        wtr_h2osoi_ice(c,k,m) = wtr_h2osoi_ice(c,k-1,m)
                        wtr_h2osoi_liq(c,k,m) = wtr_h2osoi_liq(c,k-1,m)
                      end do
...
! !USES:
    ...
    use HydrologyTracer, only : pwtrc, cdbg, jdbg
!
! !ARGUMENTS:
    implicit none
    integer, intent(in)    :: lbc, ubc                    ! column bounds
...
!
    ...
    real(r8) :: wtr_swice(lbc:ubc,nlevsno,pwtrc) ! tracer partial volume of ice [m3/m3]
    real(r8) :: wtr_swliq(lbc:ubc,nlevsno,pwtrc) ! tracer partial volume of liquid water [m3/m3]
    real(r8) :: tsno(lbc:ubc ,nlevsno) ! Nodel temperature [K]
    real(r8) :: zwice                  ! temporary
    real(r8) :: zwliq                  ! temporary
    real(r8) :: wtr_zwice(pwtrc)       ! temporary
    real(r8) :: wtr_zwliq(pwtrc)       ! temporary
...
!cdir nodep
       do fc = 1, num_snowc
          ...
          if (j <= abs(snl(c))) then
             ...
             do m = 1, pwtrc
               wtr_swice(c,j,m) = wtr_h2osoi_ice(c,j+snl(c),m)
               wtr_swliq(c,j,m) = wtr_h2osoi_liq(c,j+snl(c),m)
             end do
    ...
    do fc = 1, num_snowc
       ...
       if (msno == 1) then
          ...
          if (dzsno(c,1) > 0.03_r8) then
             ...
             tsno(c,2)  = tsno(c,1)
             do m = 1, pwtrc
               wtr_swice(c,1,m) = wtr_swice(c,1,m)/2._r8
               wtr_swliq(c,1,m) = wtr_swliq(c,1,m)/2._r8
               wtr_swice(c,2,m) = wtr_swice(c,1,m)
               wtr_swliq(c,2,m) = wtr_swliq(c,1,m)
             end do
       ...
       if (msno > 1) then
          if (dzsno(c,1) > 0.02_r8) then
             ...
             do m = 1, pwtrc
               wtr_zwice(m) = propor*wtr_swice(c,1,m)
               wtr_zwliq(m) = propor*wtr_swliq(c,1,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swice(c,1,m) = propor*wtr_swice(c,1,m)
               wtr_swliq(c,1,m) = propor*wtr_swliq(c,1,m)
             end do
             ...
             do m = 1, pwtrc
                wtr_swliq(c,2,m) = wtr_swliq(c,2,m) + wtr_zwliq(m)
                wtr_swice(c,2,m) = wtr_swice(c,2,m) + wtr_zwice(m)
             end do
             ...
             if (msno <= 2 .and. dzsno(c,2) > 0.07_r8) then
                ...
                do m = 1, pwtrc
                  wtr_swice(c,2,m) = wtr_swice(c,2,m)/2._r8
                  wtr_swliq(c,2,m) = wtr_swliq(c,2,m)/2._r8
                end do
                dzsno(c,3) = dzsno(c,2)
                swice(c,3) = swice(c,2)
                swliq(c,3) = swliq(c,2)
                do m = 1, pwtrc
                  wtr_swice(c,3,m) = wtr_swice(c,2,m)
                  wtr_swliq(c,3,m) = wtr_swliq(c,2,m)
                end do
       ...
       if (msno > 2) then
          if (dzsno(c,2) > 0.05_r8) then
             ...
             do m = 1, pwtrc
               wtr_zwice(m) = propor*wtr_swice(c,2,m)
               wtr_zwliq(m) = propor*wtr_swliq(c,2,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swice(c,2,m) = propor*wtr_swice(c,2,m)
               wtr_swliq(c,2,m) = propor*wtr_swliq(c,2,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swliq(c,3,m) = wtr_swliq(c,3,m) + wtr_zwliq(m)
               wtr_swice(c,3,m) = wtr_swice(c,3,m) + wtr_zwice(m)
             end do
             ...
             if (msno <= 3 .and. dzsno(c,3) > 0.18_r8) then
                ...
                do m = 1, pwtrc
                  wtr_swice(c,3,m) = wtr_swice(c,3,m)/2.
                  wtr_swliq(c,3,m) = wtr_swliq(c,3,m)/2.
                end do
                dzsno(c,4) = dzsno(c,3)
                swice(c,4) = swice(c,3)
                swliq(c,4) = swliq(c,3)
                do m = 1, pwtrc
                  wtr_swice(c,4,m) = wtr_swice(c,3,m)
                  wtr_swliq(c,4,m) = wtr_swliq(c,3,m)
                end do
       ...
       if (msno > 3) then
          if (dzsno(c,3) > 0.11_r8) then
             ...
             do m = 1, pwtrc
               wtr_zwice(m) = propor*wtr_swice(c,3,m)
               wtr_zwliq(m) = propor*wtr_swliq(c,3,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swice(c,3,m) = propor*wtr_swice(c,3,m)
               wtr_swliq(c,3,m) = propor*wtr_swliq(c,3,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swliq(c,4,m) = wtr_swliq(c,4,m) + wtr_zwliq(m)
               wtr_swice(c,4,m) = wtr_swice(c,4,m) + wtr_zwice(m)
             end do
             ...
             if (msno <= 4 .and. dzsno(c,4) > 0.41_r8) then
                ...
                do m = 1, pwtrc
                  wtr_swice(c,4,m) = wtr_swice(c,4,m)/2.
                  wtr_swliq(c,4,m) = wtr_swliq(c,4,m)/2.
                end do
                dzsno(c,5) = dzsno(c,4)
                swice(c,5) = swice(c,4)
                swliq(c,5) = swliq(c,4)
                do m = 1, pwtrc
                  wtr_swice(c,5,m) = wtr_swice(c,4,m)
                  wtr_swliq(c,5,m) = wtr_swliq(c,4,m)
                end do
       ...
       if (msno > 4) then
          if (dzsno(c,4) > 0.23_r8) then
             ...
             do m = 1, pwtrc
               wtr_zwice(m) = propor*wtr_swice(c,4,m)
               wtr_zwliq(m) = propor*wtr_swliq(c,4,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swice(c,4,m) = propor*wtr_swice(c,4,m)
               wtr_swliq(c,4,m) = propor*wtr_swliq(c,4,m)
             end do
             ...
             do m = 1, pwtrc
               wtr_swliq(c,5,m) = wtr_swliq(c,5,m) + wtr_zwliq(m)
               wtr_swice(c,5,m) = wtr_swice(c,5,m) + wtr_zwice(m)
             end do
          end if
       end if
...
!cdir nodep
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             do m = 1, pwtrc
               wtr_h2osoi_ice(c,j,m) = wtr_swice(c,j-snl(c),m)
               wtr_h2osoi_liq(c,j,m) = wtr_swliq(c,j-snl(c),m)
             end do
