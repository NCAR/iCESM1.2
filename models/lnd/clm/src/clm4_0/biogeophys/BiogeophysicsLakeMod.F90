...
! !USES:
    ...
    use HydrologyTracer    , only : pwtrc, get_wratio, ixbase, h2otiny, voltiny, ispec, &
                                    cdbg, pdbg, mdbg
    use HydrologyIsotope   , only : hydro_isotope, wiso_alpliq, wiso_alpice, wiso_alpkin_m78, &
                                    difrm, isphdo, isph218o, isph217o, tracer_constant_ratio
...
!
    ...
    real(r8), pointer :: RGNIP(:,:)         ! tracer ratio climatology
...
!
    ...
    real(r8), pointer :: wtr_qflx_snwcp_liq(:,:)  ! excess rainfall due to snow capping (mm H2O /s) [+]`
...
!
    ...
    real(r8) :: RSnow                   ! tracer ratio in snow
    real(r8) :: RSurf(pwtrc)            ! tracer ratio for surface water pool
    real(r8) :: RFrost(pwtrc)           ! tracer ratio for frost onto snow
    real(r8) :: alpliq(pwtrc)           ! equilibrium fractionation for liquid
    real(r8) :: alpice(pwtrc)           ! equilibrium fractionation for ice
    real(r8) :: wtr_raw(lbp:ubp,pwtrc)  ! aerodynamic resistance for water tracer
    real(r8) :: kk(lbp:ubp,pwtrc)       ! for MJ79 kinetic fractionation factor
    real(r8) :: Ren(lbp:ubp)            ! for MJ79 kinetic fractionation factor
    real(r8) :: alpk(pwtrc)             ! kinetic fractionation factor
    real(r8) :: qsnew(lbp:ubp)          ! updated saturated specific humidity
...
!    if((lbp<=47968 .and. 47968<=ubp) .or. (lbp<=22744 .and. 22744<=ubp)) ldbg=.true.
!-----------------------------------------------------------------------
    ...
    ! Assign local pointers for water tracers
    ...
    ITERATION : do while (iter <= niters .and. fncopy > 0)
       ...
       do fp = 1, fncopy
          ...
          qsnew(p) = qsatg(c)+qsatgdT(c)*(t_grnd(c)-tgbef(c))
          eflx_sh_grnd(p) = forc_rho(g)*cpair*(t_grnd(c)-thm(p))/rah(p)
          qflx_evap_soi(p) = forc_rho(g)*(qsnew(p)-forc_q(g))/raw(p)
    ...
    ! Now with final values of raw, to the water tracers
    ! Since this is over water, don't do a special case for dew.
!dir$ concurrent
!cdir nodep
    do fp = 1, num_lakep
       p = filter_lakep(fp)
       ...
       if (hydro_isotope) then
          ! Most appropriate use of MJ79 kinetic effect:
          Ren(p)=ustar(p)*z0mg(p)/1.5e-5_r8
          if(Ren(p)>=1._r8) then  !use rough surface equation
            do m = 1, pwtrc
               ...
               if(ispec(m)==isph217o) then                                !17O (back out effective n for 18O, apply to diffusivity ratio of H217O)
                  kk(p,m)=8.82_r8*ustar(p) + 0.472_r8
                  kk(p,m)=log(1000._r8/(1000._r8-kk(p,m))) / log(difrm(isph218o)) ! effective exponent for a M78 approach
                  alpk(m)=difrm(isph217o)**kk(p,m)
                  kk(p,m)=1000._r8*(1._r8 - (1._r8/alpk(m)))
               end if
               alpliq(m) = wiso_alpliq(ispec(m),t_grnd(c),.true.)
            end do
       ...
       do m = 1, pwtrc
         alpk(m) = 1000._r8/(1000._r8-kk(p,m))
         wtr_raw(p,m) = wtr_raw(p,m)*alpk(m)
         wtr_dqh(p,m) = forc_wtr_q(g,m)-(RSurf(m)*qsatg(c)/alpliq(m))
         wtr_qflx_evap_soi(p,m) = forc_rho(g)*((RSurf(m)*qsnew(p)/alpliq(m))-forc_wtr_q(g,m))/wtr_raw(p,m)
       end do
!DEBUG
!if(ldbg .and. m==mdbg .and. (p==47968 .or. p==22744)) then
if(ldbg .and. m==mdbg .and. p==pdbg) then
write(6,*) 'BioLake00: RSurf=',RSurf(m)
write(6,*) 'BioLake00: alpliq=',alpliq(m)
write(6,*) 'BioLake00: alpk=',alpk(m)
write(6,*) 'BioLake00:',ustar(p),z0mg(p),Ren(p)
write(6,*) 'BioLake00: qflx_evap_soi=',qflx_evap_soi(p),wtr_qflx_evap_soi(p,m)
endif
    end do


    do fp = 1, num_lakep    !corresponds to line 563 in iCLM3 version
       p = filter_lakep(fp)
       c = pcolumn(p)
       g = pgridcell(p)
       ...
       if (h2osno(c) > 0.5_r8 .AND. t_grnd(c) > tfrz) then
          ...
          ! Redo as above. Melted snow mixes with lake water, so just use lake water
          do m = 1, pwtrc
            wtr_raw(p,m) = raw(p)
            RSurf(m) = RGNIP(c,m)
            kk(p,m) = 0._r8
...
!if(ldbg .and. m==mdbg .and. (p==47968 .or. p==22744)) then
if(ldbg .and. m==mdbg .and. p==pdbg) then
write(6,*) 'BioLake01: RSurf=',RSurf(m)
write(6,*) 'BioLake01: alpliq=',alpliq(m)
write(6,*) 'BioLake01: alpk=',alpk(m)
write(6,*) 'BioLake01: qflx_evap_soi=',qflx_evap_soi(p),wtr_qflx_evap_soi(p,m)
endif

          ! special case for frost onto snow (shonky)
          do m = 1, pwtrc
            if (qflx_evap_soi(p) < 0._r8 .or. wtr_qflx_evap_soi(p,m) < 0._r8) then
              RFrost(m) = get_wratio(forc_wtr_q(g,m),forc_wtr_q(g,ixbase),voltiny)
              if (hydro_isotope) then
                alpice(m) = wiso_alpice(ispec(m),t_grnd(c),.true.)
!!                RFrost(m) = RFrost(m)/alpice(m) !account for alpha explicitly
              endif
              wtr_qflx_evap_soi(p,m) = qflx_evap_soi(p)*RFrost(m)/alpice(m) !CRT
!             wtr_qflx_evap_soi(p,m) = qflx_evap_soi(p)*RFrost(m)/alpliq(m)
!DEBUG
!if(ldbg .and. m==mdbg .and. (p==47968 .or. p==22744)) then
if(ldbg .and. m==mdbg .and. p==pdbg) then
write(6,*) 'BioLake02: RFrost=',RFrost(m)
write(6,*) 'BioLake02: alpice=',alpice(m)
write(6,*) 'BioLake02: qflx_evap_soi=',qflx_evap_soi(p),wtr_qflx_evap_soi(p,m)
endif
            endif
          end do
       ...
       do m = 1, pwtrc
          wtr_q_ref2m(p,m) = forc_wtr_q(g,m) + temp2(p)*wtr_dqh(p,m)*(1./temp22m(p) - 1./temp2(p))
       end do
       ...
       do m = 1, pwtrc
         RSnow = get_wratio(wtr_h2osno(c,m),wtr_h2osno(c,ixbase),h2otiny)
         wtr_qmelt(c,m) = RSnow*qmelt(c)
       end do
