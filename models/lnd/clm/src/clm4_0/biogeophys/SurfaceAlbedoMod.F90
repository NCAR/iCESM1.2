...
!
    ...
    real(r8), pointer :: wtr_h2osoi_liq(:,:,:)    ! tracer liquid water content (col,lyr) [kg/m2]
    real(r8), pointer :: wtr_h2osoi_ice(:,:,:)    ! tracer ice lens content (col,lyr) [kg/m2]
...
!
    ...
    real(r8) :: wtr_h2osno_liq(lbc:ubc,-nlevsno+1:0,pwtrc)            ! tracer liquid snow content (col,lyr) [kg m-2]
    real(r8) :: wtr_h2osno_ice(lbc:ubc,-nlevsno+1:0,pwtrc)            ! tracer ice content in snow (col,lyr) [kg m-2]
  !-----------------------------------------------------------------------
...
!         end if ! then removed for CNDV (and dyn. landuse?) cases to work
    ...
    do c=lbc,ubc
       ...
       do m = 1, pwtrc
           wtr_h2osno_liq(c,:,m) = wtr_h2osoi_liq(c,-nlevsno+1:0,m)
           wtr_h2osno_ice(c,:,m) = wtr_h2osoi_ice(c,-nlevsno+1:0,m)
       end do
