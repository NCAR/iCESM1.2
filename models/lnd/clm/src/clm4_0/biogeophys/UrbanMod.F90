...
! !USES:
    ...
    use clm_varcon         , only : cpair, vkc, spval, icol_roof, icol_sunwall, &
                                    ...
                                    wasteheat_limit, watmin
    use filterMod          , only : filter
    use FrictionVelocityMod, only : FrictionVelocity, MoninObukIni
    use QSatMod            , only : QSat
    use clm_varpar         , only : maxpatch_urb, nlevurb, nlevsoi
    use clm_time_manager   , only : get_curr_date, get_step_size, get_nstep
    use clm_atmlnd         , only : clm_a2l
    use HydrologyTracer    , only : pwtrc, get_wratio, ixbase, h2otiny, voltiny, &
                                    cdbg, mdbg, pdbg, ispec, Rstnd
    use HydrologyIsotope   , only : wiso_alpliq, wiso_alpice, &
                                    wiso_alpkin_mb96, wiso_alpkin_m78
...
!
    ...
    real(r8), pointer :: wtr_h2osoi_ice(:,:,:)! tracer ice water (kg/m2) - JN
    real(r8), pointer :: frac_sno(:)          ! fraction of ground covered by snow (0 to 1)
    ...
    real(r8), pointer :: RCanopyVapor(:,:)    ! ratio of tracer vapor in canopy air space
...
!
    ...
    real(r8) :: wtr_surf_tot           !soil ice and liquid sum for water tracer [kg/m2]
    real(r8) :: base_surf_tot          !soil ice and liquid sum for base H2O tracer [kg/m2]
    real(r8) :: RSurf(pwtrc)           ! temp tracer ratio for calcuation of tracer fluxes
    real(r8) :: RVapor(pwtrc)          ! temp tracer ratio for calcuation of tracer fluxes
    real(r8) :: alpeq                  ! equilibrium fractionation for evaporation
    real(r8) :: alpk                   ! kinetic fractionation for evaporation

    logical, parameter :: lcrat1=.true.
    logical, parameter :: lcrat2=.true.
    logical, parameter :: lcrat3=.true.
    logical, parameter :: lcrat4=.true.
    logical :: ldbg=.false.
!    if(lbc<=cdbg .and. cdbg<=ubc) ldbg=.true.
...
!DEBUG
if(ldbg .and. c==cdbg) then
write(6,*) 'urban01: h2osoi_liq0=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban01: h2osoi_liqm=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban01: h2osoi_ice0=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban01: h2osoi_icem=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban01: RCanopyVapor=',RCanopyVapor(pdbg,mdbg)
write(6,*) 'urban01: qflx_evap_soi=',qflx_evap_soi(pdbg),wtr_qflx_evap_soi(pdbg,mdbg)
write(6,*) 'urban01: qflx_tran_veg=',qflx_tran_veg(pdbg),wtr_qflx_tran_veg(pdbg,mdbg)
endif
       if (ctype(c) == icol_roof) then
         qflx_evap_soi(p) = -forc_rho(g)*wtuq_roof(l)*dqh(l)
         do m = 1, pwtrc
             ! Calculate RSurf for evap/tran same as in CanopyFluxes
             RSurf(m) = Rstnd(m)   ! initialize
             wtr_surf_tot  = wtr_h2osoi_ice(c,snl(c)+1,m)+wtr_h2osoi_liq(c,snl(c)+1,m)
             base_surf_tot = wtr_h2osoi_ice(c,snl(c)+1,ixbase)+wtr_h2osoi_liq(c,snl(c)+1,ixbase)
             RSurf(m) = get_wratio(wtr_surf_tot,base_surf_tot,voltiny)
             if(RSurf(m)==0._r8 .and. qflx_evap_soi(p).ne.0._r8) RSurf(m)=Rstnd(m)
             RVapor(m) = get_wratio(forc_wtr_q(g,m),forc_wtr_q(g,ixbase),voltiny)
             if(lcrat1) then
               if(qflx_evap_soi(p)>=0._r8) wtr_qflx_evap_soi(p,m)=RSurf(m) *qflx_evap_soi(p)
               if(qflx_evap_soi(p)< 0._r8) wtr_qflx_evap_soi(p,m)=RVapor(m)*qflx_evap_soi(p)
             else
               alpeq = wiso_alpliq(ispec(m),t_grnd(c),.false.)
               alpk = wiso_alpkin_m78(ispec(m),.67_r8,.false.) !.67=turbulent
               wtr_qflx_evap_soi(p,m) = (forc_rho(g)*wtuq_roof(l)/alpk)*((RSurf(m)*qg(c)/alpeq) - RVapor(m)*qaf(l))
             endif
         end do
!DEBUG
if(ldbg .and. c==cdbg) then
write(6,*) 'urban02: h2osoi_liq0=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban02: h2osoi_liqm=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban02: h2osoi_ice0=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban02: h2osoi_icem=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban02: RCanopyVapor=',RCanopyVapor(pdbg,mdbg)
write(6,*) 'urban02: qflx_evap_soi=',qflx_evap_soi(pdbg),wtr_qflx_evap_soi(pdbg,mdbg)
write(6,*) 'urban02: qflx_tran_veg=',qflx_tran_veg(pdbg),wtr_qflx_tran_veg(pdbg,mdbg)
endif
       else if (ctype(c) == icol_road_perv) then
         ! Evaporation assigned to soil term if dew or snow
         ! or if no liquid water available in soil column (qflx < 0 case)
         if (dqh(l) > 0._r8 .or. frac_sno(c) > 0._r8 .or. soilalpha_u(c) .le. 0._r8) then
            ...
            do m = 1, pwtrc
               ...
               if(lcrat2) then
                 ...
                 alpeq = wiso_alpliq(ispec(m),t_grnd(c),.false.)
                 alpk = wiso_alpkin_m78(ispec(m),.67_r8,.false.) !.67=turbulent
                 wtr_qflx_evap_soi(p,m) = (forc_rho(g)*wtuq_road_perv(l)/alpk)*((RSurf(m)*qg(c)/alpeq) - RVapor(m)*qaf(l))
               endif
               wtr_qflx_tran_veg(p,m) = 0._r8
            end do
!DEBUG
if(ldbg .and. c==cdbg) then
write(6,*) 'urban03: h2osoi_liq0=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban03: h2osoi_liqm=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban03: h2osoi_ice0=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban03: h2osoi_icem=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban03: RCanopyVapor=',RCanopyVapor(pdbg,mdbg)
write(6,*) 'urban03: qflx_evap_soi=',qflx_evap_soi(pdbg),wtr_qflx_evap_soi(pdbg,mdbg)
write(6,*) 'urban03: qflx_tran_veg=',qflx_tran_veg(pdbg),wtr_qflx_tran_veg(pdbg,mdbg)
endif
         ! Otherwise, evaporation assigned to transpiration term
            ...
            do m = 1, pwtrc
               ...
               RVapor(m) = get_wratio(forc_wtr_q(g,m),forc_wtr_q(g,ixbase),voltiny)
               if(lcrat3) then
                 if(qflx_tran_veg(p)>=0._r8) wtr_qflx_tran_veg(p,m)=RSurf(m) *qflx_tran_veg(p)
                 if(qflx_tran_veg(p)< 0._r8) wtr_qflx_tran_veg(p,m)=RVapor(m)*qflx_tran_veg(p)
               else
                 alpeq = wiso_alpliq(ispec(m),t_grnd(c),.false.)
                 alpk = wiso_alpkin_m78(ispec(m),.67_r8,.false.) !.67=turbulent
                 wtr_qflx_tran_veg(p,m) = (forc_rho(g)*wtuq_road_perv(l)/alpk)*((RSurf(m)*qg(c)/alpeq) - RVapor(m)*qaf(l))
               endif
               wtr_qflx_evap_soi(p,m) = 0._r8
             end do
!DEBUG
if(ldbg .and. c==cdbg) then
write(6,*) 'urban04: h2osoi_liq0=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban04: h2osoi_liqm=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban04: h2osoi_ice0=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban04: h2osoi_icem=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban04: RCanopyVapor=',RCanopyVapor(pdbg,mdbg)
write(6,*) 'urban04: qflx_evap_soi=',qflx_evap_soi(pdbg),wtr_qflx_evap_soi(pdbg,mdbg)
write(6,*) 'urban04: qflx_tran_veg=',qflx_tran_veg(pdbg),wtr_qflx_tran_veg(pdbg,mdbg)
endif
         end if
         qflx_evap_veg(p) = qflx_tran_veg(p)
         do m = 1, pwtrc
            wtr_qflx_evap_veg(p,m) = wtr_qflx_tran_veg(p,m)
         end do
       else if (ctype(c) == icol_road_imperv) then
         ...
         do m = 1, pwtrc
            ...
            if(lcrat4) then
              ...
              alpeq = wiso_alpliq(ispec(m),t_grnd(c),.false.)
              alpk = wiso_alpkin_m78(ispec(m),.67_r8,.false.) !.67=turbulent
              wtr_qflx_evap_soi(p,m) = (forc_rho(g)*wtuq_road_imperv(l)/alpk)*((RSurf(m)*qg(c)/alpeq) - RVapor(m)*qaf(l))
            endif
         end do
!DEBUG
if(ldbg .and. c==cdbg) then
write(6,*) 'urban05: h2osoi_liq0=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban05: h2osoi_liqm=',h2osoi_liq(cdbg,snl(cdbg)+1),wtr_h2osoi_liq(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban05: h2osoi_ice0=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,ixbase)
write(6,*) 'urban05: h2osoi_icem=',h2osoi_ice(cdbg,snl(cdbg)+1),wtr_h2osoi_ice(cdbg,snl(cdbg)+1,mdbg)
write(6,*) 'urban05: RCanopyVapor=',RCanopyVapor(pdbg,mdbg)
write(6,*) 'urban05: qflx_evap_soi=',qflx_evap_soi(pdbg),wtr_qflx_evap_soi(pdbg,mdbg)
write(6,*) 'urban05: qflx_tran_veg=',qflx_tran_veg(pdbg),wtr_qflx_tran_veg(pdbg,mdbg)
endif
       else if (ctype(c) == icol_sunwall) then
         qflx_evap_soi(p) = 0._r8
         do m = 1, pwtrc
    ...
    do f = 1, num_urbanp
       ...
       do m = 1, pwtrc
          RVapor(m) = get_wratio(forc_wtr_q(g,m),forc_wtr_q(g,ixbase),voltiny)
          if(RVapor(m)==0._r8) RVapor(m) = get_wratio(wtr_qg(c,m),wtr_qg(c,ixbase),voltiny)
          wtr_q_ref2m(p,m) = RVapor(m)*q_ref2m(p)
       end do
