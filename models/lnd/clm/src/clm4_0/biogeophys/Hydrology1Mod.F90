...
! !USES:
    ...
    use HydrologyTracer, only: pwtrc, ixbase, get_wratio, qflxtiny, h2otiny, Rstnd, cdbg, pdbg, mdbg
...
!
    ...
    real(r8), pointer :: wtr_h2ocan_loss(:,:)! tracer canopy water mass balance term (column)
!
...
!
    ...
    real(r8), pointer :: wtr_qflx_irrig(:,:)    ! tracer irrigation amount (mm/s)
!
...
!
! local pointers to water tracer variables (analogs of the non-'wtr-' versions avove)
!
    real(r8), pointer :: wtr_h2ocan(:,:)       ! tracer total canopy water (mm H2O)
    ...
    real(r8), pointer :: wtr_qflx_snow_grnd_pft(:,:) ! tracer snow on ground after interception (mm H2O/s) [+]
...
!
    ...
    integer  :: pi,p2                        ! pft index, p2=debugging pft index
    integer  :: p                            ! pft index
    ...
    real(r8) :: wtr_qflx_candrip(lbp:ubp,pwtrc)        ! tracer rate of canopy runoff and snow falling off canopy [mm/s]
    real(r8) :: wtr_qflx_through_rain(lbp:ubp,pwtrc)   ! tracer direct rain throughfall [mm/s]
    real(r8) :: wtr_qflx_through_snow(lbp:ubp,pwtrc)   ! tracer direct snow throughfall [mm/s]
    ...
    real(r8) :: wtr_qflx_prec_grnd_snow(lbp:ubp,pwtrc) ! tracer snow precipitation incident on ground [mm/s]
    real(r8) :: wtr_qflx_prec_grnd_rain(lbp:ubp,pwtrc) ! tracer rain precipitation incident on ground [mm/s]
    real(r8) :: wratio(pwtrc)                ! water mass ratio for tracers
...
!-----------------------------------------------------------------------
    ...
    wtr_h2ocan_loss    => cwf%wtr_h2ocan_loss
    ...
    wtr_qflx_snow_grnd_pft => pwf%wtr_qflx_snow_grnd
    ...
    do f = 1, num_nolakep
       ...
       if (ltype(l)==istsoil .or. ltype(l)==istwet .or. ltype(l)==isturb .or. &
          ...
          do m = 1, pwtrc
            wtr_qflx_candrip(p,m) = 0._r8      ! rate of canopy runoff
            wtr_qflx_through_snow(p,m) = 0._r8 ! rain precipitation direct through canopy
            wtr_qflx_through_rain(p,m) = 0._r8 ! snow precipitation direct through canopy
            wtr_qflx_prec_intr(p,m) = 0._r8    ! total intercepted precipitation
          end do
          ...
          if (ctype(c) /= icol_sunwall .and. ctype(c) /= icol_shadewall) then
             if (frac_veg_nosno(p) == 1 .and. (forc_rain(g) + forc_snow(g)) > 0._r8) then
                ...
                do m = 1, pwtrc
                  wtr_qflx_through_snow(p,m) = forc_wtr_snow(g,m) * (1._r8-fpi)
                  wtr_qflx_through_rain(p,m) = forc_wtr_rain(g,m) * (1._r8-fpi)
                end do
                
                ! Intercepted precipitation [mm/s]
                qflx_prec_intr(p) = (forc_snow(g) + forc_rain(g)) * fpi
                do m = 1, pwtrc
                   wtr_qflx_prec_intr(p,m) = (forc_wtr_snow(g,m) + forc_wtr_rain(g,m)) * fpi
                end do
                
                ! Water storage of intercepted precipitation and dew
                h2ocan(p) = max(0._r8, h2ocan(p) + dtime*qflx_prec_intr(p))
                do m = 1, pwtrc
                  wtr_h2ocan(p,m) = max(0._r8, wtr_h2ocan(p,m) + dtime*wtr_qflx_prec_intr(p,m))
                  if(h2ocan(p)==0._r8) wtr_h2ocan(p,m)=0._r8    ! if no h2ocan, no isotopic h2ocan
                end do
                
                ! Initialize rate of canopy runoff and snow falling off canopy
                qflx_candrip(p) = 0._r8
                do m = 1, pwtrc
                  wtr_qflx_candrip(p,m) = 0._r8
                end do
                ...
                if (xrun > 0._r8) then
                   ...
                   ! Preserve tracer ratio
                   do m = 1, pwtrc
                      wratio(m) = get_wratio(wtr_h2ocan(p,m),wtr_h2ocan(p,ixbase),h2otiny)
                   end do
                   ...
                   do m = 1, pwtrc
                      wtr_qflx_candrip(p,m) = wratio(m)*xrun
                      wtr_h2ocan(p,m) = min(h2ocanmx, wratio(m)*h2ocanmx)
                   end do
                end if

                ! Check if the tracer canopy water is excessive too
                do m = 1, pwtrc
                   xrun = (wtr_h2ocan(p,m)-h2ocanmx)/dtime
                   if(xrun > 0._r8) then
                      wtr_qflx_candrip(p,m) = wtr_qflx_candrip(p,m) + xrun
                      wtr_h2ocan(p,m) = h2ocanmx
                   end if
                end do
          ...
          do m = 1, pwtrc
             ...
             wtr_qflx_candrip(p,m) = 0._r8
             wtr_qflx_through_snow(p,m) = 0._r8
             wtr_qflx_through_rain(p,m) = 0._r8
             wtr_qflx_prec_intr(p,m) = 0._r8
          end do
       ...
       if (ctype(c) /= icol_sunwall .and. ctype(c) /= icol_shadewall) then
          if (frac_veg_nosno(p) == 0) then
             ...
             do m = 1,pwtrc
               wtr_qflx_prec_grnd_snow(p,m) = forc_wtr_snow(g,m)
               wtr_qflx_prec_grnd_rain(p,m) = forc_wtr_rain(g,m) + wtr_h2ocan_loss(c,m)
             end do
          else
             ...
             do m = 1, pwtrc
               wtr_qflx_prec_grnd_snow(p,m) = wtr_qflx_through_snow(p,m) + (wtr_qflx_candrip(p,m) * fracsnow(p))
               wtr_qflx_prec_grnd_rain(p,m) = wtr_qflx_through_rain(p,m) + (wtr_qflx_candrip(p,m) * fracrain(p)) + wtr_h2ocan_loss(c,m)
             end do
          end if
       ! Urban sunwall and shadewall have no intercepted precipitation
          ...
          do m = 1, pwtrc
             wtr_qflx_prec_grnd_snow(p,m) = 0._r8
             wtr_qflx_prec_grnd_rain(p,m) = 0._r8
          end do
       end if
       ...
       do m = 1, pwtrc
          wtr_qflx_prec_grnd_rain(p,m) = wtr_qflx_prec_grnd_rain(p,m) + wtr_qflx_irrig(c,m)
       end do
       ...
       do m = 1, pwtrc
         wtr_qflx_prec_grnd(p,m) = wtr_qflx_prec_grnd_snow(p,m) + wtr_qflx_prec_grnd_rain(p,m)
       end do
       if (do_capsnow(c)) then
          ...
          do m = 1, pwtrc
            wtr_qflx_snwcp_liq(p,m) = wtr_qflx_prec_grnd_rain(p,m)
            wtr_qflx_snwcp_ice(p,m) = wtr_qflx_prec_grnd_snow(p,m)
            wtr_qflx_snow_grnd_pft(p,m) = 0._r8
          ...
          do m = 1, pwtrc
            wtr_qflx_snwcp_liq(p,m) = 0._r8
            wtr_qflx_snwcp_ice(p,m) = 0._r8
            wtr_qflx_snow_grnd_pft(p,m) = wtr_qflx_prec_grnd_snow(p,m)  ! tracer ice onto ground (mm/s)
            wtr_qflx_rain_grnd(p,m)     = wtr_qflx_prec_grnd_rain(p,m)  ! tracer liquid water onto ground (mm/s)
          end do
       end if
    ...
    call p2c(pwtrc, num_nolakec, filter_nolakec, wtr_qflx_snow_grnd_pft, wtr_qflx_snow_grnd_col)
!    do m = 1, pwtrc
!       call p2c(num_nolakec, filter_nolakec, wtr_qflx_snow_grnd_pft(:,m), wtr_qflx_snow_grnd_col(:,m))
!    end do
...
!rtm_flood
    ...
    do f = 1, num_nolakec
       ...
       if (do_capsnow(c)) then
          ...
          do m = 1, pwtrc
              wtr_h2osno(c,m) = wtr_h2osno(c,m) + wtr_qflx_snow_grnd_col(c,m)*dtime
          end do
       end if
       ...
       if (snl(c) == 0 .and. qflx_snow_grnd_col(c) > 0.0_r8 .and. snowdp(c) >= 0.01_r8) then
          ...
          do m = 1, pwtrc
            wtr_h2osoi_ice(c,0,m) = wtr_h2osno(c,m) ! kg/m2
            wtr_h2osoi_liq(c,0,m) = 0._r8           ! kg/m2
          end do
       ...
       if (snl(c) < 0 .and. newnode == 0) then
          ...
          do m = 1, pwtrc
            wtr_h2osoi_ice(c,snl(c)+1,m) = wtr_h2osoi_ice(c,snl(c)+1,m)+dtime*wtr_qflx_snow_grnd_col(c,m)
          end do
