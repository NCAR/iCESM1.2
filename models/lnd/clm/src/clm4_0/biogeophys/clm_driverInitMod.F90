...
! !USES:
    ...
    use HydrologyTracer, only : pwtrc, TracerCheckEqual
...
!
    ...
    real(r8), pointer :: wtr_h2osno_old(:,:)   ! sntracer ow water (mm H2O) at previous time step
...
!
    ...
    real(r8), pointer :: forc_wtr_rain(:,:)    ! tracer atmospheric rain (mm/s)
    real(r8), pointer :: forc_wtr_snow(:,:)    ! tracer atmospheric snow (mm/s)
    real(r8), pointer :: forc_wtr_Rclim(:,:)! tracer ratio climatology () 
    ...
    real(r8) :: zbot_g, tbot_g, pbot_g, thbot_g, qbot_g, qs_g, es_g, wtr_qbot_g(pwtrc)
    real(r8) :: zbot_c, tbot_c, pbot_c, thbot_c, qbot_c, qs_c, es_c, wtr_qbot_c(pwtrc)
    real(r8) :: egcm_c, rhos_c
    real(r8) :: dum1,   dum2

    logical  :: lfail                       ! Flag for failure of TracerCheckEqual
...
!-----------------------------------------------------------------------
    ...
    wtr_h2osno_old     => cws%wtr_h2osno_old
    ...
    forc_wtr_Rclim     => cws%forc_wtr_Rclim
    ...
    do c = lbc, ubc
      ...
      do m = 1, pwtrc
        forc_wtr_q(c,m)    = clm_a2l%forc_wtr_q(g,m)
        forc_wtr_rain(c,m) = clm_a2l%forc_wtr_rain(g,m)
        forc_wtr_snow(c,m) = clm_a2l%forc_wtr_snow(g,m)
        forc_wtr_Rclim(c,m) = clm_a2l%forc_wtr_Rclim(g,m)
      end do
      ...
      do m = 1, pwtrc
        wtr_h2osno_old(c,m) = wtr_h2osno(c,m)
      end do
   ...
   ! For other columns the downscaling is a simple copy.
    ...
    do f = 1, num_nolakec
       ...
       if (ityplun(l) == istice_mec) then   ! downscale to elevation classes
          ...
          do m = 1, pwtrc
            wtr_qbot_g(m) = clm_a2l%forc_wtr_q(g,m) ! atm sfc spec humid
          end do
          ...
          do m = 1, pwtrc
            wtr_qbot_c(m) = wtr_qbot_g(m)*(qs_c/qs_g)
          end do
