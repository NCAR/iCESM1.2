...
! !USES:
    ...
    use HydrologyTracer    , only : pwtrc, ixbase, h2otiny, voltiny, get_wratio, ispec, TracerCheckEqual, &
                                    cdbg, pdbg, mdbg
...
!
    ...
    integer  :: pi            !index
    real(r8) :: alpeq         ! equilibrium fractionation for isotopes
    real(r8) :: wratio(pwtrc) ! water tracer ratio
...
!------------------------------------------------------------------------------
   ...
   ! Assign local pointers to derived type members (gridcell-level)
    ...
    do fc = 1,num_nolakec
       ...
       ! Assign ratio from soil water (look deeper if top is dry) or climatology
       ! Modified to use ratio of whatever the surface is, snow or soil - TW
       do m = 1, pwtrc
         alpeq = wiso_alpliq(ispec(m),t_grnd(c),.true.)
         wratio(m) = alpeq*get_wratio(forc_wtr_q(c,m),forc_wtr_q(c,ixbase),voltiny)  ! initialize
         if (ityplun(l)/=istwet .and. ityplun(l)/=istice) then
           ! if soil isn't soaked or ice, get ratio from the first layer > h2otiny
           do j = snl(c)+1, nlevsoi
              if (wtr_h2osoi_liq(c,j,ixbase)+wtr_h2osoi_ice(c,j,ixbase) > h2otiny) then
                 ! not exactly the evaporating water, but should do the job - TW
                 wratio(m) = get_wratio(wtr_h2osoi_liq(c,j,m     ) + wtr_h2osoi_ice(c,j,m     ), &
                                        wtr_h2osoi_liq(c,j,ixbase) + wtr_h2osoi_ice(c,j,ixbase),h2otiny)
                 exit
              endif
           end do       ! j, typically only 1
         endif
         wtr_qg(c,m) = wratio(m)*qg(c)    ! make sure you aren't double-counting this alpeq in flux calculations...
       end do
    ...
    do fp = 1,num_nolakep
       ...
       do m = 1, pwtrc
         wtr_qflx_evap_tot(p,m) = 0._r8
         wtr_qflx_evap_veg(p,m) = 0._r8
