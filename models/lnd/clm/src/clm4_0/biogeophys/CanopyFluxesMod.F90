...
! !USES:
    ...
    use clm_varcon         , only : sb, cpair, hvap, vkc, grav, denice, &
                                    ...
                                    isecspday, degpsec, watmin
    use shr_const_mod      , only : SHR_CONST_TKFRZ
    use pftvarcon          , only : nirrig
    use QSatMod            , only : QSat
    use FrictionVelocityMod, only : FrictionVelocity, MoninObukIni
    use spmdMod            , only : masterproc
    use HydrologyTracer    , only : pwtrc, get_wratio, ixbase, voltiny, h2otiny, &
                                    pdbg, cdbg, mdbg, ispec, HydrologyTracerRatios, TracerCheckEqual, &
                                    HydrologyTracerCanopy, qflxtiny, rsfcmin
...
!
   ...
   real(r8), pointer :: wtr_h2ocan(:,:)    ! tracer canopy water (mm H2O)
!
...
!
   ...
   real(r8), pointer :: raw1(:)            ! canopy-GCM moisture resistance (version to save to history files) [s/m]
   ...
   real(r8), pointer :: fcaneq(:)          ! canopy water tracer equilibration fraction
...
!   real(r8) :: rppdry                ! fraction of potential evaporation through transp [-]
   real(r8) :: rppdry(lbp:ubp)       ! fraction of potential evaporation through transp [-] (for tracer alpk, weights)
   ...
   real(r8) :: raww(lbp:ubp)         ! portion of surface-canopy moisture resistance which does not fractionate [s/m]
   ...
   real(r8) :: wtr_wtaq(pwtrc)         ! tracer latent heat conductance for air [m/s]
   real(r8) :: wtr_wtlq(pwtrc)         ! tracer latent heat conductance for leaf [m/s]
   real(r8) :: wtr_wtgq(lbp:ubp,pwtrc) ! tracer latent heat conductance for ground [m/s]
   ...
   real(r8) :: wtr_wtaq0(lbp:ubp,pwtrc) ! tracer normalized latent heat conductance for air [-]
   real(r8) :: wtr_wtlq0(lbp:ubp,pwtrc) ! tracer normalized latent heat conductance for leaf [-]
!  real(r8) :: wtr_wtgq0(pwtrc) ! tracer normalized heat conductance for ground [-]
   real(r8) :: wtr_wtgq0(lbp:ubp,pwtrc) ! tracer normalized heat conductance for ground [-] CRT
   ...
   real(r8) :: wtlsunq(lbp:ubp)      ! latent heat conoductance for sunlit leaves
   real(r8) :: wtlshaq(lbp:ubp)      ! latent heat conductance for shaded leaves
   real(r8) :: wtlcanq(lbp:ubp)      ! latent heat conductance for wet canopy
   real(r8) :: wtlsunq0(lbp:ubp)     ! normalized latent heat conoductance for sunlit leaves
   real(r8) :: wtlshaq0(lbp:ubp)     ! normalized latent heat conductance for shaded leaves
   real(r8) :: wtlcanq0(lbp:ubp)     ! normalized latent heat conductance for wet canopy
   real(r8) :: wtr_wtlsunq(lbp:ubp,pwtrc) ! tracer latent heat conoductance for sunlit leaves
   real(r8) :: wtr_wtlshaq(lbp:ubp,pwtrc) ! tracer latent heat conductance for shaded leaves
   real(r8) :: wtr_wtlcanq(lbp:ubp,pwtrc) ! tracer latent heat conductance for wet canopy
   real(r8) :: wtr_wtlsunq0(lbp:ubp,pwtrc) ! normalized tracer latent heat conoductance for sunlit leaves
   real(r8) :: wtr_wtlshaq0(lbp:ubp,pwtrc) ! normalized tracer latent heat conductance for shaded leaves
   real(r8) :: wtr_wtlcanq0(lbp:ubp,pwtrc) ! normalized tracer latent heat conductance for wet canopy
...
!   real(r8) :: efpot                 ! potential latent energy flux [kg/m2/s]
   real(r8) :: efpot(lbp:ubp)        ! potential latent energy flux [kg/m2/s] (DEBUG, for tracer alpk, weights)
   ...
   real(r8) :: rdl(lbp:ubp)          ! dry litter layer resistance for water vapor  (s/m)
   ...
   real(r8) :: wtltrnq               ! latent heat resistance for total transportaion
   real(r8) :: wtlnetq               ! latent heat resistance for sun, shade and canopy [-]

   real(r8) :: qsatlsv(lbp:ubp)      ! saved leaf specific humidity from last iteration [kg/kg]
   real(r8) :: qafsv(lbp:ubp)        ! saved humidity of canopy air from last iteration [kg/kg]

   real(r8) :: wtr_delq              ! water tracer saturation specific humidity deficit (kg/kg)
   real(r8) :: wtr_dqh               ! diff of tracer humidity between ref. height and surface
   real(r8) :: wtr_qsatl             ! effective tracer saturation over leaves

   real(r8) :: RSoilLiq(lbp:ubp,nlevsoi,pwtrc)    ! soil liquid
   real(r8) :: RSoilIce(lbp:ubp,nlevsoi,pwtrc)    ! soil ice
   real(r8) :: RSoilVol(lbp:ubp,nlevsoi,pwtrc)    ! net soil content
   real(r8) :: RXylemWater  (lbp:ubp,pwtrc)       ! water taken by roots
   real(r8) :: RSnowWater   (lbp:ubp,pwtrc)       ! snow water
   real(r8) :: RLeafWater   (lbp:ubp,pwtrc)       ! weighted leaf water (sunlit vs. shaded)
   real(r8) :: RCanopyWater(lbp:ubp,pwtrc)        ! canopy water
   real(r8) :: RSoilVapor(lbp:ubp,pwtrc)          ! Vapor in equilibrium with soil water
   real(r8) :: RDew,RFrost                        ! tracer ratio for dew
   real(r8) :: RFlux(pwtrc)                       ! tracer ratio for fluxes
   real(r8) :: alpk(2)                ! kinetic fractionation for evaporation, transpiration (2 = turb, diff, or anywhere in between)
   real(r8) :: alpeq                  ! equilibrium fractionation
   real(r8) :: delqleaf               ! specific humidity deficit at leaf [kg/kg]
   real(r8) :: h2onew(lbp:ubp,pwtrc)              ! modified (canopy/soil) water partially equilibrated with vapor
   real(r8) :: h2otot                             ! total new water after evap by PFT
   real(r8) :: wtr_qflx_exchange          ! flux associated with isotopic exchange

   real(r8) :: wtr_surf_tot           !soil ice and liquid sum for water tracer [kg/m2] -JN
   real(r8) :: base_surf_tot          !soil ice and liquid sum for base H2O tracer [kg/m2] -JN
   real(r8) :: Ren(lbp:ubp)            ! Reynolds number for surface
   real(r8) :: kk(lbp:ubp,pwtrc)        ! Merlivat and Jouzel (1979) k for alpkin

   logical  :: lfail                  ! fail flag for TracerCheckEqual
   logical :: exist
   logical :: ldbg=.false.
!   if(ubc>=cdbg .and. lbc<=cdbg) ldbg=.true.
!------------------------------------------------------------------------------
   ...
   do f = 1, fn
      ...
      alpk(1:2) = 1._r8 ! init kinetic fractionation to 1
      wtaq0(p)  = 0._r8
      obuold(p) = 0._r8
      btran(p)  = btran0
   end do
...
!cdir nodep
      do f = 1, fn
         ...
         ! (separate raw(p,2) into raww(p) (no frac) and rsfc(p) (frac), for isotopes only)
         rsfc(p)= rsfcmin
         raww(p)= raw(p,2)-rsfc(p)
         if(soilbeta(c)>rsfcmin) then
            ! Option 1: like ISOLSM
            !rsfc(p) = 150._r8*frac_sno(c) + (1._r8-frac_sno(c))*raw(p,2)*(1._r8-soilbeta(c))/soilbeta(c)
            !if(rsfc(p)<raw(p,2)) then
            !   raww(p)= raw(p,2)-rsfc(p)
            !   rsfc(p)= raw(p,2)-raww(p)
            !else
            !   rsfc(p)= rsfcmin
            !   raww(p)= raw(p,2)-rsfc(p)
            !end if
            ! Option 2: soilbeta-weighted
            raww(p) = soilbeta(c)*raw(p,2)
            rsfc(p) = (1._r8-soilbeta(c))*raw(p,2)
         end if
...
!cdir nodep
      do f = 1, fn
         ...
         if (fdry(p) .gt. 0._r8) then
            rppdry(p)  = fdry(p)*rb(p)*(laisun(p)/(rb(p)+rssun(p)) + &
                                     laisha(p)/(rb(p)+rssha(p)))/elai(p)
         else
            rppdry(p) = 0._r8
         end if
         efpot(p) = forc_rho(g)*wtl*(qsatl(p)-qaf(p))

         if (efpot(p) > 0._r8) then
            if (btran(p) > btran0) then
               qflx_tran_veg(p) = efpot(p)*rppdry(p)
               rpp = rppdry(p) + fwet(p)
            else
            ...
            rpp = min(rpp, (qflx_tran_veg(p)+h2ocan(p)/dtime)/efpot(p))
         else
         ...
         do m=1,pwtrc
            wtr_wtaq(m) = frac_veg_nosno(p)/raw(p,1)                ! no fractionation through canopy-to-GCM air (save a function call!)
            alpk(1) = wiso_alpkin_m78(ispec(m),.67_r8,.true.)       ! fractionation through air just outside leaves (leaf boundary layer)
            wtr_wtlq(m) = frac_veg_nosno(p)*(elai(p)+esai(p))/(alpk(1)*rb(p)) * rpp
         enddo
         ...
         rdl(p) = ( 1._r8 - exp(-elai_dl) ) / ( 0.004_r8*uaf(p)) ! dry litter layer resistance

         ! Calculate a kinetic effect based on Merlivat and Jouzel, 1979 [Option 2: using fit from MJ79 Fig 2]
         Ren(p)=ustar(p)*z0mg(c)/1.5e-5_r8
         ...
         if (delq(p) .lt. 0._r8) then  !dew. Do not apply beta for negative flux (follow old rsoil)
            wtgq(p) = frac_veg_nosno(p)/(raw(p,2)+rdl(p))
            do m = 1, pwtrc
               alpk(2) = wiso_alpkin_m78(ispec(m),0.0_r8,.true.)        ! kinetic effect through leaf litter layer
               ! Kinetic factor from M78
               alpk(1) = wiso_alpkin_m78(ispec(m),.67_r8,.true.)
               !wtr_wtgq(p,m) = frac_veg_nosno(p)/( raww(p) + (alpk(1)*rsfc(p)) + (alpk(2)*rdl(p)) )
               ! Kinetic factor from MJ79
               !alpk(1) = 1000._r8/(1000._r8-kk(p,m))                    ! kinetic effect through aerodynamic resistances
               !alpk(1) = alpk(1) + ((alpk(1)-1._r8)*raw(p,1)/raw(p,2))  ! cheat to apply MJ79 alpk to raw and raw'
               wtr_wtgq(p,m) = frac_veg_nosno(p)/( (alpk(1)*raw(p,2)) + (alpk(2)*rdl(p)) )
            end do
         else
            wtgq(p) = soilbeta(c)*frac_veg_nosno(p)/(raw(p,2)+rdl(p))
            if(cws%h2osno(c)>0._r8) then
               do m = 1, pwtrc
                  alpk(2) = wiso_alpkin_m78(ispec(m),0.0_r8,.true.)        ! kinetic effect through leaf litter layer
                  ! Kinetic factor from M78
                  alpk(1) = wiso_alpkin_m78(ispec(m),.67_r8,.true.)
                  !wtr_wtgq(p,m) = soilbeta(c)*frac_veg_nosno(p)/( raww(p) + (alpk(1)*rsfc(p)) + (alpk(2)*rdl(p)) )
                  ! Kinetic factor from MJ79
                  !alpk(1) = 1000._r8/(1000._r8-kk(p,m))                    ! kinetic effect through aerodynamic resistances
                  !alpk(1) = alpk(1) + ((alpk(1)-1._r8)*raw(p,1)/raw(p,2))  ! cheat to apply MJ79 alpk to raw and raw'
                  wtr_wtgq(p,m) = soilbeta(c)*frac_veg_nosno(p)/( (alpk(1)*raw(p,2)) + (alpk(2)*rdl(p)) )
               end do
            else
               do m = 1, pwtrc
                  alpk(2) = wiso_alpkin_m78(ispec(m),0.0_r8,.true.)        ! kinetic effect through leaf litter layer
                  ! Kinetic factor from MB96
                  alpk(1) = wiso_alpkin_m78(ispec(m),.67_r8,.true.)
                  !alpk(1) = wiso_alpkin_mb96( ispec(m), cws%h2osoi_vol(c,1), watmin, watsat(c,1),.true.)
                  !wtr_wtgq(p,m) = soilbeta(c)*frac_veg_nosno(p)/( raww(p) + (alpk(1)*rsfc(p)) + (alpk(2)*rdl(p)) )
                  ! Kinetic factor from MJ79
                  !alpk(1) = 1000._r8/(1000._r8-kk(p,m))                    ! kinetic effect through aerodynamic resistances
                  !alpk(1) = alpk(1) + ((alpk(1)-1._r8)*raw(p,1)/raw(p,2))  ! cheat to apply MJ79 alpk to raw and raw'
         ...
         do m=1,pwtrc
            wtsqi = 1._r8/(wtr_wtaq(m)+wtr_wtlq(m)+wtr_wtgq(p,m))
            wtr_wtgq0(p,m) = wtr_wtgq(p,m)*wtsqi !CRT
!           wtr_wtgq0(m)   = wtr_wtgq(p,m)*wtsqi
            wtr_wtlq0(p,m) = wtr_wtlq(m)*wtsqi
            wtr_wtaq0(p,m) = wtr_wtaq(m)*wtsqi
         enddo
         ...
         qsatlsv(p) = qsatl(p)+qsatldT(p)*dt_veg(p)
         qafsv(p) = wtlq0(p)*qsatlsv(p) + wtgq0*qg(c) + forc_q(g)*wtaq0(p)
         efpot(p) = forc_rho(g)*wtl*(wtgaq*(qsatl(p)+qsatldT(p)*dt_veg(p)) &
            -wtgq0*qg(c)-wtaq0(p)*forc_q(g))
         qflx_evap_veg(p) = rpp*efpot(p)
         ...
         ecidif = 0._r8
         if (efpot(p) > 0._r8 .and. btran(p) > btran0) then
            qflx_tran_veg(p) = efpot(p)*rppdry(p)
         else
            qflx_tran_veg(p) = 0._r8
         end if
         ...
         ! Note: This is done later for tracers
...
!cdir nodep
   ...
   ! Reinstate original filter array to include all points
   fn = fnorig
   filterp(1:fn) = fporig(1:fn)

   ! Water tracer fluxes, outside the energy iteration loop
   call t_startf('HydTrcRats')
   call HydrologyTracerRatios(fn, filterp, lbp, ubp, &
                     RCanopyWater, RSnowWater, RXylemWater, &
                     RSoilLiq, RSoilIce, RSoilVol )
   call t_stopf('HydTrcRats')

   ! Leaf conductances, back out from final fluxes, such that they
   ! implictly include the limitations on rpp and rppdry. Finally
   ! scale by wtlq, so that their sums match for reconstructing the
   ! canopy specific humidity, etc.
!dir$ concurrent
!cdir nodep
   do f = 1, fn
       p = filterp(f)
       ...
       delqleaf = qsatlsv(p) - qafsv(p)
       if (abs(delqleaf) > voltiny) then
         wtltrnq    = qflx_tran_veg(p)/(forc_rho(g)*delqleaf)
         wtlsunq(p) = (        fsun(p))*wtltrnq
         wtlshaq(p) = (1._r8 - fsun(p))*wtltrnq
         wtlcanq(p) = qflx_evap_veg(p)/(forc_rho(g)*delqleaf)
         ! For the tracers, basically remove rppdry (where the resistances are
         ! accounted for) and put back in rppdry with kinetic fractionations - TW 24 Oct 2014
         do m = 1, pwtrc
            alpk(1)  = wiso_alpkin_m78(ispec(m),.67_r8,.true.)  ! leaf boundary layer, for transpiration
            alpk(2)  = wiso_alpkin_m78(ispec(m),1.0_r8,.true.)  ! stomata, for transpiration
            if(rppdry(p)==0._r8) then
               wtr_wtlsunq(p,m) = 0._r8
               wtr_wtlshaq(p,m) = 0._r8
            else
               wtr_wtlsunq(p,m) = (wtlsunq(p)/rppdry(p)) * fdry(p)*rb(p)*(laisun(p)/(alpk(1)*rb(p)+alpk(2)*rssun(p)) + &
                                                                          laisha(p)/(alpk(1)*rb(p)+alpk(2)*rssha(p)))/elai(p)
               wtr_wtlshaq(p,m) = (wtlshaq(p)/rppdry(p)) * fdry(p)*rb(p)*(laisun(p)/(alpk(1)*rb(p)+alpk(2)*rssun(p)) + &
                                                                          laisha(p)/(alpk(1)*rb(p)+alpk(2)*rssha(p)))/elai(p)
            end if
            alpk(1)  = wiso_alpkin_m78(ispec(m),.67_r8,.true.) ! leaf boundary layer, for canopy evaporation
            wtr_wtlcanq(p,m) = wtlcanq(p)/alpk(1)
         end do
       else
         wtlsunq(p) = 0._r8
         wtlshaq(p) = 0._r8
         ...
         do m = 1, pwtrc
            wtr_wtlsunq(p,m) = 0._r8
            wtr_wtlshaq(p,m) = 0._r8
            wtr_wtlcanq(p,m) = 0._r8
         end do
       endif

       ! Scale the conductances so that they sum to wtlq0
       wtlnetq     = wtlsunq(p) + wtlshaq(p) + wtlcanq(p)
       if (wtlnetq > 1.e-16) then
         wtlcanq0(p) = (wtlq0(p)/wtlnetq) * wtlcanq(p)
         wtlsunq0(p) = (wtlq0(p)/wtlnetq) * wtlsunq(p)
         wtlshaq0(p) = (wtlq0(p)/wtlnetq) * wtlshaq(p)
       else
         wtlcanq0(p) = 0._r8
         wtlsunq0(p) = 0._r8
         wtlshaq0(p) = 0._r8
       ...
       do m=1, pwtrc
          wtlnetq = wtr_wtlsunq(p,m) + wtr_wtlshaq(p,m) + wtr_wtlcanq(p,m)
          if(wtlnetq > 1.e-16 ) then
             wtr_wtlcanq0(p,m) = (wtr_wtlq0(p,m)/wtlnetq) * wtr_wtlcanq(p,m)
             wtr_wtlsunq0(p,m) = (wtr_wtlq0(p,m)/wtlnetq) * wtr_wtlsunq(p,m)
             wtr_wtlshaq0(p,m) = (wtr_wtlq0(p,m)/wtlnetq) * wtr_wtlshaq(p,m)
          else
             wtr_wtlcanq0(p,m) = 0._r8
             wtr_wtlsunq0(p,m) = 0._r8
             wtr_wtlshaq0(p,m) = 0._r8
          end if
       end do

       ! Compute the canopy water flushing time scale and factor
!       fcaneq(p)   = forc_rho(g)*wtlcanq(p)*max(qsatlsv(p),qafsv(p))/max(h2ocan(p),h2otiny)
       fcaneq(p)   = forc_rho(g)*wtlcanq(p)*max(qsatlsv(p),qafsv(p)) / &
                        max(h2ocan(p)-dtime*qflx_evap_veg(p),h2otiny)
       fcaneq(p)   = exp(-dtime*fcaneq(p))
       if (fwet(p)       .eq. 0.0) fcaneq(p) = 0._r8    ! complete flushing if dry
       if (qflx_evap_veg(p) < 0.0) fcaneq(p) = 0._r8    ! Dew leads to equilibration
...
!cdir nodep
   do f = 1, fn
      ...
      if (qflx_evap_soi(p) > 0._r8 .and. t_grnd(c) > tfrz) then
         fsoieq(p)   = forc_rho(g)*wtgq(p)*qg(c) / &
                max(h2osoi_liq(c,1)-dtime*qflx_evap_soi(p),h2otiny)
         fsoieq(p)   = exp(-dtime*fsoieq(p))
      else
         fsoieq(p) = 1.0_r8             ! no equilibration
      end if

      do m = 1, pwtrc
        RSoilVapor(p,m) = get_wratio(wtr_qg(c,m),wtr_qg(c,ixbase),voltiny)
      end do
   end do      

   ! Save resistances to pointers for history file
   do f = 1, fn
      p = filterp(f)
      raw1(p)=raw(p,1)
      raw2(p)=raw(p,2)
   ...
   ! Compute leaf and canopy air space ratios
   ! (ISOTOPES: as needed)
   call t_startf('HydTrcCan')
   call HydrologyTracerCanopy(fn, filterp, lbp, ubp, lbg, ubg, &
                  qafsv, qsatlsv , wtr_wtaq0   , wtr_wtgq0 , &
                  wtr_wtlsunq0, wtr_wtlshaq0, wtr_wtlcanq0, rb    , forc_pbot, forc_t ,&
                  !!RXylemWater, RCanopyWater, RSoilVapor)
                  RXylemWater, RCanopyWater, RSoilVapor, forc_rho, wtlq0,wtaq0,wtalq)
   call t_stopf('HydTrcCan')

   ! Water isotope tracers
   ! Compute water tracer fluxes directly in three components.
!dir$ concurrent
!cdir nodep
   do f = 1, fn
       p = filterp(f)
       ...
       do m = 1, pwtrc
         ...
         !! Craig-Gordon for VEGETATION EVAPORATION
         if(      hydro_isotope) then
            alpeq = wiso_alpliq(ispec(m),t_veg(p),.true.)
         else
            alpeq = 1._r8
         endif

         wtr_qflx_evap_veg(p,m) = forc_rho(g)*wtr_wtlcanq(p,m)*( (fcaneq(p)*RCanopyWater(p,m)*qsatlsv(p)/alpeq) + &
                                  (1._r8-fcaneq(p))*RCanopyVapor(p,m)*qsatlsv(p) - RCanopyVapor(p,m)*qafsv(p))

         !! Craig-Gordon for VEGETATION TRANSPIRATION
         if(hydro_isotope) then
            alpeq = wiso_alpliq(ispec(m),t_veg(p),.true.)!this is accounted for in HydrologyTracerCanopy?
         else
            alpeq = 1._r8
         endif

         ! Use Craig-Gordon for vegetation transpiration?
         !!wtr_qflx_tran_veg(p,m) = forc_rho(g)* &
         !!     (wtr_wtlsunq(p,m)*((RLeafWaterSun(p,m)*qsatlsv(p)/alpeq) - RCanopyVapor(p,m)*qafsv(p)) &
         !!    + wtr_wtlshaq(p,m)*((RLeafWaterSha(p,m)*qsatlsv(p)/alpeq) - RCanopyVapor(p,m)*qafsv(p)) )
         ! Force what we know should be the steady state value?
         wtr_qflx_tran_veg(p,m) = RXylemWater(p,m)*qflx_tran_veg(p)

         if (btran(p) <= btran0) then
            wtr_qflx_tran_veg(p,m) = 0._r8
         endif

         if (wtr_qflx_tran_veg(p,m) < 0._r8) then
            wtr_qflx_evap_veg(p,m) = wtr_qflx_evap_veg(p,m) + wtr_qflx_tran_veg(p,m)
            wtr_qflx_tran_veg(p,m) = 0._r8
         endif

         ! Special case for frost
         ! ** Do not fractionate if dew/frost onto snow (see below too for wtr_qflx_evap_soi) ** - TW DEBUG
         ! (Dew should be taken care of perfectly fine with fcaneq = 0.)
         if ( wtr_qflx_evap_veg(p,m)<0._r8 .and. t_veg(p) < tfrz ) then
            RFrost = RCanopyVapor(p,m)
            if (hydro_isotope) then
               alpeq = wiso_alpice(ispec(m),t_veg(p),.false.) !don't change - don't fracitonate dew/frost
               RFrost = RFrost*alpeq
            endif
            wtr_qflx_evap_veg(p,m) = qflx_evap_veg(p)*RFrost
         end if

         wtr_qflx_evap_veg(p,m) = min(wtr_qflx_evap_veg(p,m), &
                                      wtr_qflx_tran_veg(p,m)+wtr_h2ocan(p,m)/dtime)

       ! GROUND COMPONENT

       ! Get leaf water isotope ratio 
       if(wtr_wtlsunq0(p,m)+wtr_wtlshaq0(p,m) > 0._r8) then
          RLeafWater(p,m) = wtr_wtlsunq0(p,m)*RLeafWaterSun(p,m) + wtr_wtlshaq0(p,m)*RLeafWaterSha(p,m)
          RLeafWater(p,m) = RLeafWater(p,m)/(wtr_wtlsunq0(p,m)+wtr_wtlshaq0(p,m))
       else
          RLeafWater(p,m) = RCanopyVapor(p,m)
       end if

       ! Craig-Gordon for GROUND EVAPORATION
       !  first guess, modify as needed below
       if(qflx_evap_soi(p) .ge. 0._r8)  wtr_qflx_evap_soi(p,m) = RSoilVapor(p,m)*qflx_evap_soi(p) !bulk evap
       if(qflx_evap_soi(p) .le. 0._r8)  wtr_qflx_evap_soi(p,m) = RCanopyVapor(p,m)*qflx_evap_soi(p) !bulk dew/frost
       !  regular evaporation (eq and kin fractionation)
       if     (qflx_evap_soi(p) > 0._r8 .and. t_grnd(c) >  tfrz) then
          if(snl(c)+1.eq.1) alpeq = wiso_alpliq(ispec(m),t_grnd(c),.true.)
          if(snl(c)+1.ne.1) alpeq = 1._r8 ! no fractionation if explicit snow present
       !  'sublimation' of soil liquid (no fractionation)
       elseif (qflx_evap_soi(p) > 0._r8 .and. t_grnd(c) <= tfrz) then
          alpeq = 1._r8
       !  dew/frost (eq and kin fractionation; no frac if onto snow layers? - TW)
       elseif (qflx_evap_soi(p) < 0._r8) then
          if(snl(c)+1.eq.1) alpeq = wiso_alpliq(ispec(m),t_grnd(c),.true.)
          if(snl(c)+1.ne.1) alpeq = 1._r8 ! no fractionation if explicit snow present
       end if
       wtr_qflx_evap_soi(p,m) = forc_rho(g)*wtr_wtgq(p,m)*( wtalq(p)*(fsoieq(p)*RSoilVapor(p,m) + &
                                (1._r8-fsoieq(p))*RCanopyVapor(p,m))*qg(c) &
                                -wtlq0(p)*RLeafWater(p,m)*qsatl(p) - wtaq0(p)*forc_wtr_q(g,m))

     end do           ! m loop
 end do       ! p loop

   ! With fluxes presumably all found, move on to get various diagnostics
!dir$ concurrent
!cdir nodep
   do f = 1,fn
      p = filterp(f)
      ...
      do m = 1, pwtrc
         wtr_dqh = forc_wtr_q(g,m) - RCanopyVapor(p,m)*qaf(p)
         wtr_q_ref2m(p,m) = forc_wtr_q(g,m) + temp2(p)*wtr_dqh*(1./temp22m(p) - 1./temp2(p))
      end do
      ...
      do m = 1, pwtrc
         wtr_h2ocan(p,m) = max(0._r8,wtr_h2ocan(p,m)+(wtr_qflx_tran_veg(p,m)-wtr_qflx_evap_veg(p,m))*dtime)
         if (h2ocan(p) == 0._r8) then    !if all bulk canopy water gone, get rid of all tracer canopy water - TW
            wtr_qflx_evap_veg(p,m) = wtr_qflx_evap_veg(p,m) + wtr_h2ocan(p,m)/dtime
            wtr_h2ocan(p,m) = 0._r8
         end if
      end do
      ! If not much canopy water left, evaporate all the tracers
      if(h2ocan(p) < 1.e-5_r8) then
         do m = 1, pwtrc
            if(m/=ixbase) then
              wtr_qflx_evap_veg(p,m) = wtr_qflx_evap_veg(p,m) + wtr_h2ocan(p,m)/dtime
              wtr_h2ocan(p,m) = 0._r8
