...
! !USES:
    ...
    use HydrologyTracer , only : pwtrc, wtrcnam, hydro_tracer, get_wratio, &
                                 h2otiny, ixbase, Rstnd, cdbg, pdbg, mdbg, ispec
    use Hydrologyisotope, only : isphdo, isph218o
!
! !ARGUMENTS:
    implicit none
...
!!              end if
!!           end if
            !! Hardwire in for MEF and BAO (same closest coordinates in GNIP
            !! dataset)
            if(ispec(m)==ixbase) cws%RGNIP(:,m)=1._r8
            if(ispec(m)==isphdo) cws%RGNIP(:,m)=0.9068_r8
            if(ispec(m)==isph218o) cws%RGNIP(:,m)=0.9874_r8
        end if
      end do
    end if
   !-------------
