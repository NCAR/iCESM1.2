...
! !INTERFACE:
   ...
   subroutine dynland_hwcontent(begg,endg,gcell_liq,gcell_ice,gcell_heat,wtr_gcell_liq,wtr_gcell_ice)
...
! !ARGUMENTS:
   ...
   real(r8), optional, intent(out) :: wtr_gcell_liq(begg:endg,pwtrc)
   real(r8), optional, intent(out) :: wtr_gcell_ice(begg:endg,pwtrc)
...
!EOP
   ...
   real(r8) :: wtr_liq(pwtrc)! tracer sum of liquid water at column level
   real(r8) :: wtr_ice(pwtrc)! tracer sum of frozen water at column level
   ...
   real(r8), pointer :: wtr_h2osoi_ice(:,:,:)  ! tracer frozen water (kg/m2)
   real(r8), pointer :: wtr_wa(:,:)          ! tracer h2o in underground aquifer
...
!-------------------------------------------------------------------------------
   ...
   do g = begg,endg ! loop over grid cells
      ...
      do m = 1,pwtrc
         wtr_gcell_liq(g,m) = 0.0_r8
         wtr_gcell_ice(g,m) = 0.0_r8
      end do
      ...
      do l = li,lf   ! loop over land units  
         ...
         do c = ci,cf   ! loop over columns
            ...
            .or. (ltype(l) == isturb .and. ctype(c) == icol_road_perv  )) then
               ...
               if ( snl(c) < 0 ) then
                  do k = snl(c)+1,0 ! loop over snow layers
                     ...
                     do m = 1,pwtrc
                        wtr_liq(m)   = wtr_liq(m)   + cws%wtr_h2osoi_liq(c,k,m)
                        wtr_ice(m)   = wtr_ice(m)   + cws%wtr_h2osoi_ice(c,k,m)
                     end do
                  end do
               else                 ! no snow layers exist
                  ice = ice + cws%h2osno(c)
                  do m = 1,pwtrc
                     wtr_ice(m) = wtr_ice(m) + cws%wtr_h2osno(c,m)
                  end do
               end if
            end if
            ...
            .or. (ltype(l) == isturb .and. ctype(c) == icol_road_perv  )) then
               liq = liq + cws%wa(c)
               do m = 1,pwtrc
                  wtr_liq(m) = wtr_liq(m) + cws%wtr_wa(c,m)
               end do
            end if
            ...
            if (ltype(l) == istsoil .or. ltype(l) == istcrop) then   ! note: soil specified at LU level
               ...
               do p = pi,pf ! loop over pfts
                  ...
                  do m = 1, pwtrc
                     wtr_liq(m) = wtr_liq(m) + pws%wtr_h2ocan(p,m) * wtcol
                  end do
               end do
            end if
            ...
            do m=1,pwtrc
               wtr_gcell_liq  (g,m) = wtr_gcell_liq  (g,m) + wtr_liq(m)   * wtgcell
               wtr_gcell_ice  (g,m) = wtr_gcell_ice  (g,m) + wtr_ice(m)   * wtgcell
            end do
