...
!EOP
    ...
    real(r8) :: wtr_init_h2ocan(pwtrc)   ! tracer initial canopy water mass
    real(r8) :: wtr_new_h2ocan(pwtrc)    ! tracer canopy water mass after weight shift
    real(r8), allocatable :: wtr_loss_h2ocan(:,:) ! tracer canopy water mass loss due to weight shift
    type(landunit_type), pointer :: lptr         ! pointer to landunit derived subtype
    type(column_type),   pointer :: cptr         ! pointer to column derived subtype
    type(pft_type)   ,   pointer :: pptr         ! pointer to pft derived subtype
...
!-----------------------------------------------------------------------
    ...
    allocate(wtr_loss_h2ocan(begp:endp,pwtrc), stat=ier)
    if (ier /= 0) then
          write(iulog,*)subname,' allocation error for wtr_loss_h2ocan'; call endrun()
    end if
    ...
    do p = begp,endp
       ...
       if (lptr%itype(l) == istsoil .or. lptr%itype(l) == istcrop) then
          ...
          if (dwt > 0._r8) then
             ...
             do m = 1, pwtrc
               pws%wtr_h2ocan(p,m) = pws%wtr_h2ocan(p,m) * (wtcol_old(p)/pptr%wtcol(p))
             end do
             ...
             do m = 1, pwtrc
                wtr_init_h2ocan(m) = pws%wtr_h2ocan(p,m) * wtcol_old(p)
                wtr_loss_h2ocan(p,m) = pws%wtr_h2ocan(p,m) * (-dwt)
                wtr_new_h2ocan(m) = wtr_init_h2ocan(m) - wtr_loss_h2ocan(p,m)
             end do
             if (abs(new_h2ocan) < 1e-8_r8) then
                new_h2ocan = 0._r8
                loss_h2ocan(p) = init_h2ocan
                do m = 1, pwtrc
                    wtr_new_h2ocan = 0._r8
                    wtr_loss_h2ocan(p,m) = wtr_init_h2ocan(m)
                end do
             end if
             if (pptr%wtcol(p) /= 0._r8) then  
                pws%h2ocan(p) = new_h2ocan/pptr%wtcol(p)
                do m = 1, pwtrc
                   pws%wtr_h2ocan(p,m) = wtr_new_h2ocan(m)/pptr%wtcol(p)
                end do
             else
                pws%h2ocan(p) = 0._r8
                loss_h2ocan(p) = init_h2ocan
                do m = 1, pwtrc
                    pws%wtr_h2ocan(p,m) = 0._r8
                    wtr_loss_h2ocan(p,m) = wtr_init_h2ocan(m)
                end do
             end if 
    ...
    do pi = 1,max_pft_per_col
       do c = begc,endc
          if (pi <= cptr%npfts(c)) then
             ...
             do m = 1, pwtrc
                cwf%wtr_h2ocan_loss(c,m) = cwf%wtr_h2ocan_loss(c,m) + wtr_loss_h2ocan(p,m)/dtime
             end do
          end if
       end do
    end do
    ...
    deallocate(loss_h2ocan, wtr_loss_h2ocan)
