...
! !USES:
  ...
  use subgridAveMod
  use clmtype
  use HydrologyTracer, only: pwtrc, mdbg, pdbg, cdbg
!
! !PUBLIC TYPES:
  implicit none
  save
...
! -----------------------------------------------------------------
    ...
    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%wtr_h2ocan, pws_a%wtr_h2ocan)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RCanopyVapor, pws_a%RCanopyVapor)
    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RCanopyWater, pws_a%RCanopyWater)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RLeafWaterSun, pws_a%RLeafWaterSun)
    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RLeafWaterSunDiag, pws_a%RLeafWaterSunDiag)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RLeafWaterSha, pws_a%RLeafWaterSha)
    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RLeafWaterShaDiag, pws_a%RLeafWaterShaDiag)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pws%RXylem, pws_a%RXylem)
    ...
    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_rain_grnd, pwf_a%wtr_qflx_rain_grnd)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_evap_tot, pwf_a%wtr_qflx_evap_tot)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_snow_grnd, pwf_a%wtr_qflx_snow_grnd)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_snwcp_liq, pwf_a%wtr_qflx_snwcp_liq)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_snwcp_ice, pwf_a%wtr_qflx_snwcp_ice)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_tran_veg, pwf_a%wtr_qflx_tran_veg)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_evap_grnd, pwf_a%wtr_qflx_evap_grnd)

    call p2c(pwtrc, num_allc, filter_allc, pwf%wtr_qflx_evap_soi, pwf_a%wtr_qflx_evap_soi)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_prec_grnd, pwf_a%wtr_qflx_prec_grnd)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_dew_grnd, pwf_a%wtr_qflx_dew_grnd)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_sub_snow, pwf_a%wtr_qflx_sub_snow)

    call p2c(pwtrc, num_nolakec, filter_nolakec, pwf%wtr_qflx_dew_snow, pwf_a%wtr_qflx_dew_snow)
