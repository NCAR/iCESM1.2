...
type, public :: pft_pstate_type
   ...
   real(r8), pointer :: raw1(:)                 !canopy-GCM aerodynamic resistance (s/m)
   real(r8), pointer :: raw2(:)                 !surface-canopy aerodynamic resistance (s/m)
   ...
   real(r8), pointer :: fcaneq(:)               !adjustment fraction for canopy water tracer
   real(r8), pointer :: fsoieq(:)               !adjustment fraction for soil water tracer
...
type, public :: pft_wstate_type
   ...
   real(r8), pointer :: RCanopyVapor(:,:) !tracer ratio of canopy vapor ()
   real(r8), pointer :: RCanopyWater(:,:) !tracer ratio of canopy water ()
   ...
   real(r8), pointer :: RCelluloseSunPsn(:,:)   !tracer ratio of sunlit cellulose times psnsun (incl Peclet)()
   real(r8), pointer :: RCelluloseShaPsn(:,:)   !tracer ratio of shaded cellulose times psnsha (incl Peclet)()
   real(r8), pointer :: RLeafWaterSunDiag(:,:)!tracer ratio of sunlit leaves (incl Peclet)()
   real(r8), pointer :: RLeafWaterShaDiag(:,:)!tracer ratio of shaded leaves (incl Peclet)()
   real(r8), pointer :: RLeafWaterSunStdy(:,:)!tracer ratio of sunlit leaves (steady state)()
   real(r8), pointer :: RLeafWaterShaStdy(:,:)!tracer ratio of shaded leaves (steady state)()
   real(r8), pointer :: RLeafWaterSun(:,:)!tracer ratio of sunlit leaves ()
   real(r8), pointer :: RLeafWaterSha(:,:)!tracer ratio of shaded leaves ()
   real(r8), pointer :: RLeafWaterSunPsn(:,:)!tracer ratio of sunlit leaves times psnsun (incl Peclet)()
   real(r8), pointer :: RLeafWaterShaPsn(:,:)!tracer ratio of shaded leaves times psnsha (incl Peclet)()
   real(r8), pointer :: RXylem(:,:)       !tracer xylem water ratio, from HydrologyTracerRatios, for RTran
   real(r8), pointer :: Rdummy(:,:)       !dummy variable to get CN not to crash (weird memory problem)
...
type, public :: column_wstate_type
   ...
   real(r8), pointer :: forc_wtr_q(:,:)       !tracer atm specific humidity, downscaled to column (kg/kg)
   real(r8), pointer :: forc_wtr_rain(:,:)    !tracer rain rate (mm/s)
   real(r8), pointer :: forc_wtr_snow(:,:)    !tracer snow rate (mm/s)
   real(r8), pointer :: forc_wtr_Rclim(:,:)   !water tracer ratio climatology

   real(r8), pointer :: wtr_qg(:,:)           !tracer ground specific humidity [kg/kg]
   real(r8), pointer :: wtr_h2osno(:,:)       !tracer snow water (mm H2O)
   real(r8), pointer :: wtr_h2osoi_liq(:,:,:) !tracer liquid water (kg/m2) (new) (-nlevsno+1:nlevgrnd)    
   real(r8), pointer :: wtr_h2osoi_liq_save(:,:,:) !tracer liquid water (kg/m2) (new) (-nlevsno+1:nlevgrnd)    
   real(r8), pointer :: wtr_h2osoi_ice(:,:,:) !tracer ice lens (kg/m2) (new) (-nlevsno+1:nlevgrnd)    
   real(r8), pointer :: wtr_h2osoi_liqice_10cm(:,:) !tracer liquid water + ice lens in top 10cm of soil (kg/m2)
   real(r8), pointer :: wtr_h2osoi_vol(:,:,:) !tracer volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]  (nlevgrnd)  
   real(r8), pointer :: wtr_h2osno_old(:,:)         !tracer snow mass for previous time step (kg/m2) (new)
   ...
   real(r8), pointer :: RGNIP(:,:)            !GNIP isotope ratio
...
type, public :: column_wflux_type
   ...
   real(r8), pointer :: wtr_h2ocan_loss(:,:)! tracer mass balance correction term for dynamic weights
...
!
   real(r8), pointer :: wtr_qflx_floodc(:,:)! tracer flood water flux at column level
   ...
   real(r8), pointer :: wtr_qflx_runoff_r(:,:)! tracer Rural total runoff (qflx_drain+qflx_surf+qflx_qrgwl) (mm H2O /s)
   real(r8), pointer :: wtr_qmelt(:,:)      ! tracer snow melt [mm/s]
   real(r8), pointer :: wtr_qflx_rsub_sat(:,:)    ! tracer soil saturation excess [mm/s]
   ...
   real(r8), pointer :: wtr_qflx_snofrz_col(:,:)  ! tracer column-integrated snow freezing rate (positive definite) (col) [kg m-2 s-1]
   real(r8), pointer :: wtr_qflx_glcice(:,:)      ! tracer flux of new glacial ice (mm H2O/s), passed to GLC
   real(r8), pointer :: wtr_qflx_glcice_frz(:,:)  ! tracer ice growth (positive definite) (mm H2O/s)
...
type, public :: gridcell_wstate_type
   ...
   real(r8), pointer :: wtr_gc_liq1(:,:)       ! tracer initial gridcell total h2o liq content
   real(r8), pointer :: wtr_gc_liq2(:,:)       ! tracer post land cover change total liq content
   real(r8), pointer :: wtr_gc_ice1(:,:)       ! tracer initial gridcell total h2o liq content
   real(r8), pointer :: wtr_gc_ice2(:,:)       ! tracer post land cover change total ice content
...
type, public :: gridcell_wflux_type
   ...
   real(r8), pointer :: wtr_qflx_runoffg(:,:)      ! tracer total grid-level liq runoff
   real(r8), pointer :: wtr_qflx_snwcp_iceg(:,:)   ! tracer total grid-level ice runoff
   ...
   real(r8), pointer :: wtr_qflx_floodg(:,:) ! tracer total grid-level flood water flux
