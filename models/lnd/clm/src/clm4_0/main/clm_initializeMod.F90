...
! !USES:
    ...
    use clm_glclnd      , only : init_glc2lnd_type, init_lnd2glc_type, &
                                 clm_x2s, clm_s2x
    use HydrologyTracer , only : HydrologyTracerInit, hydro_tracer, &
                                    pwtrc, wtrcnam, pdbg, mdbg, cdbg
    use HydrologyIsotope, only : HydrologyIsotopeInit, hydro_isotope
!!
    use clmtype
...
!if(begp<pdbg .and. endp>pdbg) ldbg=.true.
    ...
    ! ------------------------------------------------------------------------
    ! Initialize isotopes variables/tracer hydrology
    ! ------------------------------------------------------------------------

    ! Initialize water isotope variables
    if (hydro_isotope) call HydrologyIsotopeInit()

    ! Initialize tracer hydrology and, optionally, water isotopes
    if (hydro_tracer) then
        call HydrologyTracerInit()
    else
      write(6,*) 'This code has hydrology tracers hardwired in...probably.'
      call endrun('Failed to set hydro_tracer = .true.')
    end if
