...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  public :: htapes_check         ! Debugging routine to check fields for errors (TW)
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------
    ...
    str = &
    '$Id: histFileMod.F90 70608 2015-05-08 20:41:02Z anthony.e.wong@colorado.edu $'
...
! !USES:
    use clmtype
    use HydrologyTracer , only : pdbg, mdbg, pwtrc
!
! !ARGUMENTS:
    implicit none
    character(len=*), intent(in)           :: fname          ! field name
...
!if(begp<=pdbg .and. pdbg<=endp .and. (fname=='QVEGT' .or. fname=='QVEGT_H2OTR')) ldbg=.true.
       ...
       if (present(set_lake)) then
          do p = begp,endp
             ...
             if (lun%lakpoi(l))  then
                ! For some reason, this is necessary (deep lakes not being set
                ! right...) - TW 29 September 2014
...
!BOP
!
! !IROUTINE: htapes_check
!
! !INTERFACE:
  subroutine htapes_check(t, sfield)
!
! !DESCRIPTION:
! Check history tape for errors (NaNs, mostly) in certain fields.
!
! !USES:
    use clmtype
    use domainMod , only : ldomain
...
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: t                ! tape index to check
    character(len=*), intent(in) :: sfield  ! name (string) of field to check
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein
!
...
!EOP
    ...
    character(len=32) :: dim2name        ! temporary
    real(r8), pointer :: histo(:,:)      ! temporary
    character(len=*),parameter :: subname = 'htapes_check'
!-----------------------------------------------------------------------
    ...
    do f = 1,tape(t)%nflds
       ...
       if(varname==sfield) then
          write(6,*) 'htapes_check: ',sfield
          write(6,*) 'htapes_check: histo=',histo
       end if

    end do

  end subroutine htapes_check
